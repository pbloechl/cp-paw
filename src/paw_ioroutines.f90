...
!
!     ==  ALLENERGY TRAJECTORY =================================================
      CALL TRAJECTORYIO$NEW(+'ALLENERGY-TRAJECTORY')
      CALL TRAJECTORYIO$SELECT('ALLENERGY-TRAJECTORY')
      CALL TRAJECTORYIO$SETCH('FILE',-'*_EALL.TRA')
      CALL TRAJECTORYIO$SELECT('NONE')
!
!     ==  BANDS  TRAJECTORY ====================================================
...
!     ==  ALLENERGY TRAJECTORY                                        ==
!     ==================================================================
      CALL LINKEDLIST$EXISTD(LL_CNTL,'EALL',0,TCHK)
      IF(.NOT.TCHK)CALL LINKEDLIST$SET(LL_CNTL,'EALL',0,.FALSE.)
      CALL LINKEDLIST$GET(LL_CNTL,'EALL',1,TCHK)
      CALL TRAJECTORYIO$SELECT('ALLENERGY-TRAJECTORY')
      CALL TRAJECTORYIO$SETL4('ON',TCHK)
      CALL TRAJECTORYIO$SETI4('SKIP',NSKIP)
      CALL TRAJECTORYIO$SELECT('NONE')
