...
%
\item for the tailed orbitals the angular momentum expansion on the
  central site may not eb sufficient. I should include a on-site
  structure constants with an increased number of tail functions.
%
\item Rule: (1) Each head function has exactly one tail function
  attributed to it.  Thus we can identify the phidot function uniquely
  by looking for the tail function with the correct angular momentum.
 From (1) follows, that there is at most one tail
  function per site and angular momentum.
%
\item In the future, we may extend the tailed representation of
  tight-binding orbitals by including additional tail functions,
  ``higher'' partial waves. They correspond to the un-augmented tails
  of the neighboring sites. These additional tail functions have the
  character of Bessel functions in the sphere and contain tailes
  outside the sphere.
%
\item The orbitals in the tailed one-center expansion and as
  ...
  Calculations for hybrogen suggest that a good choice is $K2=-1.$ and
...
%
\item Suggestion: the largest problem is that the tails often so not
  get the true extent of teh multicenter expansion correct. A good
  idea may be to use a Hankel function augmented by bare Bessel
  functions and $\dot{\bar{\phi}}$ centered on the neighboring
  site. These functions can be expanded about the central site into
  spherical harmonics.  These functions must be expandable into the
