...
\item With the introduction of the tailed representation of the NTBO's
  ...
  Currently we use the following rule: Rule: (1) Each head function
  has exactly one tail function attributed to it.  Thus we can
  identify the phidot function uniquely by looking for the tail
  function with the correct angular momentum.  From (1) follows, that
  there is at most one tail function per site and angular momentum.
%
\item Suggestion: the exponential functions for the tailed expansion
  ...
  neighboring site, wich is augmented by (a) a bare Bessel function
  and (b) by a screened scattering partial wave. (1a) In order to make
  the formulatiuon less dependent on the neighbor distance, we could
  also include the gradients of these functions. (2) Thes functions
  are expanded about the central site into an angular momemtum
  expansion.  (3) a reasonably weighted least square fit of the
  exponential tails to these functions will provide a ``optimum
  shape'' of the tails for each angular momentum.
%
...
\end{eqnarray}
  ...
  into head and tail functions is the \textbf{interstitial envelope
    function}\index{interstitial envelope function}
  $|K^I_\alpha\rangle$.
%
...
%====================================================================
Defining the envelope function via a isotropic and translationally
...
%====================================================================
A so-called \textbf{screened LMTO representations}\index{screened
...
bare Hankel function is considsered.
...
\begin{center}
\includegraphics[width=0.4\linewidth,clip=true]{Figs/Eofk2si/eofk2si110.eps}
\includegraphics[width=0.4\linewidth,clip=true]{Figs/Eofk2si/gapofk2si110.eps}
\end{center}
%
%================= raug ================================================
...
\begin{center}
\includegraphics[width=0.5\linewidth,clip=true]{Figs/Eofraugsi/eofraug_110.eps}
\includegraphics[width=0.4\linewidth,clip=true]{Figs/Eofraugsi/gapofraugsi110.eps}\hfill
\end{center}
...
\begin{center}
\includegraphics[width=0.4\linewidth,clip=true]{Figs/Eofrtailsi/eofrtailsi110.eps}
\includegraphics[width=0.4\linewidth,clip=true]{Figs/Eofrtailsi/eofrtailsi110hr.eps}
\\
\includegraphics[width=0.4\linewidth,clip=true]{Figs/Eofrtailsi/gapofrtailsi110.eps}
\end{center}
\end{itemize}

The augmentation radius has the largest effects on the results both
for the gap and for the total energy. Beyond a certain radius the
calculation becomes even unstable.

It becomes evident that the energy is rather insensitive to the
parameters describing the local orbitals for local exchange and the
NDDO terms. As soon as the bond overlap density
$\chi_R(r)\chi_{R'}(r)$ is included the dependency becomes very
important. Note that 0.05~H$\approx$1.4~eV!

It should be noted that the double counting term is included only for
local exchange.
