...
following form:
\begin{eqnarray}
E_{xc}^{\hat{W}_R}=\int d^3r\; n_t(\vec{r})
\epsilon_{xc}[n_t(\vec{r})]
\left(\frac{n_R(\vec{r})}{n_t(\vec{r})}\right)^2
\end{eqnarray}
With $f_{xc}:=n_t\epsilon_{xc}[n_t]$ and
$\mu_{xc}:=\frac{df_{xc}}{dn}$, we obtaim
\begin{eqnarray}
dE_{xc}^{\hat{W}_R}
&=&\int d^3r\; 
\left(\frac{n_R}{n_t}\right)^2\mu_{xc} dn_t
+f_{xc}2\left(\frac{n_R}{n_t}\right)^2\frac{dn_R}{n_R}
-f_{xc}2\left(\frac{n_R}{n_t}\right)^2\frac{dn_t}{n_t}
\nonumber\\
&=&\int d^3r\; 
\biggl[
\left(\frac{n_R}{n_t}\right)^2\mu_{xc} 
-2f_{xc}\left(\frac{n_R}{n_t}\right)^2\frac{1}{n_t}\biggr]dn_t
+\biggl[2f_{xc}\left(\frac{n_R}{n_t}\right)^2\frac{1}{n_R}\biggr] dn_R
\end{eqnarray}
