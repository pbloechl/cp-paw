...
\eq{eq:deflocalorbital} is used with the terms ``small correction''
...
\eq{eq:orbitalprojectorbyfit}, and by projection,
\eq{eq:orbitalprojectorbyprojection}, are identical.
...
\includegraphics[width=0.4\textwidth]{Figs/Dihydrogenion/x22ofrcorr}
\end{center}
The graph shows (left) the 22-matrix element and (right) the 22
matrix element with the long-range part subtracted. Also shown is
(red,dash-dotted) the function $\pm d^{-1}$ and (blue,dashed)
$\e{-2r/a_0}$. The green spheres are the analytic result.
    ...
    The agreement of analytic and with the code is perfect.
    \item undo the changes made for the test of
      \verb|SIMPLELMTO_OFFSITEX22SETUP|.
  \end{itemize}
%
\item \verb|SIMPLELMTO_OFFSITEX31SETUP| is tested as follows:
  \begin{itemize}
    \item I set \verb|TPR=.true.|, which writes the integrals into the
      file X22.DAT and terminates after writing.
    \item run the code for a hydrogen atom or molecule with NPRO=1 and
      offsite exchange on.
    \item inspect the file \verb|X31.DAT| and compare with analytical
      result. I find precise agreement.
    \item undo the changes made for the test of
      \verb|SIMPLELMTO_OFFSITEX31SETUP|
\begin{center}
\includegraphics[width=0.4\textwidth]{Figs/Dihydrogenion/x31ofr}
\end{center}
The graph shows the 31-matrix element.  The green spheres are the
analytic result.
...
%% \subsubsection{Lindhard screening}

%% \begin{eqnarray}
%% k_F&=&2\pi\left(\frac{3}{4\pi\sum_\sigma}n\right)^{\frac{1}{3}}
%% \nonumber\\
%% n&=&\frac{\sum_\sigma}{(2\pi)^3}\left(\frac{4\pi}{3}k_F^3\right)
%% \end{eqnarray}



%% \begin{eqnarray}
%% v_x(\sigma,\sigma',\vec{r})
%% &=&\frac{-e^2}{4\pi\epsilon\delta_{\sigma,\sigma'}
%% |\vec{r}|}\frac{1}{(2\pi)^3 \frac{4\pi}{3}k_F^3
%% \left(3 \frac{k_F|\vec{r}|\cos(k_F|\vec{r}|)-\sin(k_F|\vec{r}|)}
%% {(k_F|\vec{r}|)^3}
%% \nonumber\\
%% &=&\frac{-e^2}{4\pi\epsilon\delta_{\sigma,\sigma'}
%% |\vec{r}|}n
...
from \eq{eq:rotationdefyvectors}. Let me calculate their derivatives.
...
%===================================================================
\editor{Caution! The section for the hydrogen atom and the molecule
  habe been included in the lecture notes for $\Phi$SX:Advanced Solid
  State Theory on May 11, 2020. The PAW tests at the end are not
...
The U-tensor is twice the self energy, \eq{eq:hatomeself}, i.e.
\begin{eqnarray}
...
The \textbf{Coulomb integral}\index{Coulomb integral} $J$ is the
...
The \textbf{resonance integral}\index{resonance integral} $K$ describes the overlap density of two orbils on different sites with the nucleus of one site
\begin{eqnarray}
...
\eq{eq:potentialfromelectronshatom}
\begin{eqnarray}
