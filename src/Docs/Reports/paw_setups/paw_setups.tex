...
so that
\begin{eqnarray}
|q_n^{(j)}(\epsilon)\rangle=\sum_{j=0}^\infty 
|q_n^{(j)}(\epsilon_\nu)\rangle
\Bigl[-(\epsilon-\epsilon_\nu)\Bigr]^j
\end{eqnarray}

The sign and the factorial have been included so that the differential
equation for the Taylor coefficients is similar to that of the
...
\nonumber\\
(\hat{h}-\epsilon_\nu)|q_n^{(j)}(\epsilon)\rangle&=&-|q_n^{(j-1)}\rangle
\qquad\text{for $j>0$}
\end{eqnarray}

The Taylor-expansion coefficients start near the origin with a high
order, $\sim r^{\ell+2n+2j}$, and they usually increase exponentially
at large distances.

The all-electron partners are obtained as follows from the node-reduced
partial waves
\begin{eqnarray}
|\phi_n^{(j)}(\epsilon_\nu)\rangle
&\defas&\frac{(-1)^j}{j!}\left.\partial_\epsilon^{j}\right|_{\epsilon_\nu}
\biggl[|\phi_n(\epsilon)\rangle
\frac{1}{\prod_{k=1}^{n-1}(\epsilon_j-\epsilon)}\biggr]
=|q_n^{(j)}(\epsilon_\nu)\rangle+\sum_{m=1}^{n-1}|u_m\rangle c_{m,j}
\label{eq:taylorexpansioncoefficientsaephi}
\end{eqnarray}
with coefficients $c_{m,j}$ derived in appendix~\ref{app:tayloraephi}.
...
functions
\begin{eqnarray}
|p'^{(j)}\rangle&\defas&\frac{(-1)^j}{j!}
\partial_\epsilon^j|p'(\epsilon)\rangle
\nonumber\\
&=&
\Bigl[-|u_{n-1}\rangle
