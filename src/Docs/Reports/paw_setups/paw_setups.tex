...
\item set the condition in the input that the outermost grid point
  ...
  \verb|atomlib$aescf|
\item RCBG is set much too small! (see \verb|setup$select|)
\end{itemize}
...
Caution the parameter \verb|This%rcbg=1/sqrt(0.218)| is set in in
$\verb|setup$select|$, when a new setup is created. This radius
corresponds to only 1.13~\AA. WARNING: This is much too small!


\begin{itemize}
\item \verb|setup$readstrcin| retrieves the following data from the
  \verb|!structure!species|-blocks and inserts them into the
  \verb|this|-structure:
\begin{verbatim}
ID,I,lnx,lmnx,lmrx,gid(->shlog,r1,dex,nr),m,rad,
parms(id,aez,rcsm,zv,type,rcl,lambda
     ,pow\_pot,rc\_pot,tval0_potm,val0_pot
     ,pow\_core,rc\_core,tval0_core,val0_core)
\end{verbatim}
%
%
\item \verb|setup$read->setup_read|: \verb|setup$read| is the main
  external call to initialize the setups object. It calls
  \verb|setup_read| for each atom species.
%
\begin{enumerate}
\item \verb|atomlib$aescf|: Self consistent spherical atom on a radial
  grid. Produced are
\begin{verbatim}
etot,pot,vfock
...
The occupations are collected from periodictable.

It uses a key to encode how the calculation is to be done, The key
...
\verb|NONREL,NONSO,NONZORA|, which stop the code if they conflict with
a positive key.
\begin{center}
...
\end{center}
%
\item \verb|setup_coreselect|: $ZV$ is used to determine the core
  states. (atomic states are reordered if necessary.)
  \verb|This%aepsi| and \verb|This%aepsism| are set.
%
\item Core densities. \verb|This%aecore| and \verb|This%pscore| are set.
%
\item \verb|setup_makeiscatt|. \verb|This%iscatt| is set.
%
...
%======================================================
\subsection{Previous sdescription}
%======================================================
