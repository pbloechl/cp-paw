...
\sum_{c;\sigma_c=\sigma_\alpha}
\sum_{L}C_{L_\alpha,L_c,L}C_{L_c,L_\beta,L}
\nonumber\\
&&\times\int d^3r\int d^3r'
\frac{e^2
\phi_{\beta}(r)\psi_{c}(r)Y_L(\vec{r})
\psi^*_c(r')
\phi_\alpha(r')Y_{L}(\vec{r'})}{4\pi\epsilon_0|\vec{r}-\vec{r'}|}
\nonumber\\
&=&-
...
\phi_\alpha(r)Y_{L_\alpha}(\vec{r})
\nonumber\\
\psi_c(\vec{x})&=&\delta_{\sigma,\sigma_c}
\phi_c(r)Y_{L_c}(\vec{r})
\end{eqnarray}
...
\begin{eqnarray}
M_{\alpha,\beta}&=&-\sum_{c} \sum_{\ell_\rho}\delta_{\ell_\alpha,\ell_\beta}
\underbrace{\biggl[\frac{4\pi}{2\ell_\rho+1}\sum_{m_\rho,m_c} C_{L_\alpha,L_c,L_\rho}^2\biggr]
}_{F(\ell_\alpha,\ell_\rho,\ell_c)}
V_{\alpha,c,\ell_\rho}(r)
\nonumber\\
V_{\alpha,c,\ell_\rho}(r)Y_{L_\rho}(\vec{r})&=&
\int_0^\infty dr\; r^2\phi_c(r)\phi_\beta(r)
\biggl[\int d^3r'\;\frac{e^2\phi_\alpha(r')\phi_c(r')Y_{L_\rho}(\vec{r'})}
