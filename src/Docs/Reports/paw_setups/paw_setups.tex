...
%====================================================================
\section{Nodeless construction}
%====================================================================
The nodeless construction is proposed by Bl\"ochl and
F\"orst\cite{bloechl12_arxiv1210_5937}.  The present description
combines a few concepts from the earlier derivation.


%====================================================================
\subsection{Energy-dependent partial waves}
%====================================================================
...
radial Schr\"odinger equation outward 
\begin{eqnarray}
...
\end{eqnarray}
at the origin. That is, the radial parts\footnote{The partial waves
  are obtained from the radial parts by multiplication with the
  corresponding real spherical harmonic.} of all partial wave behave
at the origin like $r^\ell$. We use the subscript ``$ref$'' to distinguish
it from the partial waves introduced later, which differ by an
...
%====================================================================
\subsection{Node-reduced energy-dependent partial waves}
%====================================================================
Consider an arbitrary sequence $\{\epsilon_n; n>0\}$ of energies.

For a Hamiltonian $\hat{h}$, define the sequence of energy-dependent,
\textbf{node-reduced partial waves}\index{node-reduced partial waves}
 $|q_n(\epsilon)\rangle$ by the following
recursion
\begin{eqnarray}
(\hat{h}-\epsilon)|q_n(\epsilon)\rangle=-|q_{n-1}(\epsilon_{n-1})\rangle
\end{eqnarray}
The sequence starts with
$|q_1(\epsilon)\rangle=|\phi_{ref}(\epsilon)\rangle$ defined above.
The boundary condition for $n>1$ is that $q_n(\epsilon,\vec{r})$
vanishes at the origin.

Let me define 
\begin{eqnarray}
|u_n\rangle\defas|q_n(\epsilon_n)\rangle
\end{eqnarray}
which I will call in the following \textbf{nodeless partial
  waves}\index{nodeless partial waves}.

The energy-dependent partial wave can be reconstructed with the ansatz
\begin{eqnarray}
|\phi_{ref}(\epsilon)\rangle=
\biggl\lbrace |q_n(\epsilon)\rangle
+\sum_{j=1}^{n-1}|u_j\rangle c_j\biggr\rbrace K
\end{eqnarray}

 
Let me determine the coefficients by insertion into the Schr\"odinger
equation
\begin{eqnarray}
(\hat{h}-\epsilon)|\phi_{ref}(\epsilon)\rangle\frac{1}{K}
&=&
(\hat{h}-\epsilon)
|q_n(\epsilon)\rangle
+\sum_{j=1}^{n-1}(\hat{h}-\epsilon_j)|u_j\rangle c_j
+\sum_{j=1}^{n-1}(\epsilon_j-\epsilon)|u_j\rangle c_j
\nonumber\\
&=&
-|u_{n-1}\rangle
-\sum_{j=1}^{n-1}|u_{j-1}\rangle c_j
+\sum_{j=1}^{n-1}|u_j\rangle(\epsilon_j-\epsilon) c_j
\nonumber\\
&=&
-|u_{n-1}\rangle
-\sum_{j=0}^{n-2}|u_{j}\rangle c_{j+1}
+\sum_{j=1}^{n-1}|u_j\rangle(\epsilon_j-\epsilon) c_j
\nonumber\\
&=&
|u_{n-1}\rangle\Bigl(-1+(\epsilon_{n-1}-\epsilon) c_{n-1}\Bigr)
\nonumber\\
&+&\sum_{j=0}^{n-2}|u_{j}\rangle 
\Bigl(-c_{j+1}+(\epsilon_j-\epsilon) c_j\Bigr)
-\underbrace{|u_0\rangle}_{=|\zero\rangle} c_1
\stackrel{!}{=}0
\end{eqnarray}
The requirement that the inhomogeneity vanishes, yields
\begin{eqnarray}
c_j=\prod_{k=j}^{n-1}\frac{1}{\epsilon_k-\epsilon}\;.
\end{eqnarray}

The initial conditions at the origin require that the coefficient of
$|u_1\rangle$ in $|\phi_{ref}(\epsilon)\rangle$ is equal to one, i.e.
\begin{eqnarray}
K=c_1^{-1}=\prod_{k=1}^{n-1}(\epsilon_k-\epsilon)\;.
\end{eqnarray}

\begin{myshadowminipage}{energy dependent partial waves in terms of node-reduced partial waves}

The energy-dependent partial wave $|\phi_n(\epsilon)\rangle$ which
obeys the Schr\"odinger equation
$(\hat{h}-\epsilon)|\phi_n(\epsilon)\rangle=0$, can be expressed as
\begin{eqnarray}
|\phi_{ref}(\epsilon)\rangle
&=&|q_n(\epsilon)\rangle \prod_{k=1}^{n-1}(\epsilon_k-\epsilon)
+\sum_{j=1}^{n-1}|u_j\rangle \prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
\label{eq:energydependentpartialwavefromnodeless}\\
&=&\biggl(
|q_n(\epsilon)\rangle 
+\sum_{j=1}^{n-1}|u_j\rangle \prod_{k=j}^{n-1}\frac{1}{\epsilon_k-\epsilon}
\biggr)\prod_{k=1}^{n-1}(\epsilon_k-\epsilon)
\end{eqnarray}
with
\begin{eqnarray}
|u_n\rangle&\defas&|q_n(\epsilon_n)\rangle
...
\frac{q_n(\epsilon,r)}{|\vec{r}|^\ell Y_{\ell,m}(\vec{r})}
&=&\delta_{n,1}
\end{eqnarray}
Partial waves $|\phi_n(\epsilon)\rangle$ with different indices $n$
differ only by a factor.
\end{myshadowminipage}

%====================================================================
\subsection{Node-reduced partial waves near the origin}
%====================================================================
...
When $n$ increases by one, the partial wave $q_n(\epsilon)$ at the
origin starts with a power of $r$ increased by two orders.

%====================================================================
\subsection{Energy derivatives}
%====================================================================
The following relation provides a means to construct the node-reduced
partial waves $q_n(\epsilon)$ from $|\phi_n(\epsilon)\rangle$. It
shows furthermore, that a Taylor expansion in energy can be
constructed by a sequence with equal energies $\epsilon_n$.

Consider the following relation
\begin{eqnarray}
|q_{n+1}(\epsilon)\rangle&=&
\begin{cases}
-\frac{|q_n(\epsilon)\rangle-|q_n(\epsilon_n)\rangle}
{\epsilon-\epsilon_n}
&\text{for $\epsilon\neq\epsilon_n$}\\
-\partial_\epsilon|q_n(\epsilon)\rangle
&\text{for $\epsilon=\epsilon_n$}
\end{cases}
\label{eq:qnplus1}
\end{eqnarray}
...
\begin{eqnarray}
(\hat{h}-\epsilon)
\left[-\frac{|q_n(\epsilon)\rangle-|q_n(\epsilon_n)\rangle}
{\epsilon-\epsilon_n}\right]
&=&-\frac{(\hat{h}-\epsilon)|q_n(\epsilon)\rangle-
(\hat{h}-\epsilon_n)|q_n(\epsilon_n)\rangle
+(\epsilon-\epsilon_n)|q_n(\epsilon_n)\rangle}
{\epsilon-\epsilon_n}
\nonumber\\
&=&-\frac{|u_{n-1}\rangle-
|u_{n-1}\rangle
...
&=&-\frac{|u_{n-1}\rangle-
|u_{n-1}\rangle
+(\epsilon-\epsilon_n)|u_n\rangle}
{\epsilon-\epsilon_n}
\nonumber\\
&=&-|u_n\rangle
\end{eqnarray}
This relation shows that the function
$-(|q_n(\epsilon)\rangle-|q_n(\epsilon_n)\rangle)/(\epsilon-\epsilon_n)$
is equal to $|q_{n+1}(\epsilon)\rangle$, q.e.d.

%====================================================================
\subsection{Taylor expansion}
%====================================================================
Here, I show that the individual terms $|u_{n+j}\rangle$ with $j>0$
are related to the coefficients of a Taylor expansion for
$q_n(\epsilon)$ in the energy, if all energies in the sequence with
$\{\epsilon_j\}$ with $j\ge n$ are identical.

Let me start with the identity \eq{eq:qnplus1} obtained above.
\begin{eqnarray}
|q_{n+1}(\epsilon)\rangle
&\eqrel{eq:qnplus1}{=}&
-\frac{1}{\epsilon-\epsilon_n}\Bigl(
|q_n(\epsilon)\rangle-|q_n(\epsilon_n)\rangle\Bigr)
\nonumber\\
\Rightarrow\qquad
\big|q_n(\epsilon)\big\rangle&=&\big|q_n(\epsilon_n)\big\rangle
-(\epsilon-\epsilon_n)\big|q_{n+1}(\epsilon)\big\rangle
\nonumber\\
&=&\big|q_n(\epsilon_n)\big\rangle
-(\epsilon-\epsilon_n)
\Bigl(\big|q_{n+1}(\epsilon_{n+1})\big\rangle
-(\epsilon-\epsilon_{n+1})
\big|q_{n+2}(\epsilon)\big\rangle\Bigr)
\nonumber\\
&\stackrel{\epsilon_{n+1}=\epsilon_n}{=}&\big|q_n(\epsilon_n)\big\rangle
-(\epsilon-\epsilon_n)
\big|q_{n+1}(\epsilon_{n+1})\big\rangle
+(\epsilon-\epsilon_{n})^2
\big|q_{n+2}(\epsilon)\big\rangle\Bigr)
\nonumber\\
%
%
&\stackrel{\epsilon_{n+j}=\epsilon_n}{=}&
(-1)^{j+1}(\epsilon-\epsilon_n)^{j+1}|q_{n+j}(\epsilon)\rangle
+\sum_{k=0}^j (-1)^k(\epsilon-\epsilon_n)^k|u_{n+k}\rangle
\nonumber\\
%
%
&=&
\sum_{k=0}^\infty (-1)^k(\epsilon-\epsilon_n)^k|u_{n+k}\rangle
\qquad\text{for $\epsilon_{n+k}=\epsilon_n$ with $k\ge0$}
\end{eqnarray}

Thus, I obtain the Taylor expansion coefficients as
\begin{eqnarray}
|q_{n+j}(\epsilon_n)\rangle=\frac{(-1)^j}{j!}
\left.\partial_\epsilon^j\right|_{\epsilon_n} q_n(\epsilon)
\qquad\text{if $\epsilon_k=\epsilon_n$ for $0<k<j$}
\end{eqnarray}
...
%====================================================================
The sequence of energy-dependent partial waves $|q_n(\epsilon)\rangle$
is defined by the hamiltonian $\hat{h}$ and the sequence of energies
$\epsilon_n$.

We intend to choose the energies as follows: Let $N_c$ be the number
of core wave functions and $N_v$ the number of valence wave functions.
\begin{enumerate}
\item For $n\le N_c+N_v$ the energies are chosen equal to the bound
  states of the Hamiltonian.
\item The remaining energy levels, which describe scattering states
  are chosen identical to the energy $\epsilon_{N_c+N_v}$ of the
...
\begin{itemize}
\item that the inhomogeneities for the bound states are themselfes
  limited to the atomic region. Hence, the $|q_n(\epsilon)\rangle$
  outside the atomic region is, up to a factor, equal to
  $|\phi_n(\epsilon)\rangle$.
\item that the $|q_n(\epsilon)\rangle$ for the scatterning states are the
  Taylor expansion coefficients of and expansion of the partial wave
  in terms of energy, analogous to the $\phi,\dot{\phi}$ expansion of
  the linear methods.
\end{itemize}

%====================================================================
\section{Augmentation from node-reduced wave functions}
%====================================================================
%====================================================================
\subsubsection{Pseudization}
%====================================================================
We introduce the pseudization on the level of energy-dependent partial
waves, \eq{eq:energydependentpartialwavefromnodeless}, by replacing the
core contribution by another, nearly energy-independent function
$|k(\epsilon)\rangle$
\begin{eqnarray}
\phi_{AE}(\epsilon)
&=&
\underbrace{
|q_{N_c+N_v+1}(\epsilon)\rangle\prod_{k=1}^{N_c+N_v}(\epsilon_k-\epsilon)
}_{\text{scattering}}
+
\underbrace{
\sum_{j=N_c+1}^{N_c+N_v}|u_j\rangle \prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
}_{\text{valence}}
+
\underbrace{
\sum_{j=1}^{N_c}|u_j\rangle \prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
}_{\text{core}}
 \nonumber\\
%
%
%
\tilde{\phi}_{PS}(\epsilon)
&=&
\underbrace{
|q_{N_c+N_v+1}(\epsilon)\rangle\prod_{k=1}^{N_c+N_v}(\epsilon_k-\epsilon)
}_{\text{scattering}}
...
+k(\epsilon)\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\label{eq:phiaeandpsofenergy}
\end{eqnarray}
Beyond the augmentation region, the function $|k(\epsilon)\rangle$
should ideally be identical to the contribution of the core states.
That is, $|k(\epsilon)\rangle$ must be localized in the core region.
...
%% &=&
%% \underbrace{
%% \big|q_{N_C+N_V+1}(\epsilon)\big\rangle
%% \prod_{m=N_c+1}^{N_c+N_v}(\epsilon_m-\epsilon)
%% }_{\text{scattering}}
%% +
%% \underbrace{
%% \sum_{j=N_c+1}^{N_c+N_V}\big|u_j\big\rangle \prod_{m=N_c+1}^{j-1}(\epsilon_m-\epsilon)
%% }_{\text{valence}}
%% +
%% \underbrace{
%% \sum_{j=1}^{N_c}\big|u_j\big\rangle \prod_{k=j}^{N_c}\frac{1}{\epsilon_k-\epsilon}
%% }_{\text{core}}
%% \nonumber\\
%% %
%% %
...
\begin{eqnarray}
|\phi_{AE}(\epsilon)\rangle&=&|\tilde{\phi}_{PS}(\epsilon)\rangle
+\Bigl(|\phi_{AE}(\epsilon)\rangle-|\tilde{\phi}_{PS}(\epsilon)\rangle\Bigr)
\langle\tilde{p}(\epsilon)|\tilde{\phi}_{PS}(\epsilon)\rangle
%% \nonumber\\
%% \Rightarrow\qquad|\phi_{AE}(\epsilon)\rangle-|\tilde{\phi}_{PS}(\epsilon)\rangle
%% &=&\Bigl(|\phi_{AE}(\epsilon)\rangle-|\tilde{\phi}_{PS}(\epsilon)\rangle\Bigr)
%% \langle\tilde{p}(\epsilon)|\tilde{\phi}_{PS}(\epsilon)\rangle
%% \nonumber\\
%% &=&
...
%% \langle\tilde{p}(\epsilon)|\tilde{\phi}_{PS}(\epsilon)\rangle
\end{eqnarray}
We notice that the scattering contribution in the augmentation part
can be skipped. In the nodeless construction also the valence
contribution can be skipped

The projection can be constructed such that the bi-orthogonality and
the closure relations are obeyed.  The closure relation is defined for
a specified pseudo-potential $\tilde{v}$ ($\hat{t}$ is the kinetic
energy operator)
\begin{eqnarray}
0&=&
\Bigl(\hat{t}+\hat{\tilde{v}}+|\tilde{p}(\epsilon)\rangle A(\epsilon)\langle\tilde{p}(\epsilon)|-\epsilon\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle
\nonumber\\
%
%
\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle
&=&-|\tilde{p}(\epsilon)\rangle A(\epsilon)\langle\tilde{p}(\epsilon)|\tilde{\phi}_{PS}(\epsilon)\rangle
\end{eqnarray}
This equation tells the shape of the projector function. Its norm is
defined by the bi-orthogonality relation
\begin{eqnarray}
1&=&\langle\tilde{p}(\epsilon)|\tilde{\phi}_{PS}(\epsilon)\rangle
\end{eqnarray}
which determines the projector function as
\begin{eqnarray}
|\tilde{p}(\epsilon)\rangle&=&
\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle 
\Bigl[
\langle\tilde{\phi}_{PS}(\epsilon)|
\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)
|\tilde{\phi}_{PS}(\epsilon)\rangle 
\Bigr]^{-1}
\nonumber\\
\end{eqnarray}

Now, I insert the pseudo-partial wave obtained above in
\eq{eq:phiaeandpsofenergy}
\begin{eqnarray}
&&\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle 
=\Bigl(\hat{h}-\epsilon+(\hat{\tilde{v}}-\hat{v})\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle 
\nonumber\\
%
%
%
&=&\Bigl(\hat{h}-\epsilon\Bigr)
\biggl[|\phi_{AE}(\epsilon)\rangle 
-
\underbrace{
\sum_{j=1}^{N_c}\big|u_j\big\rangle 
\prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
}_{\text{core}}
+|k(\epsilon)\rangle
\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\biggr]
+(\hat{\tilde{v}}-\hat{v})|\tilde{\phi}_{PS}(\epsilon)\rangle 
\nonumber\\
%
%
%
&=&
-
\sum_{j=1}^{N_c}\Bigl(\hat{h}-\epsilon_j\Bigr)\big|u_j\big\rangle 
\prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
-
\sum_{j=1}^{N_c}\Bigl(\epsilon_j-\epsilon\Bigr)\big|u_j\big\rangle 
\prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
\nonumber\\
&&+\Bigl(\hat{h}-\epsilon\Bigr)|k(\epsilon)\rangle
\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
+(\hat{\tilde{v}}-\hat{v})|\tilde{\phi}_{PS}(\epsilon)\rangle 
\nonumber\\
%
...
&=&
\biggl[
\sum_{j=1}^{N_c}\big|u_{j-1}\big\rangle 
\prod_{k=1}^{j-1}(\epsilon_k-\epsilon)
-
\sum_{j=1}^{N_c}\big|u_j\big\rangle 
\prod_{k=1}^{j}(\epsilon_k-\epsilon)
+\Bigl(\hat{h}-\epsilon\Bigr)|k(\epsilon)\rangle
\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\biggr]
+(\hat{\tilde{v}}-\hat{v})|\tilde{\phi}_{PS}(\epsilon)\rangle 
...
&=&
\biggl[
\sum_{j=0}^{N_c-1}\big|u_{j}\big\rangle 
\prod_{k=1}^{j}(\epsilon_k-\epsilon)
-
\sum_{j=1}^{N_c}\big|u_j\big\rangle 
\prod_{k=1}^{j}(\epsilon_k-\epsilon)
...
\biggl[
-
\big|u_{N_c}\big\rangle 
+\Bigl(\hat{h}-\epsilon\Bigr)|k(\epsilon)\rangle
\biggr]\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
+(\hat{\tilde{v}}-\hat{v})|\tilde{\phi}_{PS}(\epsilon)\rangle 
\nonumber\\
%
%
...
-
\big|u_{N_c}\big\rangle 
+\Bigl(\hat{t}+\tilde{v}-\epsilon\Bigr)|k(\epsilon)\rangle
\biggr]\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
+(\hat{\tilde{v}}-\hat{v})
\biggl[\big|\tilde{\phi}_{PS}(\epsilon)\big\rangle 
-\big|k(\epsilon)\big\rangle
\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\biggr]
\nonumber\\
%
...
\biggr]\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
+(\hat{\tilde{v}}-\hat{v})
\big|q_{N_c+1}(\epsilon)\big\rangle
\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\nonumber\\
%
%
...
-
\big|u_{N_c}\big\rangle 
+\Bigl(\hat{t}+\tilde{v}-\epsilon\Bigr)\big|k(\epsilon)\big\rangle
+(\hat{\tilde{v}}-\hat{v})
\big|q_{N_c+1}(\epsilon)\big\rangle
\biggr]\prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\label{eq:energydependentprojectorfunctiona}
\end{eqnarray}
This equation establishes a link between $|k(\epsilon)\rangle$, which
determines the shape of the partial wave, with
$\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle$,
which determines the shape of the projector function.

\eq{eq:energydependentprojectorfunctiona} provides guidance on the
spatial extent of the projector functions. The spatial extent is thus
determined by
\begin{itemize}
\item the range of the deviation of the pseudo-potential from the true
  potential $\tilde{\hat{v}}-\hat{v}$,
\item the range of the difference $|k(\epsilon)\rangle$ of the
  pseudo-partial wave $|\phi^{PS}\rangle$ from node reduced partial
  wave $|q_{N_c+1}(\epsilon)\rangle$
\item the range of the node-less partial wave describing the highest
  core state.
\end{itemize}


Check \url{Codes/AtomicPAW} for an exploration code. (Apr.7, 2020).

%===========================================================
\subsubsection{Gaussian-shaped projector function}
%===========================================================
For example, if we want projector functions that have the shape of
Gauss function $|g_{\gamma}\rangle$ defined by
\begin{eqnarray} 
g_{\gamma}(\vec{r})&=&r^{\ell}\e{-\gamma^2 r^2} Y_{\ell,m}(\vec{r})
\end{eqnarray}
we can solve
\begin{eqnarray}
\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)|k(\epsilon)\rangle
&=&
\big|u_{N_c}\big\rangle 
-(\hat{\tilde{v}}-\hat{v})|q_{N_c+1}(\epsilon)\rangle
-|g_{\gamma}\rangle \lambda(\epsilon) 
\end{eqnarray}
where $\lambda(\epsilon)$ is adjusted so that $|k(\epsilon)\rangle$ is
localized inside the augmentation region for all energies.

\begin{eqnarray}
\Bigl(\hat{t}+\hat{\tilde{v}}-\epsilon\Bigr)|\tilde{\phi}_{PS}(\epsilon)\rangle
=-|g_{\gamma}\rangle \lambda(\epsilon) \prod_{k=1}^{N_c}(\epsilon_k-\epsilon)
\end{eqnarray}
...
|\tilde{p}(\epsilon)\rangle
&=&
|g_{\gamma}\rangle \frac{1}{\langle\tilde{\phi}_{PS}(\epsilon)|g_{\gamma}\rangle}
\end{eqnarray}
...
%% \label{eq:nodelesseq}
%% \end{eqnarray}
%% The energies $\epsilon_n$ are obtained from the natural boundary
%% conditions of the nodeless bound states $|u_n\rangle$. They are
%% identical to the eigenstates of $\hat{h}$ as will become evident
%% below.
...
%% integrating the inhomogeneous Schr\"odinger equation
%% \eq{eq:nodelesseq} outward, starting with vanishing value and
%% derivative, so that
%% \begin{eqnarray}
...
%% \begin{eqnarray}
%% |\phi_{ref,n}\rangle\defas|\phi_{ref}(\epsilon_n)\rangle
%% =\sum_{m=1}^n|u_m\rangle \prod_{j=1}^{m-1}(\epsilon_j-\epsilon_n)
...
%% \qquad\text{for $n>1$}
%% \end{eqnarray}
%% For each $n$ one obtains one energy-dependent, node-reduced partial wave
%% $|q_n(\epsilon)\rangle$.
...
%% %====================================================================
%% Besides the nodeless states $|u_n\rangle$ and the node-reduced partial
%% waves $|q_n(\epsilon)\rangle$, let me define a third set of partial
%% waves.

%% Let us define \cite{bloechl12_arxiv1210_5937} a new sequence of
%% nodeless functions $|\bar{q}_n^{(j)}\rangle$ for specified set of
%% energies $\{\epsilon_{n+j}; j\ge1\}$.  The energies in the set need
%% not to be eigenstates of the Hamiltonian. They may also all be chosen
%% identical.

%% The sequence of nodeless functions is
%% \begin{eqnarray}
...
%% \end{eqnarray}
%% where $|\bar{q}_{n}^{(0)}(\epsilon)\rangle=|q_{n}(\epsilon)\rangle$
%% defined in \eq{eq:defnodereducedpartialwave}. \eq{eq:qbardiffgl0} and
%% \eq{eq:qbardiffgl} have great similarity with those of the nodeless
%% equations \eq{eq:nodelesseq}. However, the inhomogenity of the
%% differential equation need not be a bound function, and the energies
%% need not be eigenstates of the Hamiltonian.
...
This behavior (independence of the energy from the pseudization radius)
