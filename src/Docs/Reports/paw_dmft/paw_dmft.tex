...
partial fluctuations due to quantum fluctuations\footnote{With quantum
...
$\mat{h}-\mat{\Gamma}$ acts as ``non-interacting'' Hamiltonian to
...
following thought experiment. Perform calculation with the
...
to the wave functions. As conclusion from our though experiment comes
...
%====================================================================
\subsection{Lattice Green's functions in a finite, non-orthonormal basisst}
%====================================================================
The Green's function results as inverse of a hamiltonian. The Green's
function we need should only act in a sub-Hilbert space of correlated
orbitals. Therefore we introduce a projection operator
\begin{eqnarray}
...
onto that hilbert space. Thus the following equation defines the greens function.
\begin{eqnarray}
...
Now we modify the Hamiltonian by changeing the spectrum for the higher
bands
\begin{eqnarray}
\hat{h}\rightarrow\sum_{n=1}^{N_x}|\psi_n\rangle \epsilon_n\langle\psi_n|
+\sum_{n=N_x+1}^\infty|\psi_n\rangle \epsilon_x\langle\psi_n|
\end{eqnarray}
This approximation allows one to limit the sum over states to a finite sum.
\begin{eqnarray}
\mat{G}&=&
...
%====================================================================
In the last step of the previous section we did the step
\begin{eqnarray}
\sum_{n=1}^{N_x}
\langle\chi|\psi_n\rangle\langle\psi_n|\chi\rangle
\approx\sum_{n=1}^{N_x}
\mat{S}\langle\pi|\psi_n\rangle\langle\psi_n|\pi\rangle\mat{S}
\end{eqnarray}
which is only approximative.

An allowed step is
\begin{eqnarray}
\langle\chi|\psi_n\rangle\langle\psi_n|\chi\rangle
&=&
\langle\chi|\chi\rangle\langle\pi|\hat{\mathcal{P}}_{\chi}|\psi_n\rangle
\langle\psi_n|\hat{\mathcal{P}}_{\chi}|\pi\rangle\langle\chi|\chi\rangle
=
\mat{S}\langle\pi|\hat{\mathcal{P}}_{\chi}|\psi_n\rangle
\langle\psi_n|\hat{\mathcal{P}}_{\chi}|\pi\rangle\mat{S}
\nonumber\\
&\neq&
\mat{S}\langle\pi|\psi_n\rangle
\langle\psi_n|\pi\rangle\mat{S}
\end{eqnarray}

\textcolor{red}{This is only a sketch...}

Consider the  set of  wave functions obtained  by projection  onto the
correlated orbitals.
\begin{eqnarray}
\hat{\mathcal{P}}_{\chi}|\psi_n\rangle
\end{eqnarray}

We obtain  a strict identity
\begin{eqnarray}
\hat{\mathcal{P}}_{\chi}|\psi_n\rangle
=\sum_\alpha|\chi_\alpha\rangle\langle\pi_\alpha|
\hat{\mathcal{P}}_{\chi}|\psi_n\rangle
\end{eqnarray}

However, the wave functions $\hat{\mathcal{P}}_{\chi}|\psi_n\rangle$ are
  no more orthonormal, and $\langle\pi_\alpha|
  \hat{\mathcal{P}}_{\chi}|\psi_n\rangle\neq \langle\pi_\alpha|
    \hat{\mathcal{P}}_{\chi}|\psi_n\rangle$.

%====================================================================
\subsubsection{Calculate $\bar{h}$ from the density matrix}
%====================================================================
The constraint is
\begin{eqnarray}
\sum_\nu\e{i\hbar\omega_\nu\beta0^+}G_{a,b}(i\omega_\nu)=\rho_{a,b} (=\sum_n\langle\pi_a|\psi_n\rangle f_n\langle\psi_n|\pi_b\rangle)
\end{eqnarray}
