...
\begin{eqnarray}
\hat{G}(i\omega_\nu)=\sum_{a,b}|\chi_a\rangle G_{a,b}(i\omega_\nu)\langle\chi_b|
\end{eqnarray}
with
\begin{eqnarray}
&&G_{a,b}(i\omega_\nu)
...
\end{eqnarray}
with\footnote{To motivate the form for $\mat{h}$, equate the denominator of both
  expressions for the Greens function, which gives
\begin{eqnarray}
...
\Bigl(\sum_{n}\langle\pi_a|\psi_n\rangle\epsilon_n
\langle\psi_n|\pi_b\rangle\Bigr)
\Bigl(\sum_{m'}\langle\pi_d|\psi_{m'}\rangle
...
\nonumber\\
\mat{\Delta}&=&\mat{\epsilon}-\mat{P}\mat{\epsilon}\mat{P}
\end{eqnarray}
...
\begin{eqnarray}
\sum_{n'}P_{n,n'}\langle\psi_{n'}|\pi_c\rangle
&=&\sum_{n'}\underbrace{\sum_{a,b}\langle\psi_n|\pi_a\rangle
\Bigl(\sum_{m}
\langle\pi_b|\psi_m\rangle\langle\psi_m|\pi_a\rangle \Bigr)^{-1}_{a,b}
...
&=&
\sum_{a,b}\langle\psi_n|\pi_a\rangle
\underbrace{\Bigl(\sum_{m}
\langle\pi_b|\psi_m\rangle\langle\psi_m|\pi_a\rangle \Bigr)^{-1}_{a,b}
\Bigl(\sum_{n'}\langle\pi_b|\psi_{n'}\rangle\langle\psi_{n'}|\pi_c\rangle\Bigr)
}_{\delta_{a,c}}
...
\epsilon_{n''}
\underbrace{\Bigl[\sum_{b,d}\langle\psi_{n''}|\pi_b\rangle
\Bigl(\sum_{m'}
\langle\pi_d|\psi_{m'}\rangle\langle\psi_{m'}|\pi_b\rangle \Bigr)^{-1}_{b,d}
\langle\pi_d|\psi_{n'}\rangle\Bigr]}_{P_{n'',n'}}
\nonumber\\
&=&\Bigl(\sum_{m}P_{n,m}\epsilon_{m}P_{m,n}\Bigr)
\nonumber
\end{eqnarray}
...
\nonumber\\
&=&\underbrace{\Bigl(\sum_{n''}P_{n,m}\epsilon_{n''}P_{n'',n'}\Bigr)}
...
\nonumber
\end{eqnarray}
which completes the proof of the identity between \eq{eq:greenmatrix1a}
and \eq{eq:greenmatrix2a}.

%==================================================
\subsubsection{Self energy from the uncorrelated orbitals}
%==================================================
The Green's function without the additional term $\mat{\Delta}$ can be
expressed with the help of \eq{eq:greenlocform1} from
p.~\pageref{eq:greenlocform1} in matrix form without reference to the
natural orbitals which we define as unperturbed (by the uncorrelated
orbitals) Green's function
\begin{eqnarray}
