...
\item \verb|DMFT_HRHO|: (see also section~\ref{sec:routinedmfthrho} on
  ...
  Hamiltonian $\bar{\mat{h}}$ that produces the specified density
  matrix.
\begin{eqnarray}
...
\item \verb|DMFT_SOLVER|: Calculates the dynamic contribution
  ...
  \begin{enumerate}
    ...
    \item then the actual interface routine \verb|DMFT_SOLVERIO| is
      ...
      \rho_{d,a}\rho_{c,b} - \rho_{c,a}\rho_{d,b}\Bigr) \Bigr) 
      \end{eqnarray}
      \petertt{pay attention to the reversed order of the indices $c,d$!}
    \item After \verb|DMFT_SOLVERIO| the self energy and the
      derivative with respect to the U-tensor are transformed back.
  \end{enumerate}
...
\Bigl(
\mat{\mathcal{S}}^{(1)}_{dyn}(\vec{k})-\mat{\Gamma}(\vec{k})
\Bigr)
\mat{S}^{-1}(\vec{k})
%
\nonumber\\
