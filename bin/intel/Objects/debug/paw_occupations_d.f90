...
!**      'N' FILLS STATES ACCORDING TO BAND NUMBER AND DYNOCC__MODOCC  **
!**                                                                   **
!**    THE DEFAULT OCCUPATIONS CORRESPOND TO STARTTYPE='N'.           **
!**    IT FILLS THE BANDS CONSISTENT WITH THE TOTAL CHARGE AND SPIN   **
!**    ASSUMING FLAT BANDS. THEN THEY ARE MODIFIED BY DYNOCC__MODOCC   **
!**    (DYNOCC__MODOCC ALSO ADJUSTS THE TOTAL CHARGE AND SPIN!)        **
...
LOGICAL(4)  :: RESET=.TRUE.    ! SETS OUTPUT MODE OF DYNOCC__REPORT
...
!      =================================================================
       ...
       IF(TTEST) THEN
         ...
         IF(SVAR.GT.1.D-8) THEN
           CALL ERROR__STOP('CUBPOLYNOMROOT1')
         ENDIF
         SVAR=ABS(A0_+X2*(A1_+X2*(A2_+X2*A3_)))
         IF(SVAR.GT.1.D-8) THEN
           CALL ERROR__STOP('CUBPOLYNOMROOT2')
         ENDIF
         SVAR=ABS(A0_+X3*(A1_+X3*(A2_+X3*A3_)))
         IF(SVAR.GT.1.D-8) THEN
           CALL ERROR__STOP('CUBPOLYNOMROOT3')
         ENDIF
       END IF
       RETURN
...
!      *****************************************************************
       ...
       CALL ERROR__STOP('CUBNEWTONRAPHSON(DYNOCC_MODULE)')
       RETURN
...
!      *****************************************************************
       IF(F.LT.0.D0.OR.F.GT.1.D0) THEN
         CALL ERROR__MSG('OCCUPATIONS MUST LIE BETWEEN ZERO AND ONE')
         CALL ERROR__STOP('DYNOCC_XOFF')
       END IF
       X=0.5D0
       DO ITER=1,100
...
!      ******************************************************************
       ...
       DO ISPIN=1,NSPIN
         DO IKPT=1,NKPT
           DO IB=1,NB
             CALL LIB__RANDOM(SVAR)
             EPSILON(IB,IKPT,ISPIN)=SVAR
           ENDDO
         ENDDO
       ENDDO
       CALL DYNOCC__SETR8A('XK',3*NKPT,XK)
       CALL DYNOCC__SETR8A('WKPT',NKPT,WKPT)
       CALL DYNOCC__SETR8('SUMOFZ',SUMOFZ)
       CALL DYNOCC__SETR8('MASS',MASS)
       CALL DYNOCC__SETR8('TIMESTEP',DELTAT)
       CALL DYNOCC__SETR8('TEMP',1.D0)
       CALL DYNOCC__SETL4('FIXQ',.TRUE.)
       CALL DYNOCC__SETL4('FIXS',.TRUE.)
...
!REMARK WHY IS XK NEEDED?
       ...
       CALL DYNOCC__SETR8('FRICTION',FRIC)
       WRITE(NFILO,FMT='(A10,4A15)')'ITER','ECONS','EKIN','EPOT','EBAND'
       DO ITER=1,NITER
         CALL DYNOCC__SETR8A('EPSILON',NB*NKPT*NSPIN,EPSILON)
         CALL DYNOCC__PROPAGATE
         CALL DYNOCC__GETR8('EKIN',EKIN)
         CALL DYNOCC__GETR8('EPOT',EPOT)
         CALL DYNOCC__GETR8A('OCC',NB*NKPT*NSPIN,WGHT)
         EBAND=0.D0
         DO ISPIN=1,NSPIN
           DO IKPT=1,NKPT
             SVAR=0.D0
...
!      .................................................................
       SUBROUTINE DYNOCC__CREATE(NB_,NKPT_,NSPIN_)
!      *****************************************************************
!      *****************************************************************
       USE DYNOCC_MODULE
       IMPLICIT NONE
...
!      *****************************************************************
       IF(.NOT.ALLOCATED(XK)) THEN
         CALL ERROR__MSG('K-POINTS DO NOT EXIST')
         CALL ERROR__STOP('DYNOCC__CREATE')
       END IF
       IF(SUMOFZ.LT.0.D0) THEN
         CALL ERROR__MSG('SUMOFZ NOT DEFINED')
         CALL ERROR__STOP('DYNOCC__CREATE')
       END IF
       IF(.NOT.ALLOCATED(WKPT)) THEN
         CALL ERROR__MSG('K-POINT WEIGHTS DO NOT EXIST')
         CALL ERROR__STOP('DYNOCC__CREATE')
       END IF
       NB=NB_
       IF(NKPT.NE.NKPT_.AND.NKPT.NE.0) THEN
         CALL ERROR__MSG('NKPT MUST NOT BE CHANGED')
         CALL ERROR__STOP('DYNOCC__CREATE')
       END IF
       NKPT=NKPT_
       NSPIN=NSPIN_
       ...
       END
!      .................................................................
       SUBROUTINE DYNOCC__SETL4(ID_,VAL_)
!      *****************************************************************
...
!      *****************************************************************
       ...
       ELSE
         ...
         CALL ERROR__STOP('DYNOCC__SETL4')
       END IF
       RESET=.TRUE.
       RETURN
       END
!      .................................................................
       SUBROUTINE DYNOCC__SETI4A(ID_,LEN_,VAL_)
!      *****************************************************************
!      **  SET LOGICAL PARAMETERS                                     **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       ...
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
         CALL ERROR__CHVAL('ID_',ID_)
         CALL ERROR__STOP('DYNOCC__SETI4A')
       END IF
       RESET=.TRUE.
       RETURN
       END
!      .................................................................
       SUBROUTINE DYNOCC__SETI4(ID_,VAL_)
!      *****************************************************************
!      **  SET LOGICAL PARAMETERS                                     **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       IF(ID_.EQ.'NKPT') THEN
         IF(NKPT.NE.0) THEN
           CALL ERROR__MSG('NKPT IS ALREADY SET')
           CALL ERROR__STOP('DYNOCC__SETI4')
         END IF
         NKPT=VAL_
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
...
!
!      .................................................................
       SUBROUTINE DYNOCC__SETR8A(ID_,LEN_,DATA_)
!      *****************************************************************
!      **  SET REAL(8) ARRAY                                          **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       IF(ID_.EQ.'XK') THEN
         ...
         IF(LEN_.NE.3*NKPT) THEN
           ...
           CALL ERROR__STOP('DYNOCC__SETR8A')
         END IF
...
!pb031019start
         IF(abs(SUM(WKPT)-1.d0).gt.1.d-6) THEN
           CALL ERROR__MSG('OCCUPATIONS DO NOT SUM UP TO ONE!')
           CALL ERROR__STOP('DYNOCC__SETR8A')
         END IF
!pb031019end
!      ===================================================================
       ELSE IF(ID_.EQ.'EPSILON') THEN
         IF(LEN_.NE.NB*NKPT*NSPIN) THEN
           CALL ERROR__MSG('DIMENSIONS INCONSISTENT')
           CALL ERROR__CHVAL('ID_',ID_)
           CALL ERROR__I4VAL('LEN_',LEN_)
           CALL ERROR__I4VAL('NB*NKPT*NSPIN',NB*NKPT*NSPIN)
           CALL ERROR__STOP('DYNOCC__SETR8A')
         END IF
...
!
!      .................................................................
       SUBROUTINE DYNOCC__SETch(ID_,DATA_)
!      *****************************************************************
!      **  SET character                                              **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       IF(ID_.EQ.'STARTTYPE') THEN
         ...
         IF(STARTTYPE.NE.'N'.AND.STARTTYPE.NE.'E'.AND.STARTTYPE.NE.'X') THEN
           CALL ERROR__MSG('ILLEGAL VALUE OF STARTTYPE')
           CALL ERROR__MSG('ALLOWED VALUES ARE "N", "E", AND "X"')
         END IF
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
         CALL ERROR__CHVAL('ID_',ID_)
         CALL ERROR__STOP('DYNOCC__SETch')
       END IF
       RETURN
       END
!
!      .................................................................
       SUBROUTINE DYNOCC__SETR8(ID_,DATA_)
!      *****************************************************************
!      *****************************************************************
       USE DYNOCC_MODULE
       IMPLICIT NONE
...
!      *****************************************************************
       ...
       ELSE IF(ID_.EQ.'TOTCHA') THEN
         IF(SUMOFZ.LT.0.D0) THEN
           CALL ERROR__MSG('SUMOFZ MUST BE SET BEFORE TOTCHA')
           CALL ERROR__STOP('DYNOCC__SETR8')
         END IF
         charge=data_
         TOTCHA=SUMOFZ+CHARGE
         RESET=.TRUE.
       ELSE IF(ID_.EQ.'SPIN') THEN
         SPINCHA=DATA_
         RESET=.TRUE.
         PRINT*,'DYNOCC__SETR8: SPIN HAS BEEN SET TO ',SPINCHA
       ...
       END
!      .................................................................
       SUBROUTINE DYNOCC__GETL4(ID_,VAL_)
!      *****************************************************************
!      **  SET LOGICAL PARAMETERS                                     **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       ...
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
         CALL ERROR__CHVAL('ID_',ID_)
         CALL ERROR__STOP('DYNOCC__GETL4')
       END IF
       RETURN
       END
!      .................................................................
       SUBROUTINE DYNOCC__GETI4(ID_,VAL_)
!      *****************************************************************
...
!      *****************************************************************
       ...
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
         CALL ERROR__CHVAL('ID_',ID_)
         CALL ERROR__STOP('DYNOCC__GETI4')
       END IF
       RETURN
       END
!      .................................................................
       SUBROUTINE DYNOCC__GETI4A(ID_,LEN_,VAL_)
!      *****************************************************************
!      **  SET INTEGER PARAMETERS                                     **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       ...
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
         CALL ERROR__CHVAL('ID_',ID_)
         CALL ERROR__STOP('DYNOCC__GETI4A')
       END IF
       RETURN
       END
!
!      .................................................................
       SUBROUTINE DYNOCC__GETR8(ID_,DATA_)
!      *****************************************************************
!      *****************************************************************
       USE DYNOCC_MODULE
       IMPLICIT NONE
...
!        == TOTPOT-DBLE(3-2*ISPIN)*SPINPOT
       ...
       ELSE
         CALL ERROR__MSG('ID NOT RECOGNIZED')
         CALL ERROR__CHVAL('ID_',ID_)
         CALL ERROR__STOP('DYNOCC__GETR8')
       END IF
       RETURN
       END
!
!      .................................................................
       SUBROUTINE DYNOCC__GETR8A(ID,LEN,VAL)
!      *****************************************************************
!      *****************************************************************
       USE DYNOCC_MODULE
       IMPLICIT NONE
...
!      =================================================================
       IF(ID.EQ.'XK') THEN
         IF(LEN.NE.3*NKPT) THEN
           ...
           CALL ERROR__I4VAL('NKPT',NKPT)
           CALL ERROR__STOP('DYNOCC__GETR8A')
         END IF
...
!
!     ..................................................................
      SUBROUTINE DYNOCC__WRITE(NFIL,NFILO,TCHK)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE DYNOCC_MODULE
      ...
      logical(4)                   :: tnew=.true.
!     ******************************************************************
                          CALL TRACE__PUSH('OCCUPATIONS__WRITE')
...
!
!     ..................................................................
      SUBROUTINE DYNOCC__READ(NFIL,NFILO,TCHK)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE DYNOCC_MODULE
      ...
      logical(4)                     :: told
!     ******************************************************************
                          CALL TRACE__PUSH('DYNOCC__READ')
      IF(STARTTYPE.EQ.' ') THEN
        CALL ERROR__MSG('STARTTYPE NOT SPECIFIED')
        CALL ERROR__STOP('DYNOCC__READ')
      END IF
...
!     ==  BROADCAST                                                   ==
!     ==================================================================
      CALL MPE__BROADCAST(1,X0)
      CALL MPE__BROADCAST(1,XM)
      CALL MPE__BROADCAST(1,epsilon)
!
!     ==================================================================
...
!
!      .................................................................
       SUBROUTINE DYNOCC__REPORT(NFIL)
!      *****************************************************************
!      **                                                             **
...
!      == 1EL<->0.5HBAR => HBAR=2 EL
       IF(.NOT.RESET.AND.TDYN) THEN
         CALL REPORT__TITLE(NFIL,'OCCUPATIONS')
         CALL REPORT__R8VAL(NFIL,'TEMPERATURE',TEMP/KELVIN,'K')
         CALL REPORT__R8VAL(NFIL,'CHEMICAL POTENTIAL',TOTPOT/EV,'EV')
         CALL REPORT__R8VAL(NFIL,'CHARGE',-(TOTCHA-SUMOFZ),'E')
!
         IF(NSPIN.EQ.2) THEN
           CALL REPORT__R8VAL(NFIL,'MAGNETIC FIELD',SPINPOT/(EV),'EV/(HBAR/2)')
           CALL REPORT__R8VAL(NFIL,'SPIN',SPINCHA/2.D0,'HBAR')
         END IF
         CALL DYNOCC__GETR8('HEAT',SVAR)
         CALL REPORT__R8VAL(NFIL,'HEAT',SVAR,'H')
         IF(TEPSILON) THEN
           CALL DYNOCC__GETR8A('OCC',NB*NKPT*NSPIN,OCC)
...
!      =================================================================
       IF(RESET.AND.(.NOT.TDYN)) THEN
         CALL REPORT__TITLE(NFIL,'OCCUPATIONS')
         CALL REPORT__CHVAL(NFIL,'OCCUPATIONS ARE','FIXED')
         CALL REPORT__R8VAL(NFIL,'#(ELECTRONS)',TOTCHA,' ')
         CALL REPORT__R8VAL(NFIL,'CHARGE',-(TOTCHA-SUMOFZ),'E')
         IF(NSPIN.EQ.2) THEN
           CALL REPORT__R8VAL(NFIL,'SPIN',SPINCHA/2.D0,'HBAR')
         END IF
...
!      =================================================================
       IF(RESET.AND.TDYN) THEN
         CALL REPORT__TITLE(NFIL,'OCCUPATIONS')
         CALL REPORT__STRING(NFIL,'DYNAMICAL OCCUPATIONS USING MERMIN FUNCTIONAL')
         CALL REPORT__R8VAL(NFIL,'TEMPERATURE',TEMP/KELVIN,'K')
         CALL REPORT__R8VAL(NFIL,'MASS OF X',MX,'A.U.')
         IF(ANNEX.NE.0.D0) THEN
           CALL REPORT__R8VAL(NFIL,'FRICTION',ANNEX,' ')
         END IF
         IF(TSTOP) THEN
           CALL REPORT__CHVAL(NFIL,'INITIAL VELOCITIES ARE SET TO ','ZERO')
         END IF
         IF(TFIXTOT) THEN
           CALL REPORT__R8VAL(NFIL,'FIXED CHARGE',-(TOTCHA-SUMOFZ),'E')
           CALL REPORT__R8VAL(NFIL,'#(ELECTRONS)',TOTCHA,' ')
           IF(TEPSILON) THEN
             CALL REPORT__R8VAL(NFIL,'CHEMICAL POTENTIAL',TOTPOT/EV,'EV')
           END IF
         ELSE
           CALL REPORT__R8VAL(NFIL,'SUM OF NUCLEAR CHARGES',SUMOFZ,'E')
           CALL REPORT__R8VAL(NFIL,'FIXED CHEMICAL POTENTIAL',TOTPOT/EV,'EV')
           IF(TEPSILON) THEN
             CALL REPORT__R8VAL(NFIL,'CHARGE',-(TOTCHA-SUMOFZ),'E')
           END IF
         END IF
!
         IF(NSPIN.EQ.2) THEN
           IF(TFIXSPIN) THEN
             CALL REPORT__R8VAL(NFIL,'FIXED SPIN',SPINCHA*0.5D0,'HBAR')
             IF(TEPSILON) THEN
               CALL REPORT__R8VAL(NFIL,'MAGNETIC FIELD',SPINPOT/EV,'EV/(HBAR/2)')
             END IF
           ELSE
             CALL REPORT__R8VAL(NFIL,'FIXED MAGNETIC FIELD',SPINPOT/EV,'EV/(HBAR/2)')
             IF(TEPSILON) THEN
               CALL REPORT__R8VAL(NFIL,'SPIN',SPINCHA/2.D0,'HBAR')
             END IF
           END IF
       ...
       IF((.NOT.RESET).AND.(.NOT.TDYN)) THEN
         IF(TEPSILON) THEN
!          CALL DYNOCC__GETR8A('EPSILON',NB*NKPT*NSPIN,EPSILON)
...
!      *****************************************************************
       ...
       END
!      .................................................................
       SUBROUTINE DYNOCC__ORDER(IKPT,ISPIN)
!      *****************************************************************
!      **                                                             **
...
!      *****************************************************************
       IF(IKPT.LT.1.OR.IKPT.GT.NKPT) THEN
         CALL ERROR__MSG('IKPT OUT OF RANGE')
         CALL ERROR__I4VAL('IKPT',IKPT)
         CALL ERROR__STOP('DYNOCC__ORDER')
       END IF
       IF(ISPIN.LT.1.OR.ISPIN.GT.NSPIN) THEN
         CALL ERROR__MSG('SPIN OUT OF RANGE')
         CALL ERROR__I4VAL('ISPIN',ISPIN)
         CALL ERROR__STOP('DYNOCC__ORDER')
       END IF
!      == REORDER INPUT ================================================
       IF(TEPSILON) THEN
         CALL SORT__ORDERR8(1,NB,EPSILON(:,IKPT,ISPIN))
       END IF
       IF(ALLOCATED(MPSIDOT2)) THEN
         CALL SORT__ORDERR8(1,NB,MPSIDOT2(:,IKPT,ISPIN))
       END IF
!      == OCCUPATIONS ARE ONLY REORDERED IF OCCUPATIONS ARE DYNAMICAL
       IF(TDYN) THEN
         CALL SORT__ORDERR8(1,NB,XM(:,IKPT,ISPIN))
         CALL SORT__ORDERR8(1,NB,X0(:,IKPT,ISPIN))
         CALL SORT__ORDERR8(1,NB,XP(:,IKPT,ISPIN))
       END IF
       RETURN
       END
!
!      .................................................................
       SUBROUTINE DYNOCC__PROPAGATE()
!      *****************************************************************
!      **                                                             **
!      *****************************************************************
       USE DYNOCC_MODULE
...
!      *****************************************************************
       IF(.NOT.TDYN) then
         ...
         RETURN
                              ...
                              CALL TRACE__PUSH('DYNOCC__PROPAGATE')
       IF(NSPIN.EQ.1) THEN
         TFIXSPIN=.FALSE.
         SPINPOT=0.D0
       END IF
       IF(.NOT.TEPSILON) THEN
         CALL ERROR__MSG('ONE-PARTICLE ENERGIES HAVE NOT BEEN SET')
         CALL ERROR__STOP('DYNOCC__PROPAGATE')
       END IF
...
PRINT*,'DYNOCC ',TFIXTOT,TFIXSPIN,Q0,S0,TOTCHA,SPINCHA,TOTPOT,SPINPOT
       ...
       IF(.NOT.TCONV) THEN
         CALL ERROR__MSG('LOOP NOT CONVERGED')
         CALL ERROR__R8VAL('DEVIATION TOTAL CHARGE',Q0)
         CALL ERROR__R8VAL('DEVIATION TOTAL SPIN',S0)
         CALL ERROR__STOP('DYNOCC__PROPAGATE')
       END IF
...
!      *****************************************************************
       ...
       IMPLICIT NONE
!      *****************************************************************
                                  CALL TRACE__PUSH('DYNOCC__INIOCC')
...
!
!      .................................................................
       SUBROUTINE DYNOCC__MODOCC(IB_,IKPT_,ISPIN_,OCC_)
!      *****************************************************************
...
!      *****************************************************************
       IF(IB_.GT.NB.OR.IKPT_.GT.NKPT.OR.ISPIN_.GT.NSPIN) THEN
         CALL ERROR__MSG('DIMENSIONS EXCEEDED')
         CALL ERROR__I4VAL('IB_',IB_)
         CALL ERROR__I4VAL('IKPT_',IKPT_)
         CALL ERROR__I4VAL('ISPIN_',ISPIN_)
         CALL ERROR__I4VAL('NB',NB)
         CALL ERROR__I4VAL('NKPT',NKPT)
         CALL ERROR__I4VAL('NSPIN',NSPIN)
         CALL ERROR__STOP('DYNOCC__MODOCC')
       END IF
!
!      =================================================================
...
!      =================================================================
     ...
     &           ,WKPT,work(:,:),X0(:,:,ISPIN),TOTPOT,EMERMN)
           ...
           END IF
         ELSE
           CALL ERROR__MSG('NSPIN MUST BE EITHER 1  OR 2')
           CALL ERROR__STOP('DYNOCC__INIOCC')
         END IF
...
!     ******************************************************************
                           CALL TRACE__PUSH('MERMIN')
!
      IF(ISPINDEG.NE.1.AND.ISPINDEG.NE.2.D0) THEN
        CALL ERROR__MSG('SPIN-DEGENERACY CAN BE EITHER ONE OR TWO')
        CALL ERROR__STOP('DYNOCC_MERMIN')
      END IF
...
!     ==================================================================
      ...
      DO ITER=1,ITERX
        ...
        CHMPOT=X0
      ENDDO
      CALL ERROR__MSG('OCCUPATIONS NOT CONVERGED')
      CALL ERROR__STOP('MERMIN')
 110  CONTINUE
                           CALL TRACE__PASS('B')
!
!     ==================================================================
...
!
!     ..................................................................
      SUBROUTINE OCCUPATION__SET(STRING_,NBYTE_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
...
!     ******************************************************************
      CALL TRACE__PUSH('OCCUPATION__SET')
      CALL OCCUPATION_NEWLIST
      CALL LINKEDLIST__SET(LL_OCC,STRING_,0,VAL_)
      CALL TRACE__POP
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE OCCUPATION__GET(STRING_,NBYTE_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE OCCUPATION_MODULE
      ...
      LOGICAL(4)              :: TCHK
!     ******************************************************************
      CALL TRACE__PUSH('OCCUPATION__GET')
      CALL OCCUPATION_NEWLIST
      CALL LINKEDLIST__EXISTD(LL_OCC,STRING_,1,TCHK)
      IF(.NOT.TCHK) THEN
        CALL ERROR__MSG('ITEM HAS NOT BEEN STORED')
        CALL ERROR__CHVAL('STRING',STRING_)
        CALL ERROR__STOP('OCCUPATION__GET')
      END IF
      CALL LINKEDLIST__GET(LL_OCC,STRING_,1,VAL_)
      CALL TRACE__POP
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE OCCUPATION__REPORT(NFIL,STRING_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
...
!     ******************************************************************
     ...
     &             ,(DWORK(I),I=1,3)
        ENDDO
      ELSE
        CALL ERROR__MSG('STRING_ NOT RECOGNIZED')
        CALL ERROR__CHVAL('STRING_',STRING_)
        CALL ERROR__STOP('OCCUPATION__REPORT')
      END IF
      RETURN
      END
