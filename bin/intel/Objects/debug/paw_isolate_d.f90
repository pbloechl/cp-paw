...
!
!     ..................................................................
      SUBROUTINE ISOLATE__ONOFF(STRING,NFCT_,RC0_,RCFAC_,G2MAX_,DECOUPLE_)
!     ******************************************************************
...
!       __ TISOLATE DECIDES WHETHER ONLY CHARGES SHALL BE CALCULATED____
      ELSE IF(STRING.EQ.'OFF') THEN
        TON=.FALSE.
      ELSE
        CALL ERROR__MSG('STRING INCORRECT IN ISOLATE__ONOFF')
        CALL ERROR__CHVAL('STRING',STRING)
        CALL ERROR__STOP('ISOLATE__ONOFF')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ISOLATE__REPORT(NFIL)
!     ******************************************************************
...
!     ==================================================================
      ALLOCATE(G2(NG))
      CALL PLANEWAVE__GETR8A('G2',NG,G2)
...
!     ==================================================================
      ...
      ALLOCATE(EIGR(NG))
      DO IAT=1,NAT
        CALL PLANEWAVE__STRUCTUREFACTOR(POS(1,IAT),NG,EIGR)
        DO IFCT=1,NFCT
...
!     ==================================================================
      ALLOCATE(GVEC(3,NG))
      CALL PLANEWAVE__GETR8A('GVEC',3*NG,GVEC)
...
!     ==================================================================
      ...
      ELSE
        ...
        ENDdo
      END IF
      CALL MPE__COMBINE('+',GGAMMA)
      CALL MPE__COMBINE('+',RHOGAMMA)
!
!     ==================================================================
!     ==      GRHO(I)=      <F(I)|RHO>      GG(I,J)=      <F(I)|F(J)> ==
!     ==  GRADGRHO(I)=<NABLA*F(I)|RHO>  GRADGG(I,J)=<NABLA*F(I)|F(J)> ==
!     ==================================================================
                              CALL TIMING__CLOCKON('ISOLATE_A_DOT')
      DO I=1,NAT*NFCT
...
!         ____GG(I,J)=<F(I)|w|F(J)>_____________________________________
        ...
        ENDDO
      ENDDO
      CALL MPE__COMBINE('+',GRHO)
      CALL MPE__COMBINE('+',GRADGRHO)
      CALL MPE__COMBINE('+',GG)
      CALL MPE__COMBINE('+',GRADGG)
                              CALL TIMING__CLOCKOFF('ISOLATE_A_DOT')
!
!     ==================================================================
...
!     ____GGINV =[<G|G>]**(-1)__________________________________________
      call lib__invertr8(nat*nfct,gg,gginv)
!
...
!     ==================================================================
      DO IAT=1,NAT
        ...
        ENDDO
      ENDDO
      CALL ATOMLIST__SETR8A('Q',0,NAT,QMAD)
!
!     ==================================================================
!     ==  SET CHARGES IN ATOMLIST AND GET EXTERNAL POTENTIALS        ==
!     ==================================================================
...
!     ==================================================================
      CALL QMMM__INTERFACE(NAT,POS,CHARGE,FORCE1,POT1,ENERGY1)
      ENERGY=ENERGY+ENERGY1
      DO IAT=1,NAT
        FORCE(:,IAT)=FORCE(:,IAT)+FORCE1(:,IAT)
        DO IG=1,NG
...
!     ==  COUPLE CONTINUUM                                            ==
!     ==================================================================
      CALL CONTINUUM__PROPAGATE(NAT,POS,NG,RC,QI,ENERGY1,VI1,FORCE1)
      ENERGY=ENERGY+ENERGY1
      VI(:,:)=VI(:,:)+VI1(:,:)
      FORCE(:,:)=FORCE(:,:)+FORCE1(:,:)
...
!     CALL CONTINUUM__PROPAGATE(NAT,POS,CHARGE,ENERGY1,POT1,FORCE1)
...
!     ==================================================================
                              CALL TIMING__CLOCKON('ISOLATE_MADELUNG')
      CALL MADELUNG(NAT,RBAS,POS,CHARGE,ENERGY2,POT2,FORCE2)
                              CALL TIMING__CLOCKOFF('ISOLATE_MADELUNG')
!
!     ==================================================================
...
!     ******************************************************************
      ...
      DO IG=1,NG
        IF(G2(IG).LT.G2MAX) THEN
          IGS=IGS+1
          IF(IGS.GT.NGS) THEN
            CALL ERROR__STOP('ISOLATE_MAPLOWER')
          ENDIF
          MAP(IGS)=IG
        END IF
      ENDDO
      IF(IGS.NE.NGS) THEN
        CALL ERROR__MSG('INCONSISTENT NUMBER OF SHORT VECTORS')
        CALL ERROR__STOP('ISOLATE_MAPLOWER')
      END IF
      RETURN
      END
