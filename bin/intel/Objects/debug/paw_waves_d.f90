...
!**                                                                   **
!**  FUNCTIONS:                                                       **
!**    WAVES__SETR8A(ID,LEN,VAL)                                       **
!**    WAVES__GETR8A(ID,LEN,VAL)                                       **
!**    WAVES__INITIALIZE                                               **
!**    WAVES__ETOT                                                     **
!**    WAVES__PROPAGATE                                                **
!**    WAVES__ORTHOGONALIZE                                            **
!**    WAVES__SWITCH                                                   **
!**    WAVES__REPORT(NFIL)                                             **
!**                                                                   **
!**  DEPENDECIES:                                                     **
...
LOGICAL(4)  :: TRAWSTATES=.FALSE. ! PROVIDES NON-DIAGONALIZED WAVE FUNCTIONS THROUGH __GETR
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THISARRAY)) THEN
        CALL ERROR__MSG('THISARRAY DOES NOT EXIST')
        CALL ERROR__STOP('WAVES_SELECTWV')
      END IF
...
!
!     ..................................................................
      SUBROUTINE WAVES__SETR8(ID,VAL)
!     ******************************************************************
!     **  WAVES__GVECTORS                                              **
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('WAVES__SETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__GETR8(ID,VAL)
!     ******************************************************************
!     **  WAVES__GVECTORS                                              **
!     **  GENERATE G-VECTORS AND OTHER INITIALIZATION                 **
!     ******************************************************************
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('WAVES__GETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__GETR8A(ID,LEN,VAL)
!     ******************************************************************
!     **  WAVES__GVECTORS                                              **
!     **  GENERATE G-VECTORS AND OTHER INITIALIZATION                 **
!     ******************************************************************
      ...
      LOGICAL(4)              :: TINV
!     ******************************************************************
                              CALL TRACE__PUSH('WAVES__GETR8A')
...
!     ================================================================
      ELSE IF(ID.EQ.'EIGVAL') THEN
        ...
        CALL PLANEWAVE__SELECT(GSET%ID)
        IF(.NOT.ASSOCIATED(THIS%EIGVAL)) THEN
          CALL ERROR__MSG('EIGENVALUES NOT AVAILABLE')
          CALL ERROR__L4VAL('THAMILTON',THAMILTON)
          CALL ERROR__STOP('WAVES__GETR8A')
        END IF
        IF(LEN.NE.THIS%NB) THEN
          CALL ERROR__MSG('INCONSISTENT LENGTH')
          CALL ERROR__I4VAL('NB',THIS%NB)
          CALL ERROR__I4VAL('LEN',LEN)
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('WAVES__GETR8A')
        END IF
        VAL(:)=THIS%Eigval(:)
...
!     ================================================================
      ELSE IF(ID.EQ.'<PSI|H|PSI>') THEN
        ...
        IF(.NOT.ASSOCIATED(THIS%EXPECTVAL)) THEN
          CALL ERROR__MSG('ENERGY EXPECTATION VALUES NOT AVAILABLE')
          CALL ERROR__L4VAL('THAMILTON',THAMILTON)
          CALL ERROR__STOP('WAVES__GETR8A')
        END IF
        IF(LEN.NE.THIS%NB) THEN
...
!     ================================================================
      ELSE IF(ID.EQ.'PSPSI') THEN
        ...
        IF(.NOT.ASSOCIATED(THIS%EIGVEC)) THEN
          CALL ERROR__MSG('EIGENSTATES NOT AVAILABLE')
          CALL ERROR__L4VAL('THAMILTON',THAMILTON)
          CALL ERROR__STOP('WAVES__GETR8A')
        END IF
        NGL=GSET%NGL
        ...
        IF(TINV) THEN
          ...
          ENDDO
          ALLOCATE(CWORK3(NGL))
          CALL PLANEWAVE__INVERTG(NGL,CWORK2,CWORK3)
          DO IG=1,NGL
        ...
        END IF
        ALLOCATE(CWORK2(NRL))
        CALL PLANEWAVE__FFT('GTOR',1,NGL,CWORK1,NRL,CWORK2)
        deALLOCATE(CWORK1)
        IF(EXTPNTR%TIM) THEN
          DO IR=1,NRL
...
!
!     ..................................................................
      SUBROUTINE WAVES__SETI4(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE WAVES_MODULE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('WAVES__SETI4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__GETI4(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE WAVES_MODULE
...
!     ******************************************************************
      ...
      ELSE IF(ID.EQ.'NR1') THEN
        ...
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETI4('NR1',VAL)
      ELSE IF(ID.EQ.'NR1L') THEN
        CALL WAVES_SELECTWV(1,1)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETI4('NR1L',VAL)
      ELSE IF(ID.EQ.'NR2') THEN
        CALL WAVES_SELECTWV(1,1)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETI4('NR2',VAL)
      ELSE IF(ID.EQ.'NR3') THEN
        CALL WAVES_SELECTWV(1,1)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETI4('NR3',VAL)
      ELSE IF(ID.EQ.'NR1START') THEN
        CALL WAVES_SELECTWV(1,1)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETI4('NR1START',VAL)
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('WAVES__GETI4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__SETL4(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE WAVES_MODULE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('WAVES__SETL4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__GETL4(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE WAVES_MODULE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('WAVES__getL4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__SETCH(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE WAVES_MODULE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('WAVES__SETCH')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__INITIALIZE
!     ******************************************************************
!     **  GENERATE G-VECTORS AND OTHER INITIALIZATION                 **
!     ******************************************************************
      USE WAVES_MODULE
      ...
      INTEGER(4)             :: NFILO
!     ******************************************************************
                             CALL TRACE__PUSH('WAVES__GVECTORS')
!
!     ================================================================
!     ==  DETERMINE MAPPRO AND MAPBAREPRO                           ==
!     ================================================================
      CALL ATOMLIST__NATOM(MAP%NAT)
      CALL SETUP__NSPECIES(MAP%NSP)
      CALL SETUP__LNXX(MAP%LNXX)
      ALLOCATE(MAP%ISP(MAP%NAT))
      ALLOCATE(MAP%LNX(MAP%NSP))
      ALLOCATE(MAP%LMNX(MAP%NSP))
      ALLOCATE(MAP%LOX(MAP%LNXX,MAP%NSP))
      CALL ATOMLIST__GETI4A('ISPECIES',0,MAP%NAT,MAP%ISP)
      MAP%LMX=0
      MAP%LOX(:,:)=0
      MAP%NBAREPRO=0
      DO ISP=1,MAP%NSP
        CALL SETUP__LNX(ISP,MAP%LNX(ISP))
        CALL SETUP__LOFLN(ISP,MAP%LNX(ISP),MAP%LOX(1,ISP))
        MAP%LMNX(ISP)=0
        DO LN=1,MAP%LNX(ISP)
...
!     ==  ALLOCATE THISARRAY                                          ==
!     ==================================================================
      CALL DYNOCC__GETI4('NKPT',NKPT)
      CALL DYNOCC__GETI4('NSPIN',NSPIN)
...
!     ==================================================================
      ...
      CALL PLANEWAVE__DIVIDERGRIDONTASKS(EPWRHO,RBAS,NR1START,NR1L,NR2,NR3)
      CALL POTENTIAL__INITIALIZE(EPWRHO,NR1START,NR1L,NR2,NR3)
...
!     ==================================================================
      ALLOCATE(XK(3,NKPT))
      CALL DYNOCC__GETR8A('XK',3*NKPT,XK)
      ...
      DO IKPT=1,NKPT
        ...
        IF(NDIM.EQ.1) THEN
          CALL PLANEWAVE__CHECKINVERSION(XK(1,IKPT),TINV)
        ELSE
          TINV=.FALSE.
        END IF
!print*,'tinv forced to be false in waves__initialize!!!'
!tinv=.false.
        CALL PLANEWAVE__INITIALIZE(GSET%ID,RBAS,XK(1,IKPT),TINV,EPWPSI &
     &                           ,NR1START,NR1L,NR2,NR3)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETL4('TINV',GSET%TINV)
!       == GSPACE AND RSPACE DIMENSIONS ================================
        CALL PLANEWAVE__GETI4('NGL',NGL)
        CALL PLANEWAVE__GETI4('NRL',NRL)
        GSET%NGL=NGL
        NULLIFY(GSET%PRO)
        NULLIFY(GSET%DPRO)
        NULLIFY(GSET%YLM)
...
!         == ALLOCATE WAVE FUNCTIONS ===================================
          ...
          ALLOCATE(G2(NGL))
          CALL PLANEWAVE__GETR8A('G2',NGL,G2)
...
!     ==================================================================
      ...
      DO IKPT=1,NKPT
        CALL WAVES_SELECTWV(IKPT,1)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__REPORT(NFILO)
      ENDDO
!
!     ================================================================
!     ==  SEND DATA TO OPTIC CODE                                   ==
!     ================================================================
!      CALL OPTICS__GETL4('ON',TCHK)
!      IF(TCHK) THEN
!        ALLOCATE(XK(3,NKPT))
!        CALL DYNOCC__GETR8A('XK',3*NKPT,XK)
!        TCHK=.FALSE.
...
!            CALL WAVES_SELECTWV(IKPT,1)
!            CALL PLANEWAVE__SELECT(GSET%ID)
!            CALL OPTICS__LATTICE
!            EXIT
!          END IF
!        ENDDO
!        IF(.NOT.TCHK) THEN
!          CALL ERROR__MSG('OPTICS CODE REQUIRES GAMMA POINT IN THE K-POINT SET')
!          CALL ERROR__STOP('WAVES__INITIALIZE')
!        END IF
!        DEALLOCATE(XK)
!      END IF
                           CALL TRACE__POP
...
!     ==================================================================
      ...
      IF(TCHK) THEN
        IF(TSAFEORTHO) THEN
          CALL ERROR__MSG('FOR DYNAMICAL OCCUPATIONS, SAFEORTHO MUST BE FALSE')
          CALL ERROR__STOP('WAVES__ETOT')
        END IF
      END IF
!
!     ==================================================================
...
!     ==================================================================
      CALL CELL__GETL4('MOVE',TSTRESS)
      CALL ATOMS__GETL4('MOVE',TFORCE)
      TSTRESS=TSTRESS.OR.TSTRESSX
      TFORCE=TFORCE.OR.TFORCEX
      IF(TSTRESS)CALL POTENTIAL__SETL4('STRESS',TSTRESS)
      IF(TFORCE)CALL POTENTIAL__SETL4('FORCE',TFORCE)
!
!     ==================================================================
!     == COLLECT VARIABLES                                            ==
!     ==================================================================
...
!     == ATOMIC POSITIONS ==============================================
      ...
      FORCE(:,:)=0.D0
      STRESS=0.D0
      CALL ATOMLIST__GETR8A('R(0)',0,3*NAT,R)
!     == OCCUPATIONS ===================================================
      CALL DYNOCC__GETI4('NB',NBX)
      ALLOCATE(OCC(NBX,NKPT,NSPIN))
      ALLOCATE(EIG(NBX,NKPT,NSPIN))
      CALL DYNOCC__GETR8A('OCC',NBX*NKPT*NSPIN,OCC)
!
!     ==================================================================
...
!           ============================================================
     ...
     &                            ,THIS%PSI0,THIS%PROJ)
      ...
      IF(TFIRST) TFIRST=.FALSE.
                              CALL TIMING__CLOCKON('WAVES__ETOT')
!
!     ==================================================================
!     == KINETIC ENERGY                                               ==
!     ==================================================================
CALL TIMING__CLOCKON('W:EKIN')
      EKIN=0.D0
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          CALL WAVES_SELECTWV(IKPT,ISPIN)
          CALL PLANEWAVE__SELECT(GSET%ID)
          CALL PLANEWAVE__GETR8('GWEIGHT',GWEIGHT)
          NGL=GSET%NGL
          NBH=THIS%NBH
          NB=THIS%NB
...
!stop
PRINT*,'EKIN ',EKIN
      CALL ENERGYLIST__SET('PS  KINETIC',EKIN)
      CALL ENERGYLIST__ADD('AE  KINETIC',EKIN)
      CALL ENERGYLIST__ADD('TOTAL ENERGY',EKIN)
...
!     ==================================================================
CALL TRACE__PASS('BEFORE ONE-CENTER DENSITY MATRIX')
CALL TIMING__CLOCKOFF('W:EKIN')
CALL TIMING__CLOCKON('W:1CD')
      NAT=MAP%NAT
      LMNXX=0
      DO ISP=1,MAP%NSP
...
!     == NONCOLLINEAR    NSPIN=1;NDIM=2: (TOTAL,SPIN_X,SPIN_Y,SPIN_Z) ==
!     ==================================================================
CALL TRACE__PASS('BEFORE PSEUDO DENSITY')
CALL TIMING__CLOCKON('W:PSRHO')
      NRL=MAP%NRL
...
IF(1.EQ.0) THEN
  ...
  ENDDO
  CALL PLANEWAVE__GETR8('RWEIGHT',SVAR2)
  SVAR1=SVAR1*SVAR2
  CALL MPE__COMBINE('+',SVAR1)
...
!     ==================================================================
CALL TRACE__PASS('BEFORE MULTIPOLE MOMENTS')
CALL TIMING__CLOCKON('W:MOMENTS')
      CALL SETUP__LMRXX(LMRXX)
      ALLOCATE(QLM(LMRXX,NAT))
      QLM(:,:)=0.D0
      DO IAT=THISTASK,NAT,NTASKS   ! DISTRIBUTE WORK ACCROSS TASKS
        ISP=MAP%ISP(IAT)
        LMNX=MAP%LMNX(ISP)
        CALL SETUP__LMRX(ISP,LMRX)
        ALLOCATE(DENMAT1(LMNX,LMNX,1))
        DENMAT1(:,:,1)=DENMAT(1:LMNX,1:LMNX,1,IAT)
        CALL AUGMENTATION__MOMENTS(ISP,LMNX,DENMAT1,LMRX,QLM(1,IAT))
        DEALLOCATE(DENMAT1)
      ENDDO
      CALL MPE__COMBINE('+',QLM)
PI=4.D0*DATAN(1.D0)
SVAR1=0.D0
DO IAT=1,NAT
...
CALL TIMING__CLOCKOFF('W:MOMENTS')
!
!     ==================================================================
...
!     ==================================================================
      ...
      CALL POTENTIAL__VOFRHO(NRL,NDIMD,RHO,LMRXX,NAT,QLM,VQLM &
...
!     == AUGMENTATION                                                 ==
!     ==================================================================
CALL TRACE__PASS('BEFORE AUGMENTATION')
CALL TIMING__CLOCKON('W:SPHERE')
      ...
      DO IAT=THISTASK,NAT,NTASKS   ! DISTRIBUTE WORK ACCROSS TASKS
        ...
        CALL AUGMENTATION__SPHERE(ISP,IAT,LMNX,NDIMD,DENMAT1,DENMATI1 &
     &               ,LMRX,VQLM(1,IAT),RHOB,potb1,DH1,DOV1)
      ...
      ENDDO
      DEALLOCATE(VQLM)
      CALL MPE__COMBINE('+',DH)
      CALL MPE__COMBINE('+',potb)
!     ==  SPREAD INFO FROM SPHERE OVER ALL TASKS AND UPDATE ENERGYLIST
      CALL AUGMENTATION__SYNC
!
...
!     ==  to account for background density
      ...
      potb=potb/svar
      rho(:,:)=rho(:,:)+potb
CALL TIMING__CLOCKOFF('W:SPHERE')
!
!     ==================================================================
...
!     CALL OPTICS__GETL4('ON',TCHK)
!     IF(TCHK) THEN
!        CALL MPE__COMBINE('+',DO) ! DO CURRENTLY USED ONLY FOR OPTICS
!        IF(NDIM.EQ.2) THEN
!          CALL ERROR__MSG('OPTICS CODE DOES NOT WORK WITH NONCOLLINEAR MODE')
!          CALL ERROR__STOP('WAVES__ETOT')
!        END IF
!        CALL OPTICS__WRITEOUT_PARTIALS(NSPIN,NAT,LMNXX,DH,DO)
!      END IF
...
!     == FORCES AND STRESSES                                          ==
!     ==================================================================
CALL TRACE__PASS('BEFORE FORCES AND STRESSES')
...
CALL TIMING__CLOCKON('W:FORCE')
      IF(TFORCE.OR.TSTRESS) THEN
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            CALL WAVES_SELECTWV(IKPT,ISPIN)
            CALL PLANEWAVE__SELECT(GSET%ID)
            CALL PLANEWAVE__GETL4('TINV',TINV)
            CALL PLANEWAVE__GETR8('GWEIGHT',GWEIGHT)
            NB=THIS%NB
            NBH=THIS%NBH
            NGL=GSET%NGL
            ALLOCATE(GVEC(3,NGL))
            CALL PLANEWAVE__GETR8A('GVEC',3*NGL,GVEC)
            IF(TSTRESS) THEN
              ALLOCATE(GIJ(6,NGL))
              DO IG=1,NGL
...
!             == DEDPROJ= (DH-LAMBDA*DO)<P|PSPSI> ======================
              ...
              CALL SETUP__1COVERLAP(ISP,LNX,DO1)
...
!             == DE= <DPRO|DEDPRO> =====================================
              ALLOCATE(EIGR(NGL))
              CALL PLANEWAVE__STRUCTUREFACTOR(R(1,IAT),NGL,EIGR)
     ...
     &                   ,EIGR,DEDPRO,FORCE1,TSTRESS,STRESS1)
              CALL MPE__COMBINE('+',FORCE1)
              IF(TSTRESS)CALL MPE__COMBINE('+',STRESS1)
              DEALLOCATE(EIGR)
              DEALLOCATE(DEDPRO)
        ...
        ENDDO
      END IF
CALL TIMING__CLOCKOFF('W:FORCE')
!
!     ==================================================================
!     ==  EVALUATE H*PSI                                              ==
!     ==================================================================
CALL TRACE__PASS('BEFORE HPSI')
CALL TIMING__CLOCKON('W:HPSI')
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          CALL WAVES_SELECTWV(IKPT,ISPIN)
          CALL PLANEWAVE__SELECT(GSET%ID)
...
CALL TIMING__CLOCKON('W:HPSI.1')
          CALL WAVES_VPSI(GSET,NGL,NDIM,NBH,NRL,THIS%PSI0,RHO(1,ISPIN) &
     &                   ,THIS%HPSI)
CALL TIMING__CLOCKOFF('W:HPSI.1')
!
!         ==============================================================
!         ==  EVALUATE  DH<P|PSI>                                     ==
!         ==============================================================
CALL TIMING__CLOCKON('W:HPSI.2')
      ...
      &              ,NDIM,NBH,LMNX,THIS%PROJ(1,1,IPRO),HPROJ(1,1,IPRO))
            IPRO=IPRO+LMNX
          ENDDO
CALL TIMING__CLOCKOFF('W:HPSI.2')
!
!         ==============================================================
!         ==  ADD  |P>DH<P|PSI>                                       ==
!         ==============================================================
CALL TIMING__CLOCKON('W:HPSI.3')
          CALL WAVES_ADDPRO(MAP,GSET,NAT,R,NGL,NDIM,NBH,MAP%NPRO &
     &                     ,THIS%HPSI,HPROJ)
CALL TIMING__CLOCKOFF('W:HPSI.3')
          DEALLOCATE(HPROJ)
        ENDDO
      ENDDO
      DEALLOCATE(RHO)
      DEALLOCATE(DH)
CALL TIMING__CLOCKOFF('W:HPSI')
!
!     ==================================================================
!     ==  EVALUATE ENERGY EXPECTATION VALUES                          ==
!     ==================================================================
CALL TIMING__CLOCKON('W:EXPECT')
...
!PRINT*,'EIG ',EIG(:,IKPT,ISPIN)
      ...
      DEALLOCATE(HAMILTON)
!     == OCCUPATIONS ===================================================
      CALL DYNOCC__SETR8A('EPSILON',NB*NKPT*NSPIN,EIG)
!
!     ==================================================================
!     ==  EVALUATE <S^2>                                              ==
!     ==================================================================
      IF ((NDIM.EQ.2).AND.THAMILTON) THEN   ! THAMILTON SHOULD BE REPLACED
         CALL TIMING__CLOCKON('S2')
         DO IKPT=1,NKPT
            CALL WAVES_SELECTWV(IKPT,1)
            CALL PLANEWAVE__SELECT(GSET%ID)
            NB=THIS%NB
            ...
            DEALLOCATE(QMAT)
         END DO
         CALL TIMING__CLOCKOFF('S2')
         CALL FILEHANDLER__UNIT('PROT',NFILO)
         CALL WAVES__REPORTSPIN(NFILO)
      END IF
!
!     ==================================================================
!     ==  EVALUATE <PSI|H|PSI>                                        ==
      ...
      IF(THAMILTON) THEN
        DO IKPT=1,NKPT
          DO ISPIN=1,NSPIN
            ...
            CALL LIB__DIAGC8(NB,HAMILTON,THIS%EIGVAL,THIS%EIGVEC)
            DEALLOCATE(HAMILTON)
          ENDDO
        ENDDO
      ELSE
        ...
        IF(ASSOCIATED(THIS%EIGVEC))DEALLOCATE(THIS%EIGVEC)
      END IF
CALL TIMING__CLOCKOFF('W:EXPECT')
!
!     ==================================================================
...
!     == FORCES ========================================================
      ALLOCATE(FORCET(3,NAT))
      CALL ATOMLIST__GETR8A('FORCE',0,3*NAT,FORCET)
      FORCE=FORCET+FORCE
      CALL ATOMLIST__SETR8A('FORCE',0,3*NAT,FORCE)
      DEALLOCATE(FORCET)
!     == STRESS ========================================================
...
WRITE(*,FMT='("AFTER WAVES STRESS ",3F15.7)')STRESS(3,:)
      CALL CELL__GETR8A('STRESS_I',9,STRESS1)
      STRESS=STRESS1-STRESS  ! IN THIS ROUTINE STRESS=+DE/DEPSILON!
      CALL CELL__SETR8A('STRESS_I',9,STRESS)
!     == DEALLOCATE ARRAYS =============================================
      DEALLOCATE(FORCE)
      DEALLOCATE(OCC)
      DEALLOCATE(R)
      DEALLOCATE(EIG)
                              CALL TIMING__CLOCKOFF('WAVES__ETOT')
                                   CALL TRACE__POP
      RETURN
      END
!
...
!     -- REMARK: CONSIDER BLOCKING THIS INTO SMALLER BUNCHES OF BANDS
      ...
      CALL PLANEWAVE__FFT('GTOR',NBH*NDIM,NGL,PSI,NRL,PSIOFR)
!
!     ==================================================================
...
!     ==================================================================
      CALL PLANEWAVE__FFT('RTOG',NBH*NDIM,NGL,HPSI,NRL,PSIOFR)
      DEALLOCATE(PSIOFR)
!
!     ==================================================================
...
!     ==================================================================
      ...
      IF(TINV) THEN
        IF(NBH.NE.(NB+1)/2) THEN
          CALL ERROR__MSG('INCONSISTENT NUMBER OF BANDS')
          CALL ERROR__STOP('WAVES_DENMAT')
        END IF
      ELSE
        IF(NBH.NE.NB) THEN
          CALL ERROR__MSG('INCONSISTENT NUMBER OF BANDS')
...
!       ================================================================
        ...
        DO IDIM=1,NDIM
          CALL PLANEWAVE__INVERTG(NGL,PSI(1,IDIM,IB),PSI1(1,IDIM))
        ENDDO
        DO IDIM=1,NDIM
          DO IG=1,NGL
            DMAT(IG)=DMAT(IG) &
...
!       == PARALLELIZE AND CLOSE DOWN ==================================
        ...
        CALL MPE__COMBINE('+',EKIN)
        CALL MPE__COMBINE('+',STRESS)
        DEALLOCATE(GVEC)
        DEALLOCATE(DMAT)
      ELSE
        ALLOCATE(G2(NGL))
...
!     ************P.E. BLOECHL, IBM RESEARCH LABORATORY ZURICH (1999)***
      ...
      REAL(8)               :: SVAR
!     ******************************************************************
      CALL ERROR__MSG('ROUTINE MARKED FOR DELETION. CONTAINS ERRORS!')
      CALL ERROR__STOP('WAVES_EKIN_OLD')
!
!     ==================================================================
!     ==  CHECK IF SUPERWAVEFUNCTIONS ARE USED AND IF #(BANDS) CORRECT==
!     ==================================================================
...
!       ================================================================
        IF(.NOT.TINV.OR.ABS(FM).LT.DSMALL) CYCLE
        DO IDIM1=1,NDIM
          CALL PLANEWAVE__INVERTG(NGL,PSI(1,IDIM1,IB),PSI1(1,IDIM1))
        ENDDO
        DO IDIM1=1,NDIM
          DO IDIM2=IDIM1,NDIM
            SVAR=FM
...
!     ==================================================================
      ALLOCATE(PSIOFR(NRL,NDIM,NBH))
      CALL PLANEWAVE__FFT('GTOR',NBH*NDIM,NGL,PSIOFG,NRL,PSIOFR)
!
!     ==================================================================
...
!     ==================================================================
      IF(TINV) THEN
        IF(NDIM.EQ.2) THEN
          CALL ERROR__MSG('SPINOR WAVE FUNCTIONS ARE NOT COMPATIBLE')
          CALL ERROR__MSG('WITH SUPER WAVE FUNCTIONS')
          CALL ERROR__STOP('WAVES_DENSITY')
        END IF
        CALL PLANEWAVE__GETC8A('EIKR',NRL,EI2KR)
        DO IR=1,NRL
          EI2KR(IR)=EI2KR(IR)**2
        ENDDO
        RHO(:,:)=0.D0
...
!     ==================================================================
      ...
      IF(TINV) THEN
        IF(NBH.NE.(NB+1)/2) THEN
          CALL ERROR__MSG('INCONSISTENT NUMBER OF BANDS')
          CALL ERROR__STOP('WAVES_DEDPROJ')
        END IF
      ELSE
        IF(NBH.NE.NB) THEN
          CALL ERROR__MSG('INCONSISTENT NUMBER OF BANDS')
...
!     ==================================================================
      ...
      CALL LIB__MATMULC8(NGL,NDIM*NBH,LMNX,PSI,DEDPROJ1,DEDPRO)
!     IF(TESSL) THEN
!        DEDPROJ1=CONJG(DEDPROJ)
...
!     END IF
      IF(TINV) THEN
        ALLOCATE(PSIM(NGL))
        DO LMN=1,LMNX
          CALL PLANEWAVE__INVERTG(NGL,DEDPRO(1,LMN),PSIM)
          DO IG=1,NGL
...
!     ******************************************************************
                                CALL TIMING__CLOCKON('WAVES_PROJECTIONS')
!
!     ==================================================================
!     ==  CONSISTENCY CHECKS                                          ==
!     ==================================================================
      IF(NPRO.NE.MAP%NPRO) THEN
        CALL ERROR__MSG('INCONSISTENT NPRO')
        CALL ERROR__STOP('WAVES_PROJECTIONS')
      END IF
      IF(GSET%NGL.NE.NGL) THEN
        CALL ERROR__MSG('INCONSISTENT NGL')
        CALL ERROR__STOP('WAVES_PROJECTIONS')
      END IF
!
!     ==================================================================
...
!     ==================================================================
      ALLOCATE(GVEC(3,NGL))
      CALL PLANEWAVE__GETR8A('GVEC',3*GSET%NGL,GVEC)
!
!     ==================================================================
!     ==  <PRO|PSI>                                                   ==
!     ==================================================================
     ...
     &                      ,GSET%PRO(1,IBPRO),MAP%LMX,GSET%YLM,EIGR,PRO)
        CALL PLANEWAVE__SCALARPRODUCT(' ',NGL,1,LMNX,PRO,NDIM*NB,PSI &
     &                              ,PROPSI1)
        II=0
        DO IB=1,NB
          DO IDIM=1,NDIM
      ...
      DEALLOCATE(EIGR)
      DEALLOCATE(GVEC)
      CALL MPE__COMBINE('+',PROPSI)
                               CALL TIMING__CLOCKOFF('WAVES_PROJECTIONS')
      RETURN
      END
!
!     .....................................................NLSMX........
...
!     ************P.E. BLOECHL, IBM RESEARCH LABORATORY ZURICH (1998)***
      ...
      INTEGER(4)                 :: LNX,LMNX,ISP
!     ******************************************************************
                                 CALL TIMING__CLOCKON('WAVES_ADDPRO')
!
!     ==================================================================
!     ==  CONSISTENCY CHECKS                                          ==
!     ==================================================================
...
!       ===============================================================
        ...
        DO IB=1,NB
          ...
          ENDDO
        ENDDO
        CALL LIB__ADDPRODUCTC8(.FALSE.,NGL,LMNX,NDIM*NB,PRO,PROPSI1,PSI)
!       IF(TESSL) THEN
...
!       END IF
      ...
      DEALLOCATE(EIGR)
      DEALLOCATE(PROPSI1)
                                 CALL TIMING__CLOCKOFF('WAVES_ADDPRO')
      RETURN
      END
!
!     ..................................................................
...
!     ******************************************************************
                            CALL TRACE__PUSH('WAVES_UPDATEGSET')
!
!     ==================================================================
!     ==  UPDATE RADIAL PROJECTOR FUNCTIONS                           ==
!     ==================================================================
      CALL CELL__GETL4('MOVE',TSTRESS)
      CALL CELL__GETL4('ON',TSTRESS)
      CALL CELL__GETR8A('T0',9,RBAS)
      CALL POTENTIAL__SETR8A('RBAS',9,RBAS)
      CALL GBASS(RBAS,GBAS,CELLVOL)
      LMX=MAP%LMX
      DO IKPT=1,NKPT
        CALL WAVES_SELECTWV(IKPT,1)
...
!       ================================================================
        CALL PLANEWAVE__SETR8A('RBAS',9,RBAS)
!
!       ================================================================
...
!       ================================================================
        ...
        IND=0
        DO ISP=1,MAP%NSP
          CALL SETUP__ISELECT(ISP)
          DO LN=1,MAP%LNX(ISP)
            IND=IND+1
            CALL SETUP__GETFOFG('PRO',.FALSE.,LN,NGL,G2,CELLVOL,GSET%PRO(1,IND))
            CALL SETUP__GETFOFG('PRO',.TRUE.,LN,NGL,G2,CELLVOL,GSET%DPRO(1,IND))
          ENDDO
        ENDDO
!
!       ================================================================
...
!       ================================================================
        DO IND=1,10
          CALL SPHERICAL__CCMAT0(LM1,IND,LM2,CC)
          IF(LM2.EQ.0) EXIT
          IJ=0
          DO I=1,3
            DO J=I,3
...
!       ================================================================
        DO IND=1,10
          CALL SPHERICAL__CCMATM(LM1,IND,LM2,CC)
          IF(LM2.EQ.0) EXIT
          IJ=0
          DO I=1,3
            DO J=I,3
...
!     ******************************************************************
                            CALL TRACE__PUSH('WAVES__PROPAGATE')
!
!     ==================================================================
!     ==  STOP WAVE FUNCTIONS                                         ==
!     ==================================================================
...
!       ==  NOW PROPAGATE ==============================================
        DO ISPIN=1,NSPIN
          ...
          IF(.NOT.ASSOCIATED(THIS%HPSI)) THEN
            CALL ERROR__MSG('HPSI NOT AVAILABLE')
            CALL ERROR__MSG('EITHER WAVES__ETOT HAS NOT BEEN CALLED')
            CALL ERROR__MSG('OR WAVES__PROPAGATE HAS BEEN CALLED PREVIOUSLY')
            CALL ERROR__I4VAL('IKPT',IKPT)
            CALL ERROR__I4VAL('ISPIN',ISPIN)
            CALL ERROR__STOP('WAVES__PROPAGATE')
          END IF
          NBH=THIS%NBH
          DO IB=1,NBH
            DO IDIM=1,NDIM
...
!     ******************************************************************
      CALL CELL__GETL4('ON',TSTRESS)
      IF(TSTRESS) THEN
        CALL CELL__GETR8A('FRICMAT',9,FRICMAT) !\ALPHADOT*DELTA/2
      ELSE
        FRICMAT=0.D0
      END IF
...
!
!ALLOCATE(MARR(NGL))
!CALL PLANEWAVE__GETR8A('G2',NGL,MARR)
!DO IG=1,NGL
...
!           == PSI- ONLY REQUIRED IF THE OCCUPATIONS DIFFER ==========
            ...
            DO IDIM=1,NDIM
              ...
              CALL PLANEWAVE__INVERTG(NGL,TPSIP,TPSIM)
              DO IG=1,NGL
...
!     ==================================================================
      CALL MPE__COMBINE('+',EKIN)
      CALL MPE__COMBINE('+',EIG)
!
!     ==================================================================
!     == SET CONTRIBUTION TO EIGENVALUES                              ==
!     ==================================================================
      CALL DYNOCC__SETR8A('M<PSIDOT|PSIDOT>',NBX*NKPT*NSPIN,EIG)
      DEALLOCATE(EIG)
      DEALLOCATE(OCC)
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE WAVES__ORTHOGONALIZE()
!     ******************************************************************
...
!     ******************************************************************
      ...
      INTEGER(4)             :: I1,J1,K
!     ******************************************************************
                             CALL TRACE__PUSH('WAVES__ORTHOGONALIZE')
                             CALL TIMING__CLOCKON('WAVES__ORTHOGONALIZE')
      NPRO=MAP%NPRO
      NAT=MAP%NAT
      CALL CELL__GETL4('MOVE',TSTRESS)
!
...
!     ==================================================================
      ...
      CALL DYNOCC__GETR8A('OCC',NBX*NKPT*NSPIN,OCC)
      ALLOCATE(R0(3,NAT))
      CALL ATOMLIST__GETR8A('R(0)',0,3*NAT,R0)
!
!     ==================================================================
...
!         ==============================================================
          ...
          DO IAT=1,NAT
            ...
            LMNX=MAP%LMNX(ISP)
            ALLOCATE(DO(LNX,LNX))
            CALL SETUP__1COVERLAP(ISP,LNX,DO)
...
!     ==================================================================
      ALLOCATE(RP(3,NAT))
      ALLOCATE(RM(3,NAT))
      CALL ATOMLIST__GETR8A('R(+)',0,3*NAT,RP)
      IF(TSTRESS) THEN
!       == PREDICT NEW POSITIONS =======================================
        CALL CELL__GETR8A('TP',9,RBAS)
        CALL CELL__GETR8A('MAPTOCELL',9,MAPTOCELL)
        CALL ATOMLIST__GETR8A('R(-)',0,3*NAT,RM)
        DO IAT=1,NAT
...
!PRINT*,'MAX ',SVAR*27.211D0
              DO I=1,NB-1
                ...
                IF(REAL(LAMBDA(I1,I1)).GT.REAL(LAMBDA(J1,J1))) THEN
                  CALL ERROR__MSG('STATE ORDERING FAILED')
                  CALL ERROR__STOP('WAVES__ORTHOGONALIZE')
                END IF
              ENDDO
!!__PRINT*,'---------- LAMBDA SPIN: ',ISPIN,' ------------------'
!!__WRITE(*,FMT='("LAMBDA SMAP:",20I4)')SMAP
!!__WRITE(*,FMT='("LAMBDA IMAP:",20I4)') (I,I=1,NB)
!!__PRINT*,'MAX NON-DIAGNAL LAMBDA',SVAR*27.211D0
!!__WRITE(*,FMT='("LAMBDA BEFORE:",10F8.3)') (REAL(LAMBDA(I,I))*27.211D0,I=1,NB)
!!__WRITE(*,FMT='("LAMBDA OCC:   ",10F8.3)')OCC(:,IKPT,ISPIN)
!!__ELSE
!!__   PRINT*,'LAMBDA SMAP SWITCHED OFF!!'
            END IF
          END IF
!===============================================================
          DO I=1,NB
...
PRINT*,'WARNING FROM WAVES__ORTHOGONALIZE:'
...
!PRINT*,'PS-CHARGE AFTER ORTHOGONALIZATION ',CSUM
            ...
            ENDDO
            ALLOCATE(NORM(NB))
            CALL LIB__DIAGC8(NB,MAT,NORM,AUXMAT)
...
!PRINT*,'NORM ',NORM
            DO I=1,NB
              IF(ABS(NORM(I)-1.D0).GT.1.D-4) THEN
                CALL ERROR__MSG('ORTHOGONALIZATION FAILED')
                CALL ERROR__I4VAL('STATE',I)
                CALL ERROR__R8VAL('NORM',NORM(I))
                CALL ERROR__STOP('WAVES__ORTHOGONALIZE')
              END IF
            ENDDO
            DEALLOCATE(NORM)
...
!     ==================================================================
      CALL WAVES_WAVEKINETIC(WAVEEKIN2)
                             CALL TIMING__CLOCKOFF('WAVES__ORTHOGONALIZE')
                                    CALL TRACE__POP
      RETURN
      END
!
...
!      *****************************************************************
       ...
       INTEGER(4)              :: NGLNDIM
!      *****************************************************************
                               CALL TIMING__CLOCKON('WAVES_ADDOPSI')
       TINV=NBH.NE.NB
       NGLNDIM=NGL*NDIM
       IF(.NOT.TINV) THEN
         CALL LIB__ADDPRODUCTC8(.FALSE.,NGLNDIM,NBH,NBH,OPSI,LAMBDA,PSIBAR)
!        IF(TESSL) THEN
...
!        ENDIF
       ELSE
         ...
         ENDDO
!        == ADD O|PSI_+>LAMBDA1 =======================================
         CALL LIB__ADDPRODUCTC8(.FALSE.,NGLNDIM,NBH,NBH,OPSI,LAMBDA1,PSIBAR)
!        IF(TESSL) THEN
!          CALL ZGEMM('N','N',NGLNDIM,NBH,NBH,(1.D0,0.D0),OPSI,NGLNDIM &
...
!        == INVERT OPSI ================================================
         ...
         DO IBH1=1,NBH
           I=1
           DO IDIM=1,NDIM
             CALL PLANEWAVE__INVERTG(NGL,OPSI(I,IBH1),TPSI(I))
             I=I+NGL
           ENDDO
           OPSI(:,IBH1)=TPSI(:)
         ENDDO
...
!
!        == ADD O|PSI_+>LAMBDA1 =======================================
         CALL LIB__ADDPRODUCTC8(.FALSE.,NGLNDIM,NBH,NBH,OPSI,LAMBDA2,PSIBAR)
!        IF(TESSL) THEN
!          CALL ZGEMM('N','N',NGLNDIM,NBH,NBH,(1.D0,0.D0),OPSI,NGLNDIM &
...
!        END IF
         DEALLOCATE(LAMBDA2)
       END IF
                               CALL TIMING__CLOCKOFF('WAVES_ADDOPSI')
       RETURN
       END
!
!      .................................................................
...
!      **                                                          **
       ...
       LOGICAL(4)            :: TINV
!      **************************************************************
                             CALL TIMING__CLOCKON('WAVES_OVERLAP')
       TINV=(NB.NE.NBH)
       IF(.NOT.TINV) THEN
         IF(TID) THEN
           CALL PLANEWAVE__SCALARPRODUCT('=',NGL,NDIM,NB,PSI1,NB,PSI2,MAT)
         ELSE
           CALL PLANEWAVE__SCALARPRODUCT(' ',NGL,NDIM,NB,PSI1,NB,PSI2,MAT)
         END IF
         CALL MPE__COMBINE('+',MAT)
                             CALL TIMING__CLOCKOFF('WAVES_OVERLAP')
         RETURN
       ENDIF
!
!      =================================================================
...
!      == <PSI_+|PSI_+> ================================================
       ...
       IF(TID) THEN
         CALL PLANEWAVE__SCALARPRODUCT('=',NGL,NDIM,NBH,PSI1,NBH,PSI2,TMAT)
       ELSE
         CALL PLANEWAVE__SCALARPRODUCT(' ',NGL,NDIM,NBH,PSI1,NBH,PSI2,TMAT)
       END IF
       DO IBH1=1,NBH
...
!       __ NOW CONTINUE_________________________________________________
        ...
        CALL SETUP__1COVERLAP(ISP,LNX,DOVER)
!
        DO IBH1=1,NBH
          DO IBH2=1,NBH
            CSVAR1=(0.D0,0.D0)
...
!      *****************************************************************
                             CALL TRACE__PUSH('WAVES_ORTHO_Y_C')
!
!      =================================================================
!      ==  INITIALIZE                                                 ==
!      =================================================================
...
!      =================================================================
!                            CALL TRACE__PASS('BEFORE ORTHOGONALIZATION LOOP')
       DO NU=1,NB
         N=MAP(NU)
!
!        ===============================================================
...
!        ===============================================================
         ...
         ENDDO
!               CALL TRACE__PASS('ORTHOGONALIZE HIGHER PHIS TO THIS PHI')
!
!        ===============================================================
!        == NOW UPDATE MATRICES                                       ==
!        ===============================================================
...
!        ===============================================================
!               CALL TRACE__PASS('ORTHOGONALIZE HIGHER CHIS TO THIS PHI')
         DO IU=1,NU
           I=MAP(IU)
           Z(I)=(0.D0,0.D0)
         ENDDO
...
!      =================================================================
      ...
      &                   +CONJG(CHIPHI(K,I))*X(K,J)
             ...
             IF(ABS(CSVAR).GT.TOL) THEN
               CALL ERROR__MSG('ORTHOGONALIZATION FAILED')
               CALL ERROR__I4VAL('I',I)
               CALL ERROR__I4VAL('J',J)
               CALL ERROR__C8VAL('<PHI(+)|O|PHI(+)>-1',CSVAR)
               CALL ERROR__STOP('WAVES_ORTHO_Y')
             END IF
           ENDDO
         ENDDO
...
!      *****************************************************************
       ...
       INTEGER(4)            :: MAP(NB)
!      *****************************************************************
                             CALL TRACE__PUSH('WAVES_ORTHO_Y')
!
!      =================================================================
!      ==  INITIALIZE                                                 ==
!      =================================================================
       DO I=1,NB
         ...
         ALPHA(I,I)=1.D0
       ENDDO
                             CALL TRACE__PASS('BEFORE TESTME')
       IF(TPR)CALL TESTME
!
!      =================================================================
!      ==  ORTHOGONALIZATION LOOP                                     ==
         ...
         DO I=1,NB
           DO J=1,NB
             ...
             IF(ABS(SVAR).GT.TOL) THEN
               ...
               CALL ERROR__I4VAL('I',I)
               CALL ERROR__I4VAL('J',J)
               CALL ERROR__R8VAL('<PHI(+)|O|PHI(+)>-1',SVAR)
               CALL ERROR__STOP('WAVES_ORTHO_Y')
             END IF
           ENDDO
         ENDDO
...
!     ******************************************************************
                             CALL TRACE__PUSH('WAVES_ORTHO_X_C')
      ALLOCATE(GAMN(NB,NB))
!
!     ==================================================================
...
!     ==================================================================
      DO I=1,NB
        ...
        ENDDO
      ENDDO
      CALL LIB__DIAGC8(NB,GAMN,EIG,U)
...
!         __GAMN(I,J) = CHICHI(I,K)*LAMBDA(K,J)___________________________
          CALL LIB__MATMULC8(NB,NB,NB,CHICHI,LAMBDA,HAUX)
...
!         __GAMN(I,J) = LAMBDA(K,I)*HAUX(K,J)_____________________________
          CALL LIB__SCALARPRODUCTC8(.FALSE.,NB,NB,LAMBDA,NB,HAUX,GAMN)
...
!         ----  HAUX(I,L)=U(K,I)*H0(K,L)
          CALL LIB__SCALARPRODUCTC8(.FALSE.,NB,NB,U,NB,GAMN,HAUX)
!CALL ZGEMUL(U,NB,'C',GAMN,NB,'N',HAUX,NB,NB,NB,NB)
!         ----  GAMN(I,J)=HAUX(I,L)*U(L,I)
          CALL LIB__MATMULC8(NB,NB,NB,HAUX,U,GAMN)
...
!         ----  HAUX(I,L)=U(K,I)*H0(K,L)
          CALL LIB__MATMULC8(NB,NB,NB,U,GAMN,HAUX)
!CALL ZGEMUL(U,NB,'N',GAMN,NB,'N',HAUX,NB,NB,NB,NB)
!         ----  GAMN(I,J)=HAUX(I,L)*U(L,I)
          CALL LIB__DYADSUMC8(NB,NB,NB,HAUX,U,GAMN)
...
!
      ENDDO
      CALL ERROR__MSG('LOOP FOR ORTHOGONALIZATION IS NOT CONVERGED')
      CALL ERROR__STOP('WAVES_ORTHO_X_C')
!
9000  CONTINUE
      DEALLOCATE(GAMN)
                             CALL TRACE__POP
...
!     IMPLICIT NONE
      ...
      INTEGER(4),EXTERNAL      :: IDAMAX
!     ******************************************************************
                             CALL TRACE__PUSH('WAVES_ORTHO_X')
      ALLOCATE(GAMN(NB,NB))
!
!     ==================================================================
!     ==  CALCULATE  PSIPSI(I,J)= <PSIBAR(I)|PSIBAR(J)>-1             ==
...
!     ==  DIAGONALIZE 0.5*(CHIPSI(I,J)+CHIPSI(J,I))                   ==
!     ==================================================================
      CALL LIB__DIAGR8(NB,CHIPSI,EIG,U)
...
!       ================================================================
!       __GAMN(I,J) = CHICHI(I,K)*LAMBDA(K,J)___________________________
        CALL LIB__MATMULR8(NB,NB,NB,CHICHI,LAMBDA,HAUX)
!CALL DGEMUL(CHICHI,NB,'N',LAMBDA,NB,'N',HAUX,NB,NB,NB,NB)
!       __HAUX(I,J) = HAUX(I,J)+2*CHIPSI(I,J)___________________________
        HAUX=HAUX+2.D0*CHIPSI
!CALL DAXPY(NB*NB,2.D0,CHIPSI,1,HAUX,1)
!       __GAMN(I,J) = LAMBDA(K,I)*HAUX(K,J)_____________________________
        CALL LIB__SCALARPRODUCTR8(.FALSE.,NB,NB,LAMBDA,NB,HAUX,GAMN)
...
!       == TRANSFORM OVERLAP MATRIX GAMN
!       ----  HAUX(I,L)=U(K,I)*H0(K,L)
        CALL LIB__SCALARPRODUCTR8(.FALSE.,NB,NB,U,NB,GAMN,HAUX)
!CALL DGEMUL(U,NB,'T',GAMN,NB,'N',HAUX,NB,NB,NB,NB)
!       ----  GAMN(I,J)=HAUX(I,L)*U(L,I)
        CALL LIB__MATMULR8(NB,NB,NB,HAUX,U,GAMN)
...
!       == TRANSFORM OVERLAP MATRIX GAMN BACK
!       ----  HAUX(I,L)=U(K,I)*H0(K,L)
        CALL LIB__MATMULR8(NB,NB,NB,U,GAMN,HAUX)
!CALL DGEMUL(U,NB,'N',GAMN,NB,'N',HAUX,NB,NB,NB,NB)
!       ----  GAMN(I,J)=HAUX(I,L)*U(L,I)
        CALL LIB__DYADSUMR8(NB,NB,NB,HAUX,U,GAMN)
...
!      ENDDO
      CALL ERROR__MSG('LOOP FOR ORTHOGONALIZATION IS NOT CONVERGED')
      CALL ERROR__STOP('WAVES_ORTHO_X')
...
!       == FIRST FOLD DOWN OVERLAP MATRIX TO SUPER WAVE FUNCTIONS ======
        ...
        PSIINV=PSI
        CALL PLANEWAVE__ADDPRODUCT(' ',NGL,NDIM,NBH,PSI,NBH,PSIINV,X1)
        CALL PLANEWAVE__ADDPRODUCT('-',NGL,NDIM,NBH,PSI,NBH,PSIINV,X2)
        DEALLOCATE(PSIINV)
!        DO I=NBH,1,-1
...
!        ENDDO
      ...
      ELSE
        ...
        PSIINV=PSI
        CALL PLANEWAVE__ADDPRODUCT(' ',NGL,NDIM,NB,PSI,NB,PSIINV,X)
        DEALLOCATE(PSIINV)
!        DO I=NB,1,-1
!          DO J=1,I
!            DO IDIM=1,NDIM
...
!     =================================================================
      IF(TTEST) THEN
        ...
        DEALLOCATE(AUXMAT)
        ALLOCATE(X(NB,NB))
        CALL LIB__DIAGC8(NB,OVERLAP,NORM,X)
!CALL CDIAG(NB,NB,OVERLAP,NORM,X)
        DEALLOCATE(OVERLAP)
        DO I=1,NB
          IF(ABS(NORM(I)-1.D0).GT.1.D-4) THEN
            CALL ERROR__MSG('GRAM-SCHMIDT ORTHOGONALIZATION FAILED')
            CALL ERROR__I4VAL('STATE',I)
            CALL ERROR__R8VAL('NORM',NORM(I))
            CALL ERROR__STOP('WAVES_GRAMSCHMIDT')
          END IF
        ENDDO
        DEALLOCATE(X)
      END IF
...
!     ******************************************************************
      ...
      ENDDO
      CALL LIB__RANDOMSEED
      DO IB=1,NB
        DO IDIM=1,NDIM
          DO IG=1,NG
            CALL LIB__RANDOM(REC)
            CALL LIB__RANDOM(RIM)
            REC=2.D0*REC-1.D0
            RIM=2.D0*RIM-1.D0
...
!     ******************************************************************
      ...
      CALL REPORT__TITLE(NFIL,'WAVE FUNCTIONS')
      CALL WAVES_SELECTWV(1,1)
      CALL REPORT__I4VAL(NFIL,'NUMBER OF BANDS',THIS%NB,' ')

      CALL REPORT__I4VAL(NFIL,'NUMBER OF K-POINTS',NKPT,' ')
      CALL REPORT__I4VAL(NFIL,'NUMBER OF SPINS',NSPIN,' ')
      CALL REPORT__I4VAL(NFIL,'NUMBER OF SPINOR COMPONENTS',NDIM,' ')
      CALL REPORT__R8VAL(NFIL,'PLANE WAVE CUTOFF',EPWPSI/RY,'RY')
      CALL REPORT__R8VAL(NFIL,'WAVE FUNCTION MASS',EMASS,'A.U.')
      CALL REPORT__R8VAL(NFIL,'G**2 ENHANCEMENT OF FUNCTION MASS',EMASSCG2,' ')
      CALL REPORT__R8VAL(NFIL,'BUCKET POTENTIAL STARTS AT 0.5G^2=',EPWPSI0/RY,'RY')
      CALL REPORT__R8VAL(NFIL,'BUCKET POTENTIAL PREFACTOR',D2EPWPSI,'H')
      IF(ANNEE.NE.0) THEN
        CALL REPORT__R8VAL(NFIL,'FRICTION',ANNEE,' ')
      END IF
      IF(TSTOP) THEN
        CALL REPORT__CHVAL(NFIL,'INITIAL VELOCITY IS SET TO','ZERO')
      END IF
!     IF(TRANDOMIZE) THEN
!       CALL REPORT__R8VAL(NFIL &
...
!     ================================================================
      DO IKPT=1,NKPT
        CALL WAVES_SELECTWV(IKPT,1)
        NG=GSET%NGL
        CALL MPE__COMBINE('+',NG)
        CALL REPORT__I4VAL(NFIL &
...
!     ******************************************************************
      ...
      REAL(8)   ,ALLOCATABLE :: OCC(:,:,:)
!     ******************************************************************
                             CALL TRACE__PUSH('WAVES__WRITEPDOS')
      IF(.NOT.THAMILTON) THEN
        CALL ERROR__MSG('EIGENVALUES NOT PRESENT')
        CALL ERROR__STOP('WAVES__WRITEPDOS')
      END IF
      CALL MPE__QUERY(NTASKS,THISTASK)
!
!     ==================================================================
...
!     ==================================================================
      ...
      NPRO=MAP%NPRO
      LNXX=MAP%LNXX
      CALL PDOS__SETI4('NAT',NAT)
      CALL PDOS__SETI4('NSP',NSP)
      CALL PDOS__SETI4('NKPT',NKPT)
      CALL PDOS__SETI4('NSPIN',NSPIN)
      CALL PDOS__SETI4('NDIM',NDIM)
      CALL PDOS__SETI4('NPRO',NPRO)
      CALL PDOS__SETI4('LNXX',LNXX)
      CALL PDOS__SETI4A('LNX',NSP,MAP%LNX)
      CALL PDOS__SETI4A('LOX',LNXX*NSP,MAP%LOX)
      CALL PDOS__SETI4A('ISPECIES',NAT,MAP%ISP)
      IF(THISTASK.EQ.1) THEN
        CALL FILEHANDLER__UNIT('PDOS',NFIL)
        REWIND NFIL
...
!     ==================================================================
      ALLOCATE(R(3,NAT))
      ALLOCATE(ATOMID(NAT))
      CALL CELL__GETR8A('T(0)',9,RBAS)
      CALL ATOMLIST__GETR8A('R(0)',0,3*NAT,R)
      CALL ATOMLIST__GETCHA('NAME',0,NAT,ATOMID)
      CALL PDOS__SETR8A('RBAS',9,RBAS)
      CALL PDOS__SETR8A('R',3*NAT,R)
!     CALL PDOS__SETCHA('ATOMID',NAT,ATOMID)
      IF(THISTASK.EQ.1) THEN
...
!     ==================================================================
      ...
      ALLOCATE(IZ(NSP))
      ALLOCATE(RAD(NSP))
      CALL SETUP__GETI4('NR',NR)
      ALLOCATE(AEPHI(NR,LNXX))
      ALLOCATE(WORK(NR))
      CALL SETUP__GETR8('R1',R1)
      CALL SETUP__GETR8('DEX',DEX)
      XEXP=EXP(DEX)
      DO ISP=1,NSP
        OV(:,:,:)=0.D0
        CALL SETUP__ISELECT(ISP)
        LNX=MAP%LNX(ISP)
        LOX=MAP%LOX(:,ISP)
        CALL SETUP__GETR8('AEZ',AEZ)
        IZ(ISP)=NINT(AEZ)
        CALL PERIODICTABLE__GET(IZ(ISP),'R(ASA)',RAD(ISP))
        CALL SETUP__AEPARTIALWAVES(ISP,NR,LNX,AEPHI)
        DO LN1=1,LNX
          CALL RADIAL__VALUE(R1,DEX,NR,AEPHI(1,LN1),RAD(ISP),VAL(LN1,ISP))
          CALL RADIAL__DERIVATIVE(R1,DEX,NR,AEPHI(1,LN1),RAD(ISP),DER(LN1,ISP))
          DO LN2=LN1,LNX
            ...
            ENDDO
            CALL RADIAL__INTEGRAL1(R1,DEX,NR,WORK,RAD(ISP),OV(LN1,LN2,ISP))
     ...
     &              ,OV(1:LNX,1:LNX,ISP)
        END IF
      ENDDO
      CALL PDOS__SETI4A('IZ',NSP,IZ)
      CALL PDOS__SETR8A('RAD',NSP,RAD)
      CALL PDOS__SETR8A('PHI',LNXX*NSP,VAL)
      CALL PDOS__SETR8A('DPHIDR',LNXX*NSP,DER)
      CALL PDOS__SETR8A('OVERLAP',LNXX*LNXX*NSP,OV)
      DEALLOCATE(VAL)
      DEALLOCATE(IZ)
      DEALLOCATE(RAD)
      DEALLOCATE(DER)
...
!     ==================================================================
      ...
      ALLOCATE(XK(3,NKPT))
      CALL DYNOCC__GETR8A('XK',3*NKPT,XK)
      CALL PDOS__SETR8A('XK',3*NKPT,XK)
...
!
!     ..................................................................
      SUBROUTINE WAVES__REPORTEIG(NFIL)
!     ******************************************************************
!     **  WAVES__GET                                                   **
!     **  GET                                                         **
!     ******************************************************************
...
!     ******************************************************************
      ...
      DO IKPT=1,NKPT
        DO ISPIN=1,NSPIN
          ...
          IF(.NOT.ASSOCIATED(THIS%EIGVAL)) THEN
            CALL ERROR__MSG('EIGENVALUES NOT PRESENT')
            CALL ERROR__STOP('WAVES__REPORTEIG')
          END IF
          NB=THIS%NB
          IF(NSPIN.EQ.1) THEN
     ...
     &                        //'," AND SPIN ",I1)')IKPT,ISPIN
          END IF
          CALL REPORT__TITLE(NFIL,STRING)
          ITEN=0
          DO WHILE (NB.GT.ITEN)
...
!
!     ..................................................................
      SUBROUTINE WAVES__WRITE(NFIL,NFILO,TCHK)
!     ******************************************************************
!     **                                                              **
...
!
!     ..................................................................
      SUBROUTINE WAVES__READ(NFIL,NFILO,TCHK)
!     ******************************************************************
!     **                                                              **
!     **  WRITE WAVE FUNCTIONS TO RESTART FILE                        **
!     **                                                              **
...
!     ==================================================================
      ...
      DO ISPIN=1,NSPIN
        ...
        ENDDO
      ENDDO
      CALL DYNOCC__SETR8A('EPSILON',NBX*NKPT*NSPIN,EIG)
      DEALLOCATE(EIG)
!
!     ==================================================================
!     ==  CLOSE DOWN                                                  ==
...
!     ******************************************************************
      ...
      INTEGER(4)              :: NWAVE=2
!     ******************************************************************
              CALL TRACE__PUSH('WAVES_WRITEPSI')
      CALL MPE__QUERY(NTASKS,THISTASK)
      ...
      ELSE IF(RSTRTTYPE.EQ.'DYNAMIC') THEN
        NWAVE=2
      ELSE
        CALL ERROR__STOP('WAVES_WRITEPSI')
      END IF
!
!     ==================================================================
!     ==  COUNT #(RECORDS)                                            ==
...
!     ==================================================================
      ...
      DO IKPT=1,NKPT
        ...
        NBH=THIS%NBH
        NB=THIS%NB
        CALL PLANEWAVE__GETI4('NGG',NGG)
!
!       ================================================================
!       == WRITE SIZE AND TYPE OF WAVE FUNCTION                       ==
!       ================================================================
        CALL PLANEWAVE__GETL4('TINV',TSUPER)
        IF(THISTASK.EQ.1) THEN
          KEY='PSI'
...
!       ================================================================
        CALL PLANEWAVE__GETR8A('GBAS',9,GBAS)
        CALL PLANEWAVE__GETR8A('XK',3,XK)
        ALLOCATE(IGVECL(3,NGL))
        CALL PLANEWAVE__GETI4A('IGVEC',3*NGL,IGVECL)
        ALLOCATE(IGVECG(3,NGG))
        CALL PLANEWAVE__COLLECTI4(3,NGL,IGVECL,NGG,IGVECG)
        IF(THISTASK.EQ.1) THEN
        ...
        DO IWAVE=1,NWAVE
          DO ISPIN=1,NSPIN
            ...
            DO IB=1,NBH
              ...
              ENDDO
              DO IDIM=1,NDIM
                CALL PLANEWAVE__COLLECTC8(1,NGL,PSI1(1,IDIM),NGG,PSIG(1,IDIM))
              ENDDO
              IF(THISTASK.EQ.1) THEN
...
!     ==================================================================
      IF(THISTASK.EQ.1.AND.NREC1.NE.NREC) THEN
        CALL ERROR__MSG('#(RECORDS WRITTEN DIFFERENT FROM TARGET')
        CALL ERROR__I4VAL('TARGET',NREC)
        CALL ERROR__I4VAL('ACTUAL',NREC1)
        CALL ERROR__STOP('WAVES_WRITEPSI')
      END IF
              CALL TRACE__POP
      RETURN
 9999 CONTINUE
      CALL FILEHANDLER__IOSTATMESSAGE(IOS,IOSTATMSG)
      CALL ERROR__MSG('ERROR WRITING WAVE FUNCTION TO RESTART FILE')
      CALL ERROR__I4VAL('IOS',IOS)
      CALL ERROR__CHVAL('IOSTATMSG',IOSTATMSG)
      CALL ERROR__I4VAL('IB',IB)
      CALL ERROR__I4VAL('IKPT',IKPT)
      CALL ERROR__I4VAL('ISPIN',ISPIN)
      CALL ERROR__I4VAL('NGG',NGG)
      CALL ERROR__STOP('WAVES_WRITEPSI')
      RETURN
      END
!
...
!     ******************************************************************
                               CALL TRACE__PUSH('WAVES_READPSI')
      CALL MPE__QUERY(NTASKS,THISTASK)
!
!     ==================================================================
!     ==  READ SIZES                                                  ==
...
! 100    CONTINUE
 ...
 100    CONTINUE
        IF(IFORMAT.EQ.0) THEN
          CALL ERROR__MSG('FORMAT NOT RECOGNIZED')
          CALL ERROR__STOP('WAVES_READPSI')
        END IF
      END IF
      CALL MPE__BROADCAST(1,NSPIN_)
      CALL MPE__BROADCAST(1,NKPT_)
      CALL MPE__BROADCAST(1,IFORMAT)
      CALL MPE__BROADCAST(1,NWAVE)
      IF(IFORMAT.EQ.2) THEN
        CALL MPE__BROADCAST(1,RBAS)
        DO IKPT=1,NKPT
          CALL WAVES_SELECTWV(IKPT,1)
          CALL PLANEWAVE__SELECT(GSET%ID)
          CALL PLANEWAVE__SETR8A('RBAS',9,RBAS)
...
!       ================================================================
        ...
        END IF
        CALL MPE__BROADCAST(1,KEY)
        CALL MPE__BROADCAST(1,NGG_)
        CALL MPE__BROADCAST(1,NDIM_)
        CALL MPE__BROADCAST(1,NB_)
        CALL MPE__BROADCAST(1,TSUPER_)
        IF(KEY.NE.'PSI') THEN
          CALL ERROR__MSG('ID IS NOT "PSI"')
          CALL ERROR__MSG('FILE IS CORRUPTED')
          CALL ERROR__I4VAL('IKPT_',IKPT_)
          CALL ERROR__STOP('WAVES_READPSI')
        END IF
        ...
        IF(THISTASK.EQ.1) THEN
          ...
          ELSE
            ...
            IF(TSUPER_) THEN
              ...
              CALL PLANEWAVE__MINUSG(K_,NGG_,IGVECG_,MINUSG)
            END IF
            DEALLOCATE(IGVECG_)
          END IF
        END IF
        CALL MPE__BROADCAST(1,K_)
        CALL MPE__BROADCAST(1,GVECG_)
!
        IF(IFORMAT.EQ.1.AND.(.NOT.GBASFIX)) THEN
          GBASFIX=.TRUE.
          CALL MPE__BROADCAST(1,GBAS_)
...
!       ==============================================================
        ...
        CALL PLANEWAVE__GETR8A('GVEC',3*NGL,GVECL)
        CALL PLANEWAVE__COLLECTR8(3,NGL,GVECL,NGG,GVECG)
        DEALLOCATE(GVECL)
        ALLOCATE(MAPG(NGG))
        IF(THISTASK.EQ.1) THEN
          CALL WAVES_MAPG(NGG_,GBAS_,GVECG_,NGG,GVECG,MAPG)
        END IF
        CALL MPE__BROADCAST(1,MAPG)
!!__DO IG=1,NGG
!!__  IF(MAPG(IG).NE.IG) THEN
!!__     PRINT*,'MAPG ',IKPT_,IG,MAPG(IG),NGG,NGG_
!!__  END IF
!!__ENDDO
        DEALLOCATE(GVECG_)
        DEALLOCATE(GVECG)
              CALL TRACE__PASS('MARKE 7')
!
!       ==============================================================
!       ==  COLLECT DATA                                            ==
!       ==============================================================
IF(TSUPER_.AND.NDIM_.EQ.2) THEN
  CALL ERROR__MSG('NONCOLLINEAR WAVE FUNCTIONS AND SUPER WAVE FUNCTIONS')
  CALL ERROR__MSG('ARE NOT PROPERLY IMPLEMENTED BELOW')
  CALL ERROR__STOP('WAVES_READPSI')
END IF
        NBH=THIS%NBH
        NB=THIS%NB
        ...
        DO IWAVE=1,NWAVE
          DO ISPIN=1,NSPIN_
            ...
            DO IB=1,NB_
              ...
              END IF
              DO IDIM=1,NDIM_
                CALL PLANEWAVE__DISTRIBUTEC8(1,NGG,PSIG(1,IDIM),NGL,PSIL(1,IDIM,IB,ISPIN))
              ENDDO
            ENDDO
          ENDDO
!
...
!         == FROM NON-SPIN POLARIZED CALCULATION =========================
          ELSE IF(NSPIN_.EQ.1.AND.NDIM_.EQ.1) THEN
            ...
            ELSE IF(NDIM.EQ.2) THEN
              ...
              ENDDO
            ELSE
              CALL ERROR__MSG('TRANSFORMATION NOT IMPLEMENTED')
              CALL ERROR__STOP('WAVES__READPSI')
            END IF
...
!             == remove the factor exp(i*phi) from the wave function
              do ib=1,nb
                call PLANEWAVE__SCALARPRODUCT('-',NGL,1,1 &
...
!     ==  COMPLETE K-POINTS                                           ==
!     ==================================================================
      CALL TRACE__PASS('MARKE 14')
      DO IKPT=1,NKPT
        IF(TREAD(IKPT)) CYCLE
        CALL WAVES_SELECTWV(IKPT,1)
        CALL PLANEWAVE__SELECT(GSET%ID)
        CALL PLANEWAVE__GETR8A('K',3,K)
        SVAR=1.D+10
        DO IKPT_=1,NKPT_
...
!     ==================================================================
      ...
      RETURN
 9999 CONTINUE
      CALL ERROR__MSG('ERROR WHILE READING FROM FILE')
      CALL ERROR__I4VAL('IOS',IOS)
      CALL ERROR__STOP('WAVES_READPSI')
      END
!
...
!     ******************************************************************
      ...
      COMPLEX(8)   ,ALLOCATABLE:: LAMBDA2(:,:,:)
!     ******************************************************************
                           CALL TRACE__PUSH('WAVES_READLAMBDA')
      CALL MPE__QUERY(NTASKS,THISTASK)
!
!     ==================================================================
...
!     ==================================================================
      IF(THISTASK.EQ.1) THEN
        ...
        IF(KEY.NE.'LAMBDA') THEN
          CALL ERROR__MSG('ID IS NOT "LAMBDA"')
          CALL ERROR__MSG('FILE IS CORRUPTED')
          CALL ERROR__STOP('WAVES_READLAMBDA')
        END IF
      END IF
      IF(TSKIP) THEN
        DO I=1,NLAMBDA
...
!           == FROM NON-SPIN POLARIZED CALCULATION =====================
            IF(NDIM_.EQ.1.AND.NSPIN_.EQ.1) THEN
              ...
              ELSE IF(NDIM.EQ.2.AND.NSPIN.EQ.1) THEN
                ...
                ENDDO
              ELSE
                CALL ERROR__MSG('INVALID OPTION TRANSFORMING FROM')
                CALL ERROR__MSG('NON-SPINPOLARIZED DATA')
                CALL ERROR__I4VAL('NDIM',NDIM)
                CALL ERROR__I4VAL('NSPIN',NSPIN)
                CALL ERROR__I4VAL('NDIM_',NDIM_)
                CALL ERROR__I4VAL('NSPIN_',NSPIN_)
                CALL ERROR__STOP('WAVES_READLAMBDA')
              END IF
!           == FROM SPIN POLARIZED CALCULATION =========================
            ELSE IF(NDIM_.EQ.1.AND.NSPIN_.EQ.2) THEN
              ...
              ELSE
                CALL ERROR__MSG('INVALID OPTION TRANSFORMING FROM')
                CALL ERROR__MSG('SPINPOLARIZED DATA')
                CALL ERROR__I4VAL('NDIM',NDIM)
                CALL ERROR__I4VAL('NSPIN',NSPIN)
                CALL ERROR__I4VAL('NDIM_',NDIM_)
                CALL ERROR__I4VAL('NSPIN_',NSPIN_)
...
!           == FROM NONCOLLINEAR WAVE FUNCTION =========================
            ELSE IF(NDIM_.EQ.2.AND.NSPIN_.EQ.1) THEN
              ...
              ELSE
                CALL ERROR__MSG('INVALID OPTION TRANSFORMING FROM')
                CALL ERROR__MSG('NON-COLLINEAR DATA')
                CALL ERROR__I4VAL('NDIM',NDIM)
                CALL ERROR__I4VAL('NSPIN',NSPIN)
                CALL ERROR__I4VAL('NDIM_',NDIM_)
                CALL ERROR__I4VAL('NSPIN_',NSPIN_)
              ...
              END IF
            ELSE
              CALL ERROR__MSG('INVALID OPTION')
              CALL ERROR__I4VAL('NDIM',NDIM)
              CALL ERROR__I4VAL('NSPIN',NSPIN)
              CALL ERROR__I4VAL('NDIM_',NDIM_)
              CALL ERROR__I4VAL('NSPIN_',NSPIN_)
...
!     ==================================================================
      DO ISPIN=1,NSPIN
        ...
        IF(NLAMBDA.GE.1)THEN
          IF(.NOT.ASSOCIATED(THIS%RLAM0))ALLOCATE(THIS%RLAM0(NB,NB))
          CALL MPE__BROADCAST(1,THIS%RLAM0)
        END IF
        IF(NLAMBDA.GE.2)THEN
          IF(.NOT.ASSOCIATED(THIS%RLAMM))ALLOCATE(THIS%RLAMM(NB,NB))
          CALL MPE__BROADCAST(1,THIS%RLAMM)
        END IF
        IF(NLAMBDA.GE.3)THEN
          IF(.NOT.ASSOCIATED(THIS%RLAM2M))ALLOCATE(THIS%RLAM2M(NB,NB))
          CALL MPE__BROADCAST(1,THIS%RLAM2M)
        END IF
        IF(NLAMBDA.GE.4)THEN
          IF(.NOT.ASSOCIATED(THIS%RLAM3M))ALLOCATE(THIS%RLAM3M(NB,NB))
          CALL MPE__BROADCAST(1,THIS%RLAM3M)
        END IF
      ENDDO
                           CALL TRACE__POP
      RETURN
...
!     ******************************************************************
      CALL LIB__INVERTR8(3,GBASA,GBASIN)
!
!     ==================================================================
!     == DETERMINE K-POINT                                            ==
!     ==================================================================
...
!     ==================================================================
      ...
      DO IG=1,NGA
        ...
        ELSE
          CALL ERROR__MSG('TWO G-VECTORS ARE MAPPED ONTO THE SAME POINT')
          CALL ERROR__STOP('WAVES_MAPG')
        END IF
      ENDDO
!
!     ==================================================================
...
!     == GET GLOBAL GVECTORS FROM IKPT1 ================================
      ...
      CALL PLANEWAVE__GETR8A('GVEC',3*NGL1,GVECL)
      CALL PLANEWAVE__GETI4('NGG',NGG1)
      IF(THISTASK.NE.1) NGG1=1
      ALLOCATE(GVECG1(3,NGG1))
      CALL PLANEWAVE__COLLECTR8(3,NGL1,GVECL,NGG1,GVECG1)
      DEALLOCATE(GVECL)
!
!     == GET GLOBAL GVECTORS FROM IKPT2 ================================
      ...
      CALL PLANEWAVE__GETR8A('GVEC',3*NGL2,GVECL)
      CALL PLANEWAVE__GETI4('NGG',NGG2)
      IF(THISTASK.NE.1) NGG2=1
      ALLOCATE(GVECG2(3,NGG2))
      CALL PLANEWAVE__COLLECTR8(3,NGL2,GVECL,NGG2,GVECG2)
      DEALLOCATE(GVECL)
!
!     == COPY GBAS      ================================================
      CALL WAVES_SELECTWV(IKPT2,1)
      ...
      CALL WAVES_SELECTWV(IKPT2,1)
      CALL PLANEWAVE__SELECT(GSET%ID)
      CALL PLANEWAVE__SETR8A('GBAS',9,GBAS)
!
!     == DEFINE MAPPING ================================================
      IF(THISTASK.EQ.1) THEN
        ALLOCATE(MAPG(NGG1))
...
!     ==================================================================
      CALL WAVES_SELECTWV(IKPT1,1)
      CALL PLANEWAVE__SELECT(GSET%ID)
      CALL PLANEWAVE__GETL4('TINV',TSUPER1)
      NB1=THIS%NB
      NBH1=THIS%NBH
      CALL WAVES_SELECTWV(IKPT2,1)
      CALL PLANEWAVE__SELECT(GSET%ID)
      CALL PLANEWAVE__GETL4('TINV',TSUPER2)
      NB2=THIS%NB
      NBH2=THIS%NBH
          ...
          IF(TSUPER2) THEN
            DO IB=NBH2,1,-1
              ...
              DO IDIM=1,NDIM
                ...
                CALL PLANEWAVE__INVERTG(NGL2,PSI2(1,IDIM,IB),PSITMP2(1,2))
...
!         ==============================================================
          DO IB=1,NBN
            DO IDIM=1,NDIM
              CALL WAVES_SELECTWV(IKPT2,ISPIN)
              CALL PLANEWAVE__SELECT(GSET%ID)
              CALL PLANEWAVE__COLLECTC8(1,NGL2,PSI2(1,IDIM,IB),NGG2,PSIG2)
              IF(THISTASK.EQ.1) THEN
                DO IG=1,NGG1
                  IF(MAPG(IG).NE.0) THEN
              ...
              CALL PLANEWAVE__SELECT(GSET%ID)
              CALL PLANEWAVE__DISTRIBUTEC8(1,NGG1,PSIG1,NGL1,PSI1(1,IDIM,IB))
            ENDDO
          ENDDO
!
!         ==============================================================
...
!      *****************************************************************
       ...
       IF (NDIM.EQ.1.AND.NSPIN.EQ.1) THEN
          CALL ERROR__MSG('S^2 ONLY POSSIBLE FOR NOT SPIN RESTRICTED CALCULATION')
          CALL ERROR__I4VAL('NDIM',NDIM)
          CALL ERROR__I4VAL('NSPIN',NSPIN)
          CALL ERROR__STOP('WAVES_SPINOROVERLAP')
       END IF
...
!
!      .................................................................
       subroutine waves__fixrhoread()
       use wavesfixrho_module
       implicit none
       integer(4)                 :: nfil
!      ******************************************************************
       CALL FILEHANDLER__SETFILE('FIXRHO',.FALSE.,'fixrho.bin')
       CALL FILEHANDLER__SETSPECIFICATION('FIXRHO','FORM','UNFORMATTED')
       CALL FILEHANDLER__SETSPECIFICATION('FIXRHO','POSITION','REWIND')
       CALL FILEHANDLER__UNIT('FIXRHO',NFIL)
       read(nfil)qlm(:,:)
       read(nfil)rho(:,:)
       read(nfil)denmat(:,:,:,:)
       call filehandler__close('FIXRHO')
       return
       end
!
!      .................................................................
       subroutine waves__fixrhowrite()
       use wavesfixrho_module
       implicit none
       integer(4)                 :: nfil
!      ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE WAVES__REPORTSPIN(NFIL)
!     ******************************************************************
!     **  WAVES__GET                                                   **
!     **  GET                                                         **
!     ******************************************************************
...
!     ******************************************************************
      IF (NSPIN.EQ.1.AND.NDIM.EQ.1) THEN
         RETURN
      ELSE
         CALL REPORT__TITLE(NFIL,'TOTAL SPIN ANALYSIS')
         IF (NDIM.EQ.2) CALL REPORT__R8VAL(NFIL,'<S_X>',TOTSPIN(2),'HBAR')
         IF (NDIM.EQ.2) CALL REPORT__R8VAL(NFIL,'<S_Y>',TOTSPIN(3),'HBAR')
         CALL REPORT__R8VAL(NFIL,'<S_Z>',TOTSPIN(4),'HBAR')
         SVAR=DSQRT(TOTSPIN(2)**2+TOTSPIN(3)**2+TOTSPIN(4)**2)
         IF (NDIM.EQ.2) CALL REPORT__R8VAL(NFIL,'|S|',SVAR,'HBAR')
!        CALL REPORT__R8VAL(NFIL,'TOTAL SPIN <S^2>',TOTSPIN(1),'HBAR^2')
!        SVAR=-0.5+SQRT(0.25+TOTSPIN(1))
!        CALL REPORT__R8VAL(NFIL,'SPIN QUANTUM NUMBER S',SVAR,'')
         RETURN
      END IF
    END SUBROUTINE WAVES__REPORTSPIN
!
!     ....................................................................
