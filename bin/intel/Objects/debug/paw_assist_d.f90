...
!       ...............................................................
        ...
        CALL ERROR__STOP('DIALS_CHECKOUT')
...
!       ...............................................................
        ...
        CALL ERROR__MSG('DIAL NAME NOT RECOGNIZED')
        CALL ERROR__CHVAL('NAME',NAME)
        CALL ERROR__STOP('DIALS_CHECKIN')
...
!
!     ...................................................AUTOPI.........
      SUBROUTINE DIALS__SETR8(ID_,VAL_)
!     ******************************************************************
!     **  REPORT SETTING OF AUTOPILOT                                 **
!     ******************************************************************
      USE DIALS_MODULE
...
!
      ...
      IF(ISELECT.LE.0) THEN
        CALL ERROR__MSG('A DIAL MUST BE SELECTED')
        CALL ERROR__STOP('DIALS__SETR8')
      END IF
...
!
!     ...................................................AUTOPI.........
      SUBROUTINE DIALS__SETL4(ID_,VAL_)
!     ******************************************************************
!     **  REPORT SETTING OF AUTOPILOT                                 **
!     ******************************************************************
      USE DIALS_MODULE
...
!     ******************************************************************
      ...
      DO IDIAL=1,NDIALS
        ...
        CALL REPORT__TITLE(NFIL,'DIAL :'//TRIM(DIAL(IDIAL)%NAME))
        CALL REPORT__STRING(NFIL,TRIM(DIAL(IDIAL)%DESCRIPTION)//':')
        CALL REPORT__R8VAL(NFIL,'RATE',DIAL(IDIAL)%RATE,'A.U.')
        CALL REPORT__R8VAL(NFIL,'FINAL VALUE',DIAL(IDIAL)%FINAL,'A.U.')
      ENDDO
      RETURN
      END
!
!     ...................................................AUTOPI.........
      SUBROUTINE DIALS__APPLY
!     ******************************************************************
!     **  REPORT SETTING OF AUTOPILOT                                 **
!     ******************************************************************
      USE DIALS_MODULE
...
!     ==  TEMPERATURE OF THE ELECTRONS ================================
        ...
        CALL DYNOCC__GETR8('TEMP',VALUE)
        DIAL1%ACTUAL=VALUE
        CALL DIALS_CHECKIN(NAME,DIAL1)
        NAME='SPIN'
        CALL DIALS_CHECKOUT(NAME,DIAL1)
        CALL DYNOCC__GETR8('SPIN',VALUE)
        DIAL1%ACTUAL=VALUE
        CALL DIALS_CHECKIN(NAME,DIAL1)
        NAME='CHARGE'
        CALL DIALS_CHECKOUT(NAME,DIAL1)
        CALL DYNOCC__GETR8('TOTCHA',VALUE)
        DIAL1%ACTUAL=VALUE
        CALL DIALS_CHECKIN(NAME,DIAL1)
        TSET=.TRUE.
        RETURN
...
!     =================================================================
      ...
      IF(DIAL1%ON) THEN
        ...
        VALUE=DIAL1%ACTUAL
        CALL DIALS_CHECKIN(NAME,DIAL1)
        CALL DYNOCC__SETR8('TOTCHA',VALUE)
      END IF
!
!     =================================================================
!     ==  NEXT                                                       ==
...
!     ******************************************************************
      ...
      IF(.NOT.TAUTO) RETURN
      CALL REPORT__TITLE(NFIL,'AUTOMATIC MINIMIZER')
      DO I=1,NAUTO
     ...
     &    .AND.DABS(CONTROL(I)%UPPERFACTOR-1.D0).GT.DSMALL) THEN
          CALL REPORT__R8VAL(NFIL,'CONSTANT FRICTION FOR '//TRIM(CONTROL(I)%NAME) &
                                    ,CONTROL(I)%LOWERFRICTION,' ')
        ELSE
          CALL REPORT__R8VAL(NFIL,'LOWER FRICTION FOR '//TRIM(CONTROL(I)%NAME) &
                                    ,CONTROL(I)%LOWERFRICTION,' ')
          IF(DABS(CONTROL(I)%LOWERFACTOR-1.D0).GT.DSMALL) THEN
            CALL REPORT__R8VAL(NFIL,'SCALING FACTOR OF LOWER FRICTION FOR ' &
   &                    //TRIM(CONTROL(I)%NAME),CONTROL(I)%LOWERFACTOR,' ')
          END IF
          CALL REPORT__R8VAL(NFIL,'UPPER FRICTION FOR '//TRIM(CONTROL(I)%NAME) &
                                    ,CONTROL(I)%UPPERFRICTION,' ')
          IF(DABS(CONTROL(I)%UPPERFACTOR-1.D0).GT.DSMALL) THEN
            CALL REPORT__R8VAL(NFIL,'SCALING FACTOR OF UPPER FRICTION FOR '//TRIM(CONTROL(I)%NAME) &
                                    ,CONTROL(I)%UPPERFACTOR,' ')
          END IF
        END IF
      ENDDO
      CALL REPORT__R8VAL(NFIL,'ENERGY TOLERANCE FOR TERMINATION',ETOL,' ')
      CALL REPORT__I4VAL(NFIL,'#STEPS BEFORE TERMINATION IS CONSIDERED',NTOL,' ')
      CALL REPORT__STRING(NFIL,'      FRICTION=0: NEWTON DYNAMICS')
      CALL REPORT__STRING(NFIL,'      FRICTION=1: STEEPEST DESCENT')
      RETURN
      END
!
!     ...................................................AUTOPI.........
...
!
        ...
        ELSE IF(CONTROL(I)%NAME.EQ.'CONTINUUM') THEN
          CALL CONTINUUM__SETR8('FRICTION',FRICTION)
        END IF
      ENDDO
      EPREV=ETOT
      RETURN
...
!     ******************************************************************
      ...
      DO I=1,NAUTO
        ...
        END IF
      ENDDO
      CALL ERROR__MSG('KEYWORD NOT RECOGNIZED')
      CALL ERROR__CHVAL('ID_',ID_)
      CALL ERROR__STOP('AUTO__SELECT')
      END
!
!     ..................................................................
      SUBROUTINE AUTO__SETL4(IDENT_,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE AUTOPILOT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      IF(ISELECT.EQ.0) THEN
        CALL ERROR__MSG('CONTROL MUST BE SELECTED')
        CALL ERROR__STOP('AUTO__SETL4')
      END IF
...
!
!     ..................................................................
      SUBROUTINE AUTO__SETi4(IDENT_,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE AUTOPILOT_MODULE
      IMPLICIT NONE
...
!
!     ..................................................................
      SUBROUTINE AUTO__SETR8(IDENT_,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE AUTOPILOT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('KEYWORD NOT RECOGNIZED')
        CALL ERROR__CHVAL('IDENT_',IDENT_)
        CALL ERROR__STOP('AUTO__SETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE AUTO__GETL4(IDENT_,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE AUTOPILOT_MODULE
      IMPLICIT NONE
