...
!     ******************************************************************
      ...
      IF(DET.EQ.0.D0) THEN
        CALL ERROR__MSG('LATTICE VECTORS LINEAR DEPENDEND')
        PRINT*,'RBAS=',RBAS
        CALL ERROR__STOP('GBASS')
      END IF
      FAC = 2.D0*PI/DET
      DO I=1,3
        DO J=1,3
...
!     ==================================================================
      ...
      IF(DET.EQ.0.D0) THEN
        CALL ERROR__MSG('DISPLACEMENT VECTORS LINAR DEPENDEND')
        CALL ERROR__STOP('BOXSPH')
      END IF
      DO I=1,3
        G1(I)=G1(I)/DET
        G2(I)=G2(I)/DET
...
!     ==================================================================
      call lib__invertr8(3,rbas,rbasin)
!      DO I=1,3
!        DO J=1,3
!          RBASIN(I,J)=RBAS(I,J)
!        ENDDO
...
!      *****************************************************************
       IF(STRING.NE.'T'.AND.STRING.NE.'R'.AND.STRING.NE.'TR') THEN
         CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED IN NOMOM')
         CALL ERROR__STOP('NOMOM')
       END IF
!
!      =================================================================
...
!
         ...
         ENDDO
       ENDDO
       CALL ERROR__MSG('ITERATION IN NOMOM NOT CONVERGED')
       CALL ERROR__R8VAL('WTOT',WTOT)
       CALL ERROR__STOP('NOMOM')
2000   CONTINUE
       IF(STRING.EQ.'R') THEN
         DO IAT=1,NAT
...
!
!     ..................................................................
      SUBROUTINE SORT__SET(LEN_,CRIT)
!     ******************************************************************
!     ******************************************************************
      USE SORT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      IF(TSET) THEN
        CALL ERROR__MSG('SORT OBJECT ALREADY IN USE')
        CALL ERROR__STOP('SORT__SET')
      END IF
      TSET=.TRUE.
      LEN=LEN_
      ALLOCATE(IND(LEN))
...
!     ******************************************************************
      IF(.NOT.TSET) THEN
        CALL ERROR__MSG('SORT OBJECT NOT INITIALIZED')
        CALL ERROR__STOP('SORT__RESTART')
      END IF
      CALL SORTRANK(LEN,IND,RANK)
      I0=1
      IHOLE=0
...
!
!     ..................................................................
      SUBROUTINE SORT__ORDERC16(LEN,NB,ARRAY)
!     ******************************************************************
!     ******************************************************************
      IMPLICIT NONE
...
!
!     ..................................................................
      SUBROUTINE SORT__ORDERR8(LEN,NB,ARRAY)
!     ******************************************************************
!     ******************************************************************
      IMPLICIT NONE
      INTEGER(4),INTENT(IN)    :: NB
...
!
!     ..................................................................
      SUBROUTINE SORT__ORDERI4(LEN,NB,ARRAY)
!     ******************************************************************
!     ******************************************************************
      INTEGER(4),INTENT(IN)    :: NB
      INTEGER(4),INTENT(IN)    :: LEN
...
!     ==================================================================
      CALL MPE__QUERY(NTASKNUM,ITASK)
!
!     ==================================================================
...
!     ==================================================================
      ...
      DO K=1,10
        ...
        DO I=1,10000
          X=0.1D0*DBLE(I)
          CALL LIB__ERFCR8(X,ERFCX)   !ERFC=1-ERF
          Y=PI/2.D0*ERFCX
!         PRINT*,'R: X=',X,' Y=',Y
          IF(Y.LT.SVAR) THEN
            C1=2.D0*X
...
!         __ SELECTION FOR PARALLEL PROCESSING__________________________
     ...
     &                     .AND.IT1.EQ.0.AND.IT2.EQ.0.AND.IT3.EQ.0) THEN
                      ...
                      RFAC2=0.D0
                    ELSE
                      CALL LIB__ERFCr8(DLEN*FAC,ERFCX)
                      RFAC1=ERFCX/DLEN
...
!         __ END SELECTION FOR PARALLEL PROCESSING______________________
        ...
        ENDDO
      ENDDO
      CALL MPE__COMBINE('+',FMAD)
      CALL MPE__COMBINE('+',VMAD)
