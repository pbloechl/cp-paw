...
!     ******************************************************************
      ...
      CALL ERROR__CHVAL('ID ON INPUT   ',ID_)
      DO I=1,NE
        CALL ERROR__CHVAL('POSSIBLE ID ',ID(I))
      ENDDO
      CALL ERROR__STOP('AUGMENTATION_ADD')
      STOP
      END
!
!     ..................................................................
      SUBROUTINE AUGMENTATION__SYNC
!     ******************************************************************
!     **  SYNCHRONIZES THE TEMPORARY STACK WITH THE ENERGYLIST        **
!     **  AND RESETS THE TEMPORARY STACK                              **
!     **                                                              **
!     **  MUST BE CALLED AFTER AUGMENTATION__SPHERE HAS BEEN           **
...
!     ******************************************************************
      TINI=.FALSE.
      CALL MPE__COMBINE('+',VAL)
      DO I=1,NE
        CALL ENERGYLIST__SET(ID(I),VAL(I))
      ENDDO
!     ==  ID(1)='AE1 EXCHANGE-CORRELATION' =============================
!     ==  ID(2)='PS1 EXCHANGE-CORRELATION' =============================
!     ==  ID(3)='AE1 ELECTROSTATIC' ====================================
...
!     ==  ID(8)='LDA+U EXCHANGE' =======================================
      ...
      CALL ENERGYLIST__ADD('AE  KINETIC',VAL(7))
      CALL ENERGYLIST__ADD('AE  EXCHANGE-CORRELATION',VAL(1)-VAL(2)+VAL(8))
      CALL ENERGYLIST__ADD('AE  ELECTROSTATIC',VAL(3)-VAL(4))
      CALL ENERGYLIST__ADD('BACKGROUND',VAL(5)-VAL(6))
      VAL(:)=0.D0
      RETURN
      END
!
!     .....................................................MOMNTS.......
      SUBROUTINE AUGMENTATION__MOMENTS(ISP,LMNX,DENMAT,LMRX,QLM)
!     ******************************************************************
!     **                                                             **
...
!     ******************************************************************
                          CALL TRACE__PUSH('AUGMENTATION__MOMENTS')
!
...
!     ==================================================================
      ALLOCATE(PSCORE(NR))
      ALLOCATE(AECORE(NR))
      CALL SETUP__PSCORE(ISP,NR,PSCORE)
      CALL SETUP__AECORE(ISP,NR,AECORE)
      CALL SETUP__AEZ(ISP,AEZ)
!
!     ==================================================================
...
!
!     ..................................................................
      SUBROUTINE AUGMENTATION__SPHERE(ISP,IAT,LMNX,NDIMD,DENMAT,denmati &
...
!     ******************************************************************
      ...
      real(8)                 :: pi
!     ******************************************************************
                            CALL TRACE__PUSH('AUGMENTATION__SPHERE')
      pi=4.d0*datan(1.d0)
!
!     ==================================================================
...
!     ==================================================================
      ...
      ALLOCATE(PSCORE(NR))
      CALL SETUP__PSCORE(ISP,NR,PSCORE)
      CALL SETUP__RCSM(ISP,RCSM)
      ALLOCATE(VADD(NR))
      CALL SETUP__VBAR(ISP,nr,VADD)
      ALLOCATE(DOVER(LNX,LNX))
      CALL SETUP__1COVERLAP(ISP,LNX,DOVER)
      ALLOCATE(DTKIN(LNX,LNX))
      CALL SETUP__1CKINETIC(ISP,LNX,DTKIN)
!
!     ==================================================================
!     ==  SELF TEST                                                   ==
!     ==================================================================
 1000 CONTINUE
      IF(TTEST) THEN
        IF(ITEST.EQ.1.OR.ITEST.EQ.3) THEN
          CALL SELFTEST__START('SPHERE',LMNX*LMNX*NDIMD,DENMAT,1.D-3)
          VQLM(:)=0.D0
          DO IDIM=1,NDIMD
...
! without pseudo core!
      ...
      CALL HYPERFINE__SET1CRHO('PS','TOT',IAT,R1,DEX,NR,NR,LMRX,PSRHO)
      AEPOT(:,:,1)=AERHO(:,:,1)
      AEPOT(:,1,1)=AEPOT(:,1,1)+AECORE(:)
      CALL HYPERFINE__SET1CRHO('AE','TOT',IAT,R1,DEX,NR,NR,LMRX,AEPOT)
      IF(NDIMD.GT.1) THEN
        ...
        END IF
        CALL HYPERFINE__SET1CRHO('PS','SPIN' &
     &                         ,IAT,R1,DEX,NR,NR,LMRX,PSPOT)
        CALL HYPERFINE__SET1CRHO('AE','SPIN' &
...
!     =================================================================
      ...
      ENDDO
      CALL RADIAL__INTEGRAL(R1,DEX,NR,DWORK1,POTB)
      POTB=-POTB
!peb03 potb=0.d0
      DEALLOCATE(DWORK1)
!
...
!     == ANALYSIS: ELECTRIC FIELD GRADIENTS ==========================
      CALL HYPERFINE__SET1CPOT('AE',IAT,R1,DEX,NR,NR,LMRX,AEPOT1)
      CALL HYPERFINE__SET1CPOT('PS',IAT,R1,DEX,NR,NR,LMRX,PSPOT1)
!
!     == ANALYSIS: POTENTIAL PLOT           ==========================
      CALL GRAPHICS__SET1CPOT('AE',IAT,R1,DEX,NR,NR,LMRX,AEPOT1)
      CALL GRAPHICS__SET1CPOT('PS',IAT,R1,DEX,NR,NR,LMRX,PSPOT1)
!
...
!     ==================================================================
 1001 CONTINUE
      IF(TTEST) THEN
        IF(ITEST.EQ.2) THEN
          CALL SELFTEST__START('SPHERE-EKI',LMNX*LMNX*NDIMD,DENMAT,1.D-3)
...
!     ==================================================================
      ...
      IF(TTEST) THEN
        IF(ITEST.EQ.2) THEN
          CALL SELFTEST__END('SPHERE-EKI',LMNX*LMNX*NDIMD,DATH,EKINNL,TBACK)
          IF(TBACK) GOTO 1001
          CALL ERROR__MSG('STOP AFTER SELFTEST')
          CALL ERROR__STOP('SPHERE')
        END IF
      END IF
!
...
!     ================================================================
      CALL ATOMLIST__GETCH('NAME',IAT,ATOM)
      ALLOCATE(DATP(LMNX,LMNX,Ndimd))
      CALL EXTERNAL1CPOT__APPLY(ATOM,LMNX,NDIMD,DENMAT,DATP,DETOT)
      DATH(:,:,:)=DATH(:,:,:)+DATP(:,:,:)
      DEALLOCATE(DATP)
      CALL AUGMENTATION_ADD('LDA+U EXCHANGE',DETOT)
!
...
!     ================================================================
      IF(TTEST) THEN
        IF(ITEST.EQ.1.OR.ITEST.EQ.3) THEN
          ...
          CALL SELFTEST__END('SPHERE',LMNX*LMNX*NDIMD,dath,enl,TBACK)
          IF(TBACK) GOTO 1000
          CALL ERROR__MSG('STOP AFTER SELFTEST')
          CALL ERROR__STOP('SPHERE')
        ENDIF
...
!     ******************************************************************
      ...
      DO LM=1,LMRX
        ...
        ENDDO
        CALL RADIAL__INTEGRAL(R1,DEX,NR,DWORK,QLM(LM))
      ENDDO
!
      RI=R1/XEXP
      DO IR=1,NR
      ...
      ENDDO
      CALL RADIAL__INTEGRAL(R1,DEX,NR,DWORK,RES)
...
!     ******************************************************************
      CALL TRACE__PUSH('AUGMENTATION_XC')
      EXC=0.D0
!
!     ==================================================================
...
!     ==================================================================
      CALL TRACE__PASS('BEFORE GRADIENTS')
      IF(TGRA) THEN
        GRHO(:,:,:)=0.D0
        DO ISPIN=1,NSPIN
          DO LM=1,LMRX
            CALL RADIAL__DERIVE(R1,DEX,NR,RHO(:,LM,ISPIN),GRHO(:,LM,ISPIN))
          ENDDO
        ENDDO
      ELSE
        GRHO(:,:,:)=0.D0
...
!       == NOW CALCULATE ENERGY DENSITY AND DERIAVTIVES =================
       ...
       &              +0.5D0*Y0*XVAL(IR,I,LM)*V3XC5(:,I,J)*XVAL(IR,J,LM)
        ...
        ENDDO
      ENDDO
      CALL RADIAL__INTEGRAL(R1,DEX,NR,WORK1(:)*R(:)**2,EXC)
!
!     ==================================================================
...
!     ==================================================================
      ...
      PSE(:)=0.D0
      AEDMU(:)=0.D0
      CALL RADIAL__POISSON(R1,DEX,NR,0,RHOCOR,AEDMU)
      RI=R1/XEXP
      DO IR=1,NR
        RI=RI*XEXP
        RI2=RI**2
      ...
      ENDDO
      CALL RADIAL__INTEGRAL(R1,DEX,NR,AEE,AEH)
      CALL RADIAL__INTEGRAL(R1,DEX,NR,PSE,PSH)
...
!     ==================================================================
      ...
      DO LM=1,LMRX
        L=INT(DSQRT(DBLE(LM-1)+.01D0))
        CALL RADIAL__POISSON(R1,DEX,NR,L,AERHO(1,LM),AEDMU)
        CALL RADIAL__POISSON(R1,DEX,NR,L,PSRHO(1,LM),PSDMU)
        ALPHA=1.D0/RCSM**2
        CALL GAUSSN(L,ALPHA,CL)
        QLM1=QLM(LM)
        RI=R1/XEXP
...
!     ******************************************************************
      ...
      DO IR=1,NR
        ...
        POT(IR)=POT(IR)+SVAR1
      ENDDO
      CALL RADIAL__INTEGRAL(R1,DEX,NR,WORK,EB)
      RETURN
      END
!
!     ..................................................................
...
!             ==========================================================
     ...
     &               -PSDMU(IR,ISPIN)*PSPHI(IR,LN1)*PSPHI(IR,LN2))*RI**2
                ENDDO
                CALL RADIAL__INTEGRAL(R1,DEX,NR,DWORK1,SVAR)
...
!!__!
!!__!     ..................................................................
!!__      subroutine augmentation_corelevels(ndimd,r1,dex,nr,lmrx,aepot)
!!__      implicit none
!!__      integer(4),intent(in) :: lmrx
!!__      integer(4),intent(in) :: ndimd
!!__      REAL(8)   ,INTENT(IN) :: r1
!!__      REAL(8)   ,INTENT(IN) :: dex
!!__      integer(4),intent(in) :: nr
!!__      REAL(8)   ,INTENT(IN) :: AEPOT(NR,LMRX,NDIMD)
!!__      integer(4)            :: nc         ! number of core states
!!__      integer(4),allocatable:: lofc(:)    !(nc)
!!__      real(8)   ,allocatable:: eofc(:)    !(nc)
!!__      real(8)   ,allocatable:: vatom(nr)  !(nc)
!!__      real(8)   ,allocatable:: phic(:,:)  !(nr,nc)
!!__      real(8)   ,allocatable:: r2(:)   ! r**2
!!__      real(8)   ,allocatable:: h(:,:)  ! hamiltonian
!!__      complex(8),allocatable:: uc(:,:) ! core eigenvectors
!!__      real(8)   ,allocatable:: eigc(:) ! core eigenvalues
!!__      integer(4)            :: ne ! dimensiona of the hamiltonian
!!__      integer(4)            :: i1,i2,m1,m2,l1,l2,lm1,lm2,lm3
!!__!     ******************************************************************
!!__      if(ndimd.ne.1) then
!!__        call error__stop('augmentation_corelevels')
!!__      end if
!!__      pi=4.d0*datan(1.d0)
!!__      c000=1/dsqrt(4.d0*pi)
!!__!
!!__!     ==================================================================
!!__!     ==  read core levels                                            ==
!!__!     ==================================================================
...
!!__      allocate(h(nc,nc))
!!__      allocate(eigc(nc))
!!__      allocate(ucc(nc,nc))
!!__!
!!__!     ==================================================================
!!__!     ==  radial grid r2=r**2                                         ==
!!__!     ==================================================================
!!__      xexp=dexp(dex)
!!__      ri=r1/xexp
!!__      do ir=1,nr
...
!!__!
!!__!     ==================================================================
!!__!     ==  calculate hamiltonian                                       ==
!!__!     ==================================================================
!!__      do i1=1,nc
!!__        l1=lofc(i1)
!!__        do i2=i1,nc
!!__          l2=lofc(i2)
!!__          aux(:)=phi(:,i1)*phi(:,i2)*r2(:)
!!__          do m1=1,2*l1+1
!!__            lm1=l1**2+m1
!!__            do m2=1,2*l2+1
!!__              lm2=l2**2+m2
!!__              sum(:)=0.d0
!!__              DO LM3=1,LMRX
!!__                CALL CLEBSCH(LM1,LM2,LM3,CG)
!!__                if(cg.eq.0.d0) cycle
!!__                sum(:)=sum(:)+cg*aepot(:,lm3,1)
!!__              enddo
!!__              if(lm1.eq.lm2) sum(:)=sum(:)-vc(:)*c000
!!__              sum(:)=sum(:)*aux(:)
!!__              CALL RADIAL__INTEGRAL(R1,DEX,NR,sum,h(ie1,ie2))
!!__              h(ie2,ie1)=h(ie1,ie2)
!!__! add eigenvalues
!!__            enddo
!!__          enddo
!!__        enddo
...
!!__!
!!__!     ==================================================================
!!__!     ==  diagonalize Hamiltonian                                     ==
!!__!     ==================================================================
!!__      call lib__diagr8(ne,h,eigc,uc)
!!__!
!!__!     ==================================================================
!!__!     ==  print eigenvalues                                           ==
!!__!     ==================================================================
!!__      call filehandler__unit('prot',nfilo)
!!__      write(nfilo,fmt='("core eigenvalues"//"================")')
!!__      write(nfilo,fmt='(10f10.3)')eigc
!!__      return
!!__      end
...
!
!     ..................................................................
      SUBROUTINE EXTERNAL1CPOT__SETPOT(ATOM,TYPE,iDIMD,VALUE,RC,PWR)
...
!
!     ..................................................................
      SUBROUTINE EXTERNAL1CPOT__REPORT(NFIL)
      USE EXPERTNAL1CPOT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      IF(NPOT.EQ.0) RETURN
!     CALL REPORT__TITLE(NFIL,"EXTERNAL POTENTIALS ON ORBITALS")
      WRITE(NFIL,FMT='("EXTERNAL POTENTIALS ON ORBITALS"/30("="))')
      DO IPOT=1,NPOT
!       CALL REPORT__I4VAL(NFIL,"POTENTIAL NR:",IPOT,' ')
!       CALL REPORT__CHVAL(NFIL,'ATOM',POT(IPOT)%ATOM)
!       CALL REPORT__CHVAL(NFIL,'TYPE',POT(IPOT)%TYPE)
!       CALL REPORT__R8VAL(NFIL,'VALUE',POT(IPOT)%VALUE,'H')
!       CALL REPORT__I4VAL(NFIL,'SPIN',POT(IPOT)%NSPIN,' ')
...
!
!     ..................................................................
      SUBROUTINE EXTERNAL1CPOT__APPLY(ATOM,LMNX,ndimd,DENMAT,DATH,ETOT)
!     ******************************************************************
!     ******************************************************************
      USE EXPERTNAL1CPOT_MODULE
      IMPLICIT NONE
...
!       ==================================================================
        ...
        ELSE IF(TRIM(POT1%TYPE).EQ.'ALL') THEN
          LANG=-1
        ELSE
          CALL ERROR__MSG('VALUE OF TYPE NOT RECOGNIZED')
          CALL ERROR__STOP('EXTERNAL1CPOT__APPLY')
        END IF
!
!       ==================================================================
...
!
        ...
        DO LN1=1,LNX
          ...
          DO LN2=1,LNX
            ...
            ENDDO
            CALL RADIAL__INTEGRAL(R1,DEX,NR,AUX,UONE(LN1,LN2))
          ENDDO
        ENDDO
        DEALLOCATE(AEPHI)
        DEALLOCATE(RDEP)
...
!       ==============================================================
        IF(POT1%IDIMD.LT.1.OR.POT1%IDIMD.GT.NDIMD) THEN
          CALL ERROR__MSG('VALUE OF POT1%IDIMD NOT RECOGNIZED')
          CALL ERROR__I4VAL('POT1%IDIMD',POT1%IDIMD)
          CALL ERROR__I4VAL('NDIMD',NDIMD)
          CALL ERROR__STOP('EXTERNAL1CPOT__APPLY')
        END IF
        DO IDIMD=1,NDIMD
...
!!__!
!!__!     .....................................................................
!!__      subroutine augmentation_softcore(isp,r1,dex,nr,aerho1,aepot1)
!!__      USE SCHRGL_INTERFACE_MODULE, ONLY : OUTBOUNDARY
!!__      implicit none
!!__      integer(4),intent(in) :: isp
!!__      real(8)   ,intent(in) :: r1
!!__      real(8)   ,intent(in) :: dex
!!__      integer(4),intent(in) :: nr
!!__      integer(4),intent(in) :: lmrx
!!__      real(8)   ,intent(in) :: aerho1(nr)
!!__      real(8)   ,intent(in) :: aepot1(nr)
!!__      real(8)               :: aecore0(nr)
!!__      real(8)               :: phi(nr,3)
!!__      real(8)   ,parameter  :: tolb=1.d-6
!!__      integer(4),parameter  :: ibix=100
!!__      integer(4)            :: nb
!!__      integer(4),allocatable:: lofb(:)  ! main angular momentum
!!__      integer(4),allocatable:: nnofb(:) ! #(nodes)
!!__      real(8)   ,allocatable:: fofb(:)  ! occupation
!!__      real(8)   ,allocatable:: eofb(:)  ! energy
!!__      real(8)   ,allocatable:: rclofb(:)
!!__      real(8)               :: aez
!!__      integer(4)            :: niter
!!__      integer(4)            :: iter,ib
!!__!     **********************************************************************
!!__      CALL SETUP__AECORE(ISP,NR,AECORE0)
!!__      CALL SETUP__AEZ(ISP,AEZ)
!!__!     ======================================================================
!!__!     ==  hardwire parameters for iron                                    ==
!!__!     ======================================================================
!!__      allocate(lofb(nb))
!!__      allocate(fofb(nb))
!!__      allocate(eofb(nb))
!!__      allocate(rclofb(nb))
!!__      nb=5
!!__      lofb=(/0,0,1,0,1/)
!!__      nnofb=(/0,1,0,2,1/)
!!__      do ib=1,nb
!!__        fofb(ib)=real(2*l+1)
!!__        zeff=aez-sum(fofb(1:ib-1))
!!__        rclofb(ib)=1.d0/zeff/real(lofb(ib)+nnofb(ib)+1)
!!__        ir=1+nint(log(rclofb(ib)/r1)/dex)
!!__        eofc(ib)=aepot(ir)
!!__      enddo
!!__
!!__!     ======================================================================
...
!!__          istart=1
!!__          tconv=.false.
!!__          call BISEC(ISTART,IBI,X0,Y0,DX,XM,YM)
!!__          do ibi=1,ibix
!!__            tforward=.true.
!!__            ir=1+nint(log(rclofb(ib)/r1)/dex)
!!__            call SCHROEDER(tforward,.false.,R1,DEX,NR,Lofb(ib),Eofb(ib) &
!!__     &                    ,AEZ,ir,4.d0 &
!!__     &                     ,aePOT,PHI,GINH,DLG)
!!__            din=rclofb(ib)*phi(ir,2)/phi(ir,1)
!!__            din=nnofb(ib)+0.5d0+datan(din)/pi
!!__            tforward=.false.
!!__            call SCHROEDER(tforward,.false.,R1,DEX,NR,Lofb(ib),Eofb(ib) &
!!__     &                    ,AEZ,ir,4.d0 &
!!__     &                     ,aePOT,PHI,GINH,DLG)
!!__            dout=rclofb(ib)*phi(ir,2)/phi(ir,1)
!!__            din=0.5d0+datan(dout)/pi
!!__            y0=din-dout
!!__            tconv=abs(dx).lt.tolb
!!__            if(tconv) exit
!!__            call BISEC(ISTART,IBI,X0,Y0,DX,XM,YM)
!!__            eofb(ib)=x0
!!__          enddo
!!__          if(.not.tconv) then
!!__            call error__msg('loop not converged')
!!__            call error__stop('augmentation_softcore')
!!__          end if
!!__          aecore(:)=aecore(:)+fb(ib)*phi(:)**2
!!__        enddo
!!__!
!!__!       ==============================================================
!!__!       ==  construct density                                       ==
!!__!       ==============================================================
!!__        rho(:)=rho0(:)+aecore(:)-aecore0(:)
!!__!
!!__!       ==============================================================
!!__!       ==  construct potential                                     ==
!!__!       ==============================================================
!!__
!!__
!!__
