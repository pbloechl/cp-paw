...
!     ******************************************************************
      IF(.NOT.ALLOCATED(FASTACCESS)) THEN
        IF(.NOT.ASSOCIATED(FIRST)) THEN
          CALL ERROR__MSG('NO SETUPS DEFINED')
          CALL ERROR__STOP('SETUP__ISELECT')
        END IF
        THIS=>FIRST
        NSP=1
...
!
      IF(I.GT.NSP) THEN
        CALL ERROR__MSG('INDEX I OUT OF RANGE')
        CALL ERROR__I4VAL('I',I)
        CALL ERROR__I4VAL('NSP',NSP)
        CALL ERROR__STOP('SETUP__ISELECT')
      END IF
!
...
!
!     ..................................................................
      SUBROUTINE SETUP__GETCH(ID,VAL)
!     ******************************************************************
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('SETUP__GETCH')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE SETUP__GETI4(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     **  REMARK: REQUIRES PROPER SETUP TO BE SELECTED                **
!     **                                                              **
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('SETUP__GETI4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE SETUP__GETI4A(ID,LEN,VAL)
!     ******************************************************************
!     **                                                              **
!     **  REMARK: REQUIRES PROPER SETUP TO BE SELECTED                **
!     **                                                              **
...
!     ******************************************************************
      IF(ID.EQ.'LOX') THEN
        IF(LEN.NE.THIS%LNX) THEN
          ...
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('SETUP__GETR8A')
        END IF
        VAL=THIS%LOX
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
...
!
!     ..................................................................
      SUBROUTINE SETUP__GETR8(ID,VAL)
!     ******************************************************************
!     **                                                              **
!     **  REMARK: REQUIRES PROPER SETUP TO BE SELECTED                **
!     **                                                              **
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('SETUP__GETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE SETUP__GETR8A(ID,LEN,VAL)
!     ******************************************************************
!     **                                                              **
!     **  REMARK: REQUIRES PROPER SETUP TO BE SELECTED                **
!     **                                                              **
...
!
!     ..................................................................
      SUBROUTINE SETUP__GETFOFG(ID,TDER,IND,NG_,G2,CELLVOL,F)
!     ******************************************************************
!     **                                                              **
!     **  REMARK: REQUIRES PROPER SETUP TO BE SELECTED                **
!     **                                                              **
...
!     ******************************************************************
      ...
      IF(ID.EQ.'PRO') THEN
        IF(IND.LT.1.OR.IND.GT.THIS%LNX) THEN
          CALL ERROR__MSG('LN OUT OF RANGE')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__I4VAL('IND',IND)
          CALL ERROR__I4VAL('LNX',THIS%LNX)
          CALL ERROR__STOP('SETUP__GETFOFG')
        END IF
...
!     ==================================================================
      ...
      IF(TDER) THEN
        G=DSQRT(G2(1))
        CALL RADIAL__DERIVATIVE(G1,DEX,NG,FOFG,G,F(1))
        F(1)=G*F(1)
        DO IG=2,NG_
          ...
          ELSE
            ...
            CALL RADIAL__DERIVATIVE(G1,DEX,NG,FOFG,G,F(IG))
            F(IG)=G*F(IG)
          END IF
        ENDDO
      ELSE
        G=DSQRT(G2(1))
        CALL RADIAL__VALUE(G1,DEX,NG,FOFG,G,F(1))
        DO IG=2,NG_
          IF(DABS(G2(IG)-G2(IG-1)).LT.1.D-6) THEN
            F(IG) =F(IG-1)
          ELSE
            G=DSQRT(G2(IG))
            IF(G.LT.1.D-6) NGAMMA=IG
            CALL RADIAL__VALUE(G1,DEX,NG,FOFG,G,F(IG))
          END IF
        ENDDO
      END IF
!
...
!     ******************************************************************
                            CALL TRACE__PUSH('SETUP__READ')
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO SETUP SELECTED')
        CALL ERROR__STOP('SETUP__READ')
      END IF
!
      CALL ATOMTYPELIST__NAME(THIS%I,THIS%ID)
      CALL ATOMTYPELIST__SELECT(THIS%ID)
      CALL ATOMTYPELIST__GETR8('M',THIS%M)
      CALL ATOMTYPELIST__GETR8('ZV',THIS%ZV)
      CALL ATOMTYPELIST__GETR8('PS<G2>',THIS%PSG2)
      CALL ATOMTYPELIST__GETR8('PS<G4>',THIS%PSG4)
!
      CALL FILEHANDLER__UNIT(THIS%FILEID,NFIL)
!
      CALL INPOT__LNX(NFIL,LNX)
      THIS%LNX=LNX
...
!     ==================================================================
      ...
      ALLOCATE(NPRO(LX+1))
      CALL ATOMTYPELIST__SELECT(THIS%ID)
      CALL ATOMTYPELIST__GETI4A('NPRO',LX+1,NPRO)
      DO L=0,LX
        ISVAR=0
        DO LN=1,THIS%LNX
        ...
        NPRO(L+1)=ISVAR
      ENDDO
      CALL ATOMTYPELIST__SETI4A('NPRO',LX+1,NPRO)
      DEALLOCATE(NPRO)
!
      LNOLD=THIS%LNX
      LNX=0
...
!     ==================================================================
                            CALL TRACE__PASS('CHECK MAX. RADIUS')
      IRMAX=0
      DO IR=1,NR
...
!     ==  DEFINE ARRAYS                                             ==
!     ================================================================
                            CALL TRACE__PASS('DEFINE ARRAYS')
!
...
!     == LIMIT MAX ANGULAR MOMENTUM FOR THE DENSITY TO MAX VALUE =======
      CALL ATOMTYPELIST__SELECT(THIS%ID)
      CALL ATOMTYPELIST__GETI4('LRHOX',LMRXCUT)
      LMRXCUT=(LMRXCUT+1)**2
...
!     ******************************************************************
      ...
      CALL CONSTANTS__GET('U',U)
      THIS1=>FIRST
      DO
        WRITE(NFIL,*)
        CALL REPORT__TITLE(NFIL,'ATOMIC SETUP: '//TRIM(THIS1%ID))
        CALL REPORT__R8VAL(NFIL,'ATOMIC NUMBER',THIS1%AEZ,' ')
        CALL REPORT__R8VAL(NFIL,'ATOMIC MASS  ',THIS1%M/U,'U')
        CALL REPORT__R8VAL(NFIL,'#(VALENCE ELECTRONS)',THIS1%ZV,' ')
        L=0
        NPROSUM=0
        DO WHILE (NPROSUM.LT.THIS1%LNX)
          ...
          IF(NPRO.NE.0) THEN
             ...
             STRING=TRIM(STRING)//')'
             CALL REPORT__I4VAL(NFIL,TRIM(STRING),NPRO,' ')
          END IF
          L=L+1
          NPROSUM=NPROSUM+NPRO
        ENDDO

        CALL REPORT__I4VAL(NFIL,'MAX #(ANGULAR MOMENTA (L,M) FOR 1C-DENSITY)' &
     &                        ,THIS1%LMRX,' ')
        CALL REPORT__R8VAL(NFIL,'GAUSSIAN DECAY FOR COMPENSATION DENSITY ' &
     &                        ,THIS1%RCSM,'ABOHR ')
        CALL REPORT__R8VAL(NFIL,'GAUSSIAN DECAY FOR EXTENDED COMPENSATION DENSITY' &
     &                        ,THIS1%RCBG,'ABOHR ')
        CALL REPORT__R8VAL(NFIL,'PARAMETER PS<G2> FOR MASS RENORMALIZATION' &
     &                        ,THIS1%PSG2,'H')
        CALL REPORT__R8VAL(NFIL,'PARAMETER PS<G4> FOR MASS RENORMALIZATION' &
     &                        ,THIS1%PSG4,'H')
...
!     ==  CREATE SETUPS                                               ==
!     ==================================================================
      CALL ATOMTYPELIST__LENGTH(NSP1)
      DO ISP=1,NSP1
        CALL ATOMTYPELIST__NAME(ISP,NAME)
        CALL ATOMTYPELIST__SELECT(NAME)
        CALL ATOMTYPELIST__UNSELECT
        CALL SETUP__SELECT(NAME)
      ENDDO
!
!     ==================================================================
!     ==  READ SETUP FILES                                            ==
...
!
!     ..................................................................
      SUBROUTINE SETUP__RADIALGPRO(ISP_,LNX_,CELLVOL_,NG_,G2_,PROG_,DPROG_)
!     ******************************************************************
...
!     ******************************************************************
      ...
      INTEGER(4)            :: IG,LN,L
!     ******************************************************************
      CALL ERROR__MSG('MARKED FOR DELTION: DO NOT USE!')
      CALL ERROR__STOP('SETUP__RADIALGPRO')
                            CALL TRACE__PUSH('SETUP__GPROJECTORS')
!
!     ==================================================================
!     ==  SOME INITIAL CHECKS                                         ==
!     ==================================================================
      CALL SETUP__ISELECT(ISP_)
      IF(LNX_.NE.THIS%LNX) THEN
        CALL ERROR__OVERFLOW('LNX_',LNX_,THIS%LNX)
        CALL ERROR__STOP('SETUP__GPROJECTORS')
      END IF
...
!
!     ..................................................................
      SUBROUTINE SETUP__COMPENSATION(ISP_,NG_,G2_,CELLVOL_ &
     &                             ,G0_,V0_,DG0_,DV0_)
!     ******************************************************************
!     ** GAUSSIANS AND POTENTIAL FOR THE EWALD TRICK WITH THE         **
!     ** COMPENSATION CHARGE DENSIT TO THE DENSITY G-GRID             **
...
!     ******************************************************************
      CALL ERROR__MSG('MARKED FOR DELTION: DO NOT USE!')
      CALL ERROR__STOP('SETUP__COMPENSATION')
                              CALL TRACE__PUSH('SETUP__COMPENSATION')
!
!     ==================================================================
!     ==  SOME INITIAL CHECKS                                         ==
!     ==================================================================
      IF(ISP_.GT.NSP) THEN
        CALL ERROR__OVERFLOW('ISP_',ISP_,NSP)
        CALL ERROR__STOP('SETUP__COMPENSATION')
      END IF
!
!     ==================================================================
!     ==  CALCULATE G0,V0                                             ==
!     ==================================================================
               CALL TIMING__CLOCKON('FORMF-BESSOV COMPENSATION')
      CALL SETUP_COMPENSATIONGAUSSIANS(THIS%RCBG,THIS%RCSM &
     &          ,NG_,G2_,CELLVOL_,G0_,V0_,DG0_,DV0_)
             CALL TIMING__CLOCKOFF('FORMF-BESSOV COMPENSATION')
                            CALL TRACE__POP
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE SETUP__PSCOREG(ISP_,NG_,G2_,CELLVOL_,PSCOREG_,DPSCOREG_)
!     ******************************************************************
...
!     ******************************************************************
      CALL ERROR__MSG('MARKED FOR DELTION: DO NOT USE!')
      CALL ERROR__STOP('SETUP__pscoreg')
                              CALL TRACE__PUSH('SETUP__PSCOREG')
      CALL SETUP__ISELECT(ISP_)
!
!     ==================================================================
!     ==  CALCULATE PSEUDOCOREDENSITY IN RECIPROCAL SPACE             ==
!     ==================================================================
                              CALL TIMING__CLOCKON('FORMF-BESSOV PSCORE')
      CALL SETUP_RADTOG(0,R1,DEX,NRX,RMAX,THIS%PSCORE,NG_,CELLVOL_,G2_ &
     &               ,PSCOREG_,DPSCOREG_)
                              CALL TIMING__CLOCKOFF('FORMF-BESSOV PSCORE')
                              CALL TRACE__POP
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE SETUP__VBARG(ISP_,NG_,G2_,CELLVOL_,VBARG_,DVBARG_)
!     ******************************************************************
...
!     ******************************************************************
      CALL ERROR__MSG('MARKED FOR DELTION: DO NOT USE!')
      CALL ERROR__STOP('SETUP__vbarg')
                            CALL TRACE__PUSH('SETUP__VBARG')
             CALL TIMING__CLOCKON('FORMF-BESSOV VBAR')
      CALL SETUP__ISELECT(ISP_)
      CALL SETUP_RADTOG(0,R1,DEX,NRX,RMAX,THIS%VADD,NG_,CELLVOL_,G2_ &
     &               ,VBARG_,DVBARG_)
             CALL TIMING__CLOCKOFF('FORMF-BESSOV VBAR')
                            CALL TRACE__POP
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE SETUP__AEPARTIALWAVES(ISP_,NRX_,LNX_,AEPHI_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__PSPARTIALWAVES(ISP_,NRX_,LNX_,PSPHI_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__AECORE(ISP_,NRX_,AECORE_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__PSCORE(ISP_,NRX_,PSCORE_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__VBAR(ISP_,NRX_,VBAR_)
!     ******************************************************************
!     **  RETURN PS CORE DNSITY                                       **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__1COVERLAP(ISP_,LNXX_,DOVER_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__1CKINETIC(ISP_,LNXX_,DTKIN_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__RCSM(ISP_,RCSM_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__RCBG(ISP_,RCBG_)
!     ******************************************************************
!     **  RETURN NUMBER OF PARTIAL WAVES                              **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__LNX(ISP_,LNX_)
!     ******************************************************************
!     **  RETURN NUMBER OF PARTIAL WAVES                              **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__LMNX(ISP_,LMNX_)
!     ******************************************************************
!     **  RETURN NUMBER OF PARTIAL WAVES                              **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__LMNXX(LMNXX_)
!     ******************************************************************
!     **  RETURN NUMBER OF PARTIAL WAVES                              **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__LOFLN(ISP_,LNX_,LOX_)
!     ******************************************************************
...
!     ******************************************************************
      ...
      IF(LNX_.GT.THIS%LNX) THEN
        CALL ERROR__MSG('LNX ON INPUT TOO SMALL')
        CALL ERROR__OVERFLOW('LNX_',LNX_,THIS%LNX)
        CALL ERROR__STOP('SETUP__LOFLN')
      END IF
      DO LN=1,THIS%LNX
        LOX_(LN)=THIS%LOX(LN)
      ENDDO
...
!
!     ..................................................................
      SUBROUTINE SETUP__NSPECIES(NSP_)
!     ******************************************************************
!     **  RETURN NUMBER OF PARTIAL WAVES                              **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__LMRXX(LMRXX_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE SETUP__LMRX(ISP_,LMRX_)
!     ******************************************************************
!     **  RETURN MAXIMUM ANGULAR MOMENTUM FOR THE 1C-DENSITY          **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__AEZ(ISP_,AEZ_)
!     ******************************************************************
!     **  RETURN MAXIMUM ANGULAR MOMENTUM FOR THE 1C-DENSITY          **
!     ******************************************************************
      USE SETUP_MODULE
...
!
!     ..................................................................
      SUBROUTINE SETUP__RADGRID(ISP_,R1_,DEX_,NR_)
!     ******************************************************************
...
!     ******************************************************************
      ...
      INTEGER(4)            :: IG,NSTART
!     ******************************************************************
      CALL ERROR__MSG('MARKED FOR DELETION: DO NOT USE!')
      CALL ERROR__STOP('SETUP_COMPENSATIONGAUSSIANS')
      PI=4.D0*DATAN(1.D0)
!
!     ==================================================================
...
!     == AN EARLIER VERSION USED THE VALUE OF THE FIRST RADIAL GRID POINT
      G=DSQRT(G2(1))
      CALL RADIAL__VALUE(G1,DEX,NR1,FOFG1,G,FOFG(1))
      CALL RADIAL__DERIVATIVE(G1,DEX,NR1,FOFG1,G,DFOFG(1))
      DFOFG(1)=G*DFOFG(1)
      DO IG=2,NG
        IF(DABS(G2(IG)-G2(IG-1)).LT.1.D-6) THEN
          FOFG(IG) =FOFG(IG-1)
        ...
        ELSE
          G=DSQRT(G2(IG))
          CALL RADIAL__VALUE(G1,DEX,NR1,FOFG1,G,FOFG(IG))
          CALL RADIAL__DERIVATIVE(G1,DEX,NR1,FOFG1,G,DFOFG(IG))
          DFOFG(IG)=G*DFOFG(IG)
        END IF
      ENDDO
      RETURN
...
!
!     ...........................................INPOT..................
      SUBROUTINE INPOT__LNX(NFIL,LNX)
!     ******************************************************************
!     **                                                              **
!     **                                                              **
!     ******************************************************************
...
!
!     ...........................................INPOT..................
      SUBROUTINE INPOT__READALL(NFIL,NRX,R1,DEX,NR,LNX,LOX,AEZ,PSZ &
...
!     ******************************************************************
                              CALL TRACE__PUSH('INPOT__READALL')
      REWIND NFIL
...
6000  FORMAT(F15.10,F10.5,2I4,2F5.2,F20.15,I5)
      IF(R11.NE.R1.OR.DEX1.NE.DEX.OR.NR1.NE.NR) THEN
        CALL ERROR__MSG('ONLY ONE TYPE OF RADIAL GRID ALLOWED')
        CALL ERROR__STOP('INPOT')
      END IF
