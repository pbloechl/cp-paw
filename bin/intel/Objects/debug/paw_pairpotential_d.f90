...
!     ==================================================================
 1000 CONTINUE
      IF(TTEST_V.AND.TTEST_FORCE) THEN
        CALL ERROR__MSG('CANNNOT TEST FORCE AND POTENTIAL AT THE SAME TIME')
        CALL ERROR__STOP('PAIRP')
      END IF
      IF(TTEST_V) THEN
        CALL SELFTEST__START('PAIRP-QLM',LMRXX*NAT,QLM,1.D-3)
        VQLM(:,:)=0.D0
      END IF
      IF(TTEST_FORCE) THEN
        CALL SELFTEST__START('PAIRP-FORCE',3*NAT,R0,1.D-4)
        FORCE(:,:)=0.D0
      END IF
!
!     ==================================================================
...
!     ==================================================================
      CALL MPE__COMBINE('+',EPAIR)
      CALL MPE__COMBINE('+',VQLM)
      CALL MPE__COMBINE('+',FORCE)
...
!     ==================================================================
      IF(TTEST_V) THEN
        CALL SELFTEST__END('PAIRP-QLM',LMRXX*NAT,VQLM,EPAIR,TBACK)
        IF(TBACK) GOTO 1000
      ...
      IF(TTEST_FORCE) THEN
        ! NOTE: FORCE = -DE/DR!
        CALL SELFTEST__END('PAIRP-FORCE',3*NAT,FORCE,-EPAIR,TBACK)
        IF(TBACK) GOTO 1000
        CALL ERROR__MSG('NORMAL STOP AFTER SELFTEST')
        CALL ERROR__STOP('PAIRP')
      END IF
...
!     ******************************************************************
      ...
      GOFXB = DEXP(-XB**2)
      SVAR=DSQRT(PI)/2.D0
      CALL LIB__ERFR8(XS,QLS)
      QLS=SVAR*QLS
      CALL LIB__ERFR8(XB,QLB)
      QLB=SVAR*QLB
      DQS=-XS*GOFXS
      DQB=-XB*GOFXB
      FACS=1.D0/XS
