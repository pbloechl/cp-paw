...
!
!     ..................................................................
      SUBROUTINE POTENTIAL__GETR8(ID,VALUE)
!     ******************************************************************
!     ******************************************************************
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('POTENTIAL__GETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE POTENTIAL__SETL4(ID,VAL)
!     ******************************************************************
!     ******************************************************************
      USE POTENTIAL_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('POTENTIAL__SETL4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE POTENTIAL__SETR8A(ID,LEN,VAL)
!     ******************************************************************
!     ******************************************************************
      USE POTENTIAL_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      IF(ID.EQ.'RBAS') THEN
        IF(LEN.NE.9) THEN
          CALL ERROR__MSG('SIZE INCONSISTENCY')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('POTENTIAL__SETR8A')
        END IF
...
!
!     ..................................................................
      SUBROUTINE POTENTIAL__REPORT(NFIL)
!     ******************************************************************
!     **                                                              **
...
!     ******************************************************************
      ...
      CALL REPORT__TITLE(NFIL,'POTENTIAL')
      IF(.NOT.TINI) THEN
        CALL REPORT__STRING(NFIL,'POTENTIAL OBJECT NOT YET INITIALIZED')
        RETURN
      END IF
      CALL REPORT__R8VAL(NFIL,'DENSITY PLANE WAVE CUTOFF',EPWRHO/RY,'RY')
      CALL PLANEWAVE__SELECT('DENSITY')
      CALL PLANEWAVE__GETI4('NGG',NGG)
      CALL REPORT__I4VAL(NFIL,'#(G-VECTORS FOR DENSITY)',NGG,' ')
      MEMORY=REAL(8*(NGL*NSP)*4   & ! V0,G0,PSCOREG,VBARG
     &           +8*(NGL*LMRXX)*1 & ! YLMOFG
     &           +4*(NSP)*1       & ! LMRX
     &           ,KIND=8)/MBYTE
      CALL REPORT__R8VAL(NFIL,'MEMORY REQUIRED BIG PERMANENT ARRAYS' &
     &           ,MEMORY/MBYTE,'MBYTE')
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE POTENTIAL__CLEAR
!     ******************************************************************
...
!     ******************************************************************
      ...
      IMPLICIT NONE
!     ******************************************************************
                                CALL TRACE__PUSH('POTENTIAL__CLEAR')
...
!
!     ..................................................................
      SUBROUTINE POTENTIAL__INITIALIZE(EPWRHO_,NR1START_,NR1L_,NR2_,NR3_)
!     ******************************************************************
...
!     ******************************************************************
      ...
      INTEGER(4)            :: ISP
!     ******************************************************************
                                CALL TRACE__PUSH('POTENTIAL__INITIALIZE')
      IF(TINI) THEN
        CALL ERROR__MSG('POTENTIAL OBJECT IS ALREADY INITIALIZED')
        CALL ERROR__STOP('POTENTIAL__INITIALIZE')
      END IF
      TINI=.TRUE.
      TSET=.FALSE.
!
...
!     ================================================================
      ...
      CALL PLANEWAVE__INITIALIZE('DENSITY',RBAS,K,.TRUE.,EPWRHO &
     &                         ,NR1START,NR1L,NR2,NR3)
      CALL PLANEWAVE__SELECT('DENSITY')
      CALL PLANEWAVE__SETL4('SUPER',.TRUE.)
      CALL PLANEWAVE__GETI4('NGL',NGL)
      CALL SETUP__LMRXX(LMRXX)
      CALL SETUP__NSPECIES(NSP)
      ALLOCATE(LMRX(NSP))
      DO ISP=1,NSP
        CALL SETUP__LMRX(ISP,LMRX(ISP))
      ENDDO
...
!     ******************************************************************
                                CALL TRACE__PUSH('POTENTIAL_UPDATE')
      CALL PLANEWAVE__GETCH('ID',PLANEWAVESELECTION)
!
!     ================================================================
!     == ALLOCATE ARRAYS                                            ==
!     ================================================================
...
!     ================================================================
      DO ISP=1,NSP
          ...
          CALL SETUP__GETFOFG('G0',.FALSE.,1,NGL,G2,CELLVOL,G0(1,ISP))
          CALL SETUP__GETFOFG('G0',.TRUE. ,1,NGL,G2,CELLVOL,DG0(1,ISP))
          CALL SETUP__GETFOFG('V0',.FALSE.,1,NGL,G2,CELLVOL,V0(1,ISP))
          CALL SETUP__GETFOFG('V0',.TRUE. ,1,NGL,G2,CELLVOL,DV0(1,ISP))
          CALL SETUP__GETFOFG('PSCORE',.FALSE.,1,NGL,G2,CELLVOL,PSCOREG(1,ISP))
          CALL SETUP__GETFOFG('PSCORE',.TRUE. ,1,NGL,G2,CELLVOL,DPSCOREG(1,ISP))
          CALL SETUP__GETFOFG('VADD',.FALSE.,1,NGL,G2,CELLVOL,VBARG(1,ISP))
          CALL SETUP__GETFOFG('VADD',.TRUE. ,1,NGL,G2,CELLVOL,DVBARG(1,ISP))
!
!        CALL SETUP__COMPENSATION(ISP,NGL,G2,CELLVOL &
!     &                      ,G0(1,ISP),V0(1,ISP),DG0(1,ISP),DV0(1,ISP))
!        CALL SETUP__PSCOREG(ISP,NGL,G2,CELLVOL,PSCOREG(1,ISP),DPSCOREG(1,ISP))
!        CALL SETUP__VBARG(ISP,NGL,G2,CELLVOL,VBARG(1,ISP),DVBARG(1,ISP))
      ENDDO
      DEALLOCATE(G2)
      DEALLOCATE(GVEC)
      CALL PLANEWAVE__SELECT(PLANEWAVESELECTION)
                              CALL TRACE__POP
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE POTENTIAL__VOFRHO(NRL,NDIMD,RHO,LMRXX_,NAT_,QLM,VQLM &
...
!     ******************************************************************
      ...
      REAL(8)                  :: SVAR
!     ******************************************************************
                                CALL TRACE__PUSH('POTENTIAL__VOFRHO')
                                CALL TIMING__CLOCKON('POTENTIAL')
      IF(.NOT.TINI) THEN
        CALL ERROR__MSG('POTENTIAL OBJECT IS NOT INITIALIZED')
        CALL ERROR__STOP('POTENTIAL__VOFRHO')
      END IF
      IF(LMRXX_.NE.LMRXX) THEN
        CALL ERROR__MSG('INPUT DATA INCONSISTENT WITH MODULE')
        CALL ERROR__I4VAL('LMRXX_',LMRXX_)
        CALL ERROR__I4VAL('LMRXX',LMRXX)
        CALL ERROR__STOP('POTENTIAL__VOFRHO')
      END IF
!     == COLLECT FROM ATOMLIST =======================================
      CALL ATOMLIST__NATOM(NAT)
      ALLOCATE(ISPECIES(NAT))
      CALL ATOMLIST__GETI4A('ISPECIES',0,NAT,ISPECIES)
      IF(NAT_.NE.NAT) THEN
        CALL ERROR__MSG('INPUT DATA INCONSISTENT WITH MODULE')
        CALL ERROR__I4VAL('NAT_',NAT_)
...
!       CALL FILEHANDLER__UNIT('DENSITY.DX',NFIL)
!       CALL  WRITEDX_DENSITY
!    &       (NFIL,NR1GLOB,NR2,NR3,NR1GLOB,NR2,NR3,RBAS,RHOE)
!       CALL FILEHANDLER__CLOSE('DENSITY.DX')
...
!     == COLLECT FROM PLANEWAVE OBJECT =================================
      ...
      CALL PLANEWAVE__GETR8A('G2',NGL,G2)
      CALL PLANEWAVE__GETR8A('GVEC',3*NGL,GVEC)
      CALL PLANEWAVE__GETI4('NRL',ISVAR)
      IF(ISVAR.NE.NRL) THEN
        CALL ERROR__MSG('#(GRID POINTS INCONSISTENT)')
        CALL ERROR__STOP('POTENTIAL__VOFRHO')
      END IF
      CALL PLANEWAVE__GETI4('NR1',NR1GLOB)
...
!     ==================================================================
      ...
      DEALLOCATE(G2)
      DEALLOCATE(GVEC)
                                CALL TIMING__CLOCKOFF('POTENTIAL')
                                CALL TRACE__POP
      RETURN
      END
!
...
!     ==================================================================
!      if(toptic)CALL OPTICS__VXCG_VALENCE(TGRA,NSPIN,NRL,NRL,NR1GLOB*NR2*NR3 &
!     &                        ,NR1,NR2,NR3,NGL,CELLVOL,RHOE)
!
!     ==================================================================
!     == TRANSFORM PSEUDO DENSITY INTO FOURIER SPACE                  ==
!     ==================================================================
!CALL PLANEWAVE__GETI4('NGAMMA',NGAMMA)
!svar=0.d0
!do ir=1,nrl
!  svar=svar+rhoe(ir,1)
!enddo
...
!print*,'nspin,ngl,nrl,cellvol,ngamma ',nspin,ngl,nrl,cellvol,ngamma
      ...
      CALL PLANEWAVE__SUPFFT('RTOG',NSPIN,NGL,RHOG,NRL,RHOE)
...
!     ==================================================================
!      if(toptic)CALL OPTICS__VXCG_CORE(TGRA,NSPIN,NRL,NRL,NR1GLOB*NR2*NR3 &
 ...
 1000 CONTINUE
      IF(TTEST) THEN
        IF(ITEST.EQ.1) THEN
          ...
          CALL SELFTEST__START('POTENTIAL-RHO',2*NGL*NSPIN,RHELP,1.D-5)
...
!         == REDEFINE QLM AS INTENT(INOUT)
          RHOG(:,:)=RHO_SELFTEST(:,:)
          CALL SELFTEST__START('POTENTIAL-QLM',LMRXX*NAT,QLM,1.D-1)
          VQLM(:,:)=0.D0
        ELSE IF(ITEST.EQ.3) THEN
!         == REDEFINE TAU0 AS INTENT(INOUT)
          RHOG(:,:)=RHO_SELFTEST(:,:)
          CALL SELFTEST__START('POTENTIAL-FORCE',3*NAT,TAU0,1.D-4)
          FION(:,:)=0.D0
        END IF
      END IF
!
...
!     ==================================================================
                                CALL TIMING__CLOCKON('VOFRHO: HARTREE')
      EHARTREE=0.D0
      FORCE1(:,:)=0.D0
      VQLM1(:,:)=0.D0
     ...
     &            ,TSTRESS,DG0,DV0,STRESS1)
      CALL MPE__COMBINE('+',RHOBT)
      FIONT(:,:)=FIONT(:,:)+FORCE1(:,:)
      VQLMT(:,:)=VQLMT(:,:)+VQLM1(:,:)
      STRESST(:,:)=STRESST(:,:)+STRESS1(:,:)
                                CALL TIMING__CLOCKOFF('VOFRHO: HARTREE')
!
!     ==================================================================
!     ==  CALCULATE PAIR-POTENTIAL                                    ==
!     ==================================================================
                                CALL TIMING__CLOCKON('VOFRHO: PAIRP')
      DO ISP=1,NSP
        CALL SETUP__RCSM(ISP,RCSM(ISP))
        CALL SETUP__RCBG(ISP,RCBG(ISP))
      ENDDO
      EPAIR=0.D0
      FORCE1(:,:)=0.D0
      VQLM1(:,:)=0.D0
     ...
     &                          ,TAU0,FORCE1,QLM,VQLM1,EPAIR,STRESS1)
      ...
      STRESS(:,:)=STRESS(:,:)+STRESS1(:,:)
                                CALL TIMING__CLOCKOFF('VOFRHO: PAIRP')
...
!     CALL DIPOLE(RBAS,NGL,NGL,GVEC,RHOG(1,1),NAX,NSP,NA,TAU0,LMRXX,LMRX,QLM)
                                CALL TIMING__CLOCKON('VOFRHO: ISOLATE')
      EISOLATE=0.D0
      FORCE1(:,:)=0.D0
      VQLM1(:,:)=0.D0
     ...
     &            ,LMRXX,LMRX,QLM,VQLM1,RHOBT,NGL,RHOG(1,1),VHARTREE)
      FION(:,:)=FION(:,:)+FORCE1(:,:)
      VQLM(:,:)=VQLM(:,:)+VQLM1(:,:)
                                CALL TIMING__CLOCKOFF('VOFRHO: ISOLATE')
!
!     ==================================================================
!     ==  HYPERFINE PARAMETERS                                        ==
!     ==================================================================
!     == CHECK IF NTILDE+NTILDEC (LIKE HERE) OR ONLY NTILDE SHALL BE USED
      CALL HYPERFINE__SETPWRHO('TOT',NGL,GVEC,RHOG(1,1))
      CALL HYPERFINE__GETFLAG('SPIN',TCHK)
      IF(TSPIN.AND.TCHK) THEN
        CALL HYPERFINE__SETPWRHO('SPIN',NGL,GVEC,RHOG(1,NSPIN))
      END IF
      CALL HYPERFINE__SETPWPOT(NGL,GVEC,VHARTREE)
!
!     ==================================================================
!     ==  GRAPHICS  PARAMETERS                                        ==
!     ==================================================================
      CALL GRAPHICS__SETPWPOT('ELECTROSTATIC',NGL,VHARTREE)
!
!     ==================================================================
!     ==  TRANSFORM  TOTAL CHARGE DENSITY AND POTENTIAL TO REAL SPACE ==
!     ==================================================================
                            CALL TIMING__CLOCKON('VOFRHO: FFT POTENTIAL')
      CALL PLANEWAVE__SELECT('DENSITY')
      CALL PLANEWAVE__SUPFFT('GTOR',NSPIN,NGL,RHOG,NRL,RHOE)
                          CALL TIMING__CLOCKOFF('VOFRHO: FFT POTENTIAL')
!
!     ==================================================================
!     ==  CALCULATE |NABLA|RHO IN G-SPACE AND TRANSFORM TO REAL SPACE ==
!     ==================================================================
      IF(TGRA) THEN
                        CALL TIMING__CLOCKON('VOFRHO: NABLA-RHO AND FFT')
        ALLOCATE(GRHO(NNR,3,NSPIN))
        CALL POTENTIAL_GRADIENT('RHO',NDIMD,NGL,GVEC,RHOG,NRL,GRHO)
                       CALL TIMING__CLOCKOFF('VOFRHO: NABLA-RHO AND FFT')
      ELSE
        ALLOCATE(GRHO(1,1,1))
      END IF
!
...
!     ==================================================================
                           CALL TIMING__CLOCKON('VOFRHO: XC-POTENTIAL')
      EXC=0.D0
...
WRITE(*,FMT='("XC STRESS ",3F15.7)')STRESS1(3,:)
                           CALL TIMING__CLOCKOFF('VOFRHO: XC-POTENTIAL')
!
!     ==================================================================
!     == FOURIER TRANSFORM XC POTENTIALS                              ==
!     ==================================================================
      IF(TGRA) THEN
                             CALL TIMING__CLOCKON('VOFRHO: GRAD-COR-POT')
        CALL POTENTIAL_GRADIENT('POT',NSPIN,NGL,GVEC,RHOG,NRL,GRHO)
                           CALL TIMING__CLOCKOFF('VOFRHO: GRAD-COR-POT')
      ELSE
        RHOG(:,:)=(0.D0,0.D0)
      END IF
      DEALLOCATE(GRHO)
      ...
      CALL PLANEWAVE__SUPFFT('RTOG',NSPIN,NGL,CWORK,NRL,RHOE)
      DO ISPIN=1,NSPIN
        DO IG=1,NGL
...
!     ==================================================================
!     if(toptic)CALL OPTICS__VXCG(NR1,NR2,NR3,NGL,RHOG,NSPIN)
!
!     ==================================================================
...
!     ==================================================================
      CALL TIMING__CLOCKON('VOFRHO: FORCE ON PS-CORE')
...
WRITE(*,FMT='("CORE STRESS ",3F15.7)')STRESS1(3,:)
      CALL TIMING__CLOCKOFF('VOFRHO: FORCE ON PS-CORE')
!
!     ==================================================================
!     == SEND POTENTIAL TO OPTICS CODE                                ==
!     ==================================================================
!     if(toptic)CALL OPTICS__VOFG(NR1,NR2,NR3,NGL,RHOG,NSPIN)
!
!     ==================================================================
!     ==  COMBINE ARRAYS FOR PARALELL PROCESSING                      ==
!     ==================================================================
!     ++ PARALLEL START ++++++++++++++++++++++++++++++++++++++++++++++++
      CALL MPE__COMBINE('+',EHARTREE)
      CALL MPE__COMBINE('+',FIONT)
      CALL MPE__COMBINE('+',VQLMT)
      CALL MPE__COMBINE('+',STRESST)
      CALL MPE__COMBINE('+',EXC)
!     ++ PARALLEL END ++++++++++++++++++++++++++++++++++++++++++++++++++
...
!     ==================================================================
      CALL ENERGYLIST__ADD('TOTAL ENERGY',EHARTREE)
      CALL ENERGYLIST__ADD('AE  ELECTROSTATIC',EHARTREE)
      CALL ENERGYLIST__ADD('PS  ELECTROSTATIC',EHARTREE)
!
      CALL ENERGYLIST__SET('PAIRPOTENTIAL',EPAIR)
      CALL ENERGYLIST__ADD('TOTAL ENERGY',EPAIR)
      CALL ENERGYLIST__ADD('AE  ELECTROSTATIC',EPAIR)
      CALL ENERGYLIST__ADD('PS  ELECTROSTATIC',EPAIR)
!
      CALL ENERGYLIST__ADD('TOTAL ENERGY',EISOLATE)
      CALL ENERGYLIST__ADD('AE  ELECTROSTATIC',EISOLATE)
      CALL ENERGYLIST__ADD('PS  ELECTROSTATIC',EISOLATE)
      CALL ENERGYLIST__SET('ISOLATE ENERGY',EISOLATE)
!
      CALL ENERGYLIST__ADD('TOTAL ENERGY',EXC)
      CALL ENERGYLIST__ADD('AE  EXCHANGE-CORRELATION',EXC)
      CALL ENERGYLIST__ADD('PS  EXCHANGE-CORRELATION',EXC)
!
!     ==================================================================
!     ==  SELFTEST                                                    ==
!     ==================================================================
      IF(TTEST) THEN
        ...
        IF(ITEST.EQ.1) THEN
          ...
          END IF
          CALL SELFTEST__END('POTENTIAL-RHO',2*NGL*NSPIN,RHELP,SVAR,TBACK)
          DEALLOCATE(RHELP)
        ELSE IF(ITEST.EQ.2) THEN
          CALL SELFTEST__END('POTENTIAL-QLM',LMRXX*NAT,VQLM,SVAR,TBACK)
        ELSE IF(ITEST.EQ.3) THEN
          CALL SELFTEST__END('POTENTIAL-FORCE',3*NAT,FION,-SVAR,TBACK)
        END IF
        IF(TBACK) GOTO 1000
        CALL ERROR__MSG('NORMAL STOP AFTER SELFTEST')
        CALL ERROR__STOP('POTENTIAL_VOFRHO')
...
!     ******************************************************************
      ...
      DO IAT=1,NAT
        ISP=ISPECIES(IAT)
        CALL PLANEWAVE__STRUCTUREFACTOR(TAU0(1,IAT),NGL,EIGR)
        DO IG=1,NGL
...
!     ==================================================================
      ...
      IF(IDENT.EQ.'RHO') THEN
        DO IDIM=1,NDIMD
          ...
          ENDDO
        ENDDO
        CALL PLANEWAVE__SUPFFT('GTOR',3*NDIMD,NGL,CWORK,NRL,GRHOOFR)
      ELSE IF(IDENT.EQ.'POT') THEN
!       ================================================================
!       == CALCULATE DIVERGENCE OF THE GRADIENTPOTENTIAL IN G-SPACE   ==
!       ================================================================
        CALL PLANEWAVE__SUPFFT('RTOG',3*NDIMD,NGL,CWORK,NRL,GRHOOFR)
        RHOOFG(:,:)=(0.D0,0.D0)
        DO IDIM=1,NDIMD
          DO I=1,3
            DO IG=1,NGL
        ...
        ENDDO
      ELSE
        CALL ERROR__MSG('INCORRECT IDENTIFIER')
        CALL ERROR__CHVAL('IDENT',IDENT)
        CALL ERROR__STOP ('POTENTIAL_GRADIENT')
      END IF
      DEALLOCATE(CWORK)
      RETURN
      END
...
!     ******************************************************************
      ...
      DO IAT=1,NAT
        ISP=ISPECIES(IAT)
        CALL PLANEWAVE__STRUCTUREFACTOR(RAT(1,IAT),NGL,EIGR)
...
!     == (-I)**L/(2L+1)!! ==============================================
      ...
      DO IAT=1,NAT
        IF(TSTRESS) THEN
          ...
          DO LM1=1,LMRX(ISP)
            DO I=1,10
              CALL SPHERICAL__CCMAT0(LM1,I,LM2,CC)
              IF(LM2.EQ.0) EXIT
              P0(:,:,LM2)=P0(:,:,LM2)+CFAC(LM1)*QLM(LM1,IAT)*CC(:,:)
            ENDDO
            DO I=1,10
              CALL SPHERICAL__CCMATM(LM1,I,LM2,CC)
              IF(LM2.EQ.0) EXIT
...
!
      IF(EXC.NE.EXC) THEN
        CALL ERROR__MSG('POTENTIAL_XC')
        CALL ERROR__R8VAL('EXC',EXC)
        CALL ERROR__STOP('POTENTIAL_XC')
      END IF
      RETURN
      END
!
...
!     ******************************************************************
      ...
      IF(LMX.NE.(LX+1)**2) THEN
        CALL ERROR__MSG('LMX MUST CONTAIN ALL MAGNETIC QUANTUM NUMBER')
        CALL ERROR__MSG('FOR EACH MAIN ANGULAR MOMENTUM')
        CALL ERROR__I4VAL('LMX ',LMX)
        CALL ERROR__STOP('POTENTIAL_YLMOFG')
      END IF
