...
!     ******************************************************************
      ...
      REAL(8)                :: R1,DEX
!     ******************************************************************
                              CALL TRACE__PUSH('STATEANALYSIS')
call error__msg('routine marked for deletion')
call error__stop('STATEANALYSIS')
!
      IF(.NOT.ALLOCATED(FNL)) THEN
        CALL ERROR__MSG('FNL NOT SET')
        CALL ERROR__STOP('STATEANALYSIS')
      END IF
      CALL DYNOCC__GETI4('NB',NB1)
      CALL DYNOCC__GETI4('NKPT',NKPT1)
      CALL DYNOCC__GETI4('NSPIN',NSPIN1)
      IF(NB.NE.NB1.OR.NKPT.NE.NKPT1.OR.NSPIN.NE.NSPIN1) THEN
        CALL ERROR__I4VAL('NB    ',NB)
        CALL ERROR__I4VAL('NB1   ',NB1)
        CALL ERROR__I4VAL('NKPT  ',NKPT)
        CALL ERROR__I4VAL('NKPT1 ',NKPT1)
        CALL ERROR__I4VAL('NSPIN ',NSPIN)
        CALL ERROR__I4VAL('NSPIN1',NSPIN1)
...
!     ==================================================================
                              CALL TRACE__PASS('DIAGONALIZE HAMILTONIAN')
      ...
      DO ISPIN=1,NSPIN
        ...
        DO IKPT=1,NKPT
          ...
          CALL WAVES__GETR8A('<PSI|H|PSI>',NB*NB,HAMLTN(1,1,IKPT,ISPIN))
          CALL LIB__DIAGR8(NB,HAMLTN(1,1,IKPT,ISPIN) &
...
!         ENDDO
        ENDDO
      ENDDO
      CALL OCCUPATION__SET('EIG',NB*NKPT*NSPIN,EIG)
      DEALLOCATE(HAMLTN)
!
!     ==================================================================
!     ==  CALCULATE OVERLAP MATRIX OF AE PARTIAL WAVES                ==
!     ==================================================================
                              CALL TRACE__PASS('CALCULATE OVERLAP')
      ...
      DO ISP=1,NSP
        ...
        CALL SETUP__AEZ(ISP,AEZ(ISP))
        CALL SETUP__LNX(ISP,LNX(ISP))
        CALL SETUP__LMNX(ISP,LMNX(ISP))
        LMNXX1=MAX(LMNXX1,LMNX(ISP))
        LNXX=MAX(LNXX,LNX(ISP))
      ENDDO
      IF(LMNXX1.NE.LMNXX) THEN
        CALL ERROR__MSG('INCONSISTENCY OF LMNXX')
        CALL ERROR__STOP('STATEANALYSIS')
      END IF
      ALLOCATE(AEPHI(NR,LNXX,NSP))
      ALLOCATE(LOX(LNXX,NSP))
      DO ISP=1,NSP
        CALL SETUP__LOFLN(ISP,LNX(ISP),LOX(1,ISP))
        CALL SETUP__AEPARTIALWAVES(ISP,NR,LNX(ISP),AEPHI(1,1,ISP))
      ENDDO
      NRX=NR
                              CALL TRACE__PASS('CALCULATE OVERLAPG')
      CALL PDOS(NRX,LNXX,LMNXX,NB,NKPT,NB,NKPT,NSPIN &
     &         ,ISPECIES &
     &         ,NSP,AEZ,R1,DEX,NR,LNX,LOX,AEPHI &
     &         ,ZR,NAT,NB,FNL,EIG)
                              CALL TRACE__PASS('CALCULATE OVERLAPH')
      DEALLOCATE(LOX)
      DEALLOCATE(AEZ)
      DEALLOCATE(LNX)
      DEALLOCATE(LMNX)
...
!
!     .................................................................
      SUBROUTINE STATEANALYSIS__SETPROJECTIONS(NAT_,NB_,LMNXX_,NKPT_,NSPIN_ &
     &                   ,FNL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
...
!     ******************************************************************
call error__msg('routine marked for deletion')
call error__stop('STATEANALYSIS__SETPROJECTIONS')
      NAT=NAT_
      NB=NB_
      LMNXX=LMNXX_
      NKPT=NKPT_
...
!     ******************************************************************
                              CALL TRACE__PUSH('PDOS')
call error__msg('routine marked for deletion')
call error__stop('pdos')
!
!     ==================================================================
...
!     ==================================================================
      ...
      IF(TAMPFUL.AND.(THISTASK.EQ.1)) THEN
        ...
        DO IAT=1,NAT
          ISP=ISPECIES(IAT)
          CALL ATOMLIST__GETCH('NAME',IAT,STRING)
          CALL ATOMLIST__GETR8A('R(0)',IAT,3,RPOS)
          LMAX=0
          DO LN=1,LNX(ISP)
...
!     ==   SELECT PRINTOUT                                            ==
!     ==================================================================
      ENTRY PDOS__SELECT(TPDOS1,TAMP1,TWEIGHT1,TAMPFUL1)
        TPDOS=TPDOS1
        TAMP=TAMP1
        TAMPFUL=TAMPFUL1
        TWEIGHT=TWEIGHT1
...
!     ******************************************************************
                              CALL TRACE__PUSH('PDOS_WOFL')
call error__msg('routine marked for deletion')
call error__stop('STATEANALYSIS_wofl')
!
!     ==================================================================
!     ==  CALCULATE OVERLAP MATRIX OF AE PARTIAL WAVES                ==
!     ==================================================================
...
!     ******************************************************************
call error__msg('routine marked for deletion')
call error__stop('STATEANALYSIS_overcov')
      XEXP=DEXP(DEX)
      DO ISP=1,NSP
        IZ=NINT(AEZ(ISP))
!
...
!           CALL PERIODICTABLEI4(IZ,'NODE(S)',NODE)
            CALL PERIODICTABLE__GET(IZ,'#NODES(S)',NODE)
          ELSE IF(L.EQ.1) THEN
!           CALL PERIODICTABLEI4(IZ,'NODE(P)',NODE)
            CALL PERIODICTABLE__GET(IZ,'#NODES(P)',NODE)
          ELSE IF(L.EQ.2) THEN
!           CALL PERIODICTABLEI4(IZ,'NODE(D)',NODE)
            CALL PERIODICTABLE__GET(IZ,'#NODES(D)',NODE)
          ELSE IF(L.EQ.3) THEN
!           CALL PERIODICTABLEI4(IZ,'NODE(F)',NODE)
            CALL PERIODICTABLE__GET(IZ,'#NODES(F)',NODE)
          ELSE
            NODE=0
          END IF
...
!       CALL PERIODICTABLER8(IZ,'R(ASA)',RCOV)
        CALL PERIODICTABLE__GET(IZ,'R(ASA)',RCOV)
        ...
        DO LN1=1,LNX(ISP)
          ...
          DO LN2=LN1,LNX(ISP)
            ...
            IF(L1.EQ.L2) THEN
              ...
              ENDDO
              CALL RADIAL__INTEGRAL(R1,DEX,IOUT,AUX,VAL1)
              CALL RADIAL__INTEGRAL(R1,DEX,IOUT+1,AUX,VAL2)
