...
!**                                                                   **
!**  FUNCTIONS:                                                       **
!**    FILEHANDLER__SETROOT(ROOTNAME)                                  **
!**    FILEHANDLER__UNIT(ID,STRING)                                    **
!**    FILEHANDLER__SETFILE(ID,EXT,FILENAME)                           **
!**    FILEHANDLER__SETSPECIFICATION(ID,SPEC,VALUE)                    **
!**    FILEHANDLER__CLOSE(ID)                                          **
!**    FILEHANDLER__CLOSEALL                                           **
!**    FILEHANDLER__PRINTFILEOFUNIT(UNIT)                              **
!**    FILEHANDLER__REPORT(NFIL,STRING)                                **
!**    FILEHANDLER__FILENAME(ID,NAME)                                  **
!**                                                                   **
!**  REMARKS                                                          **
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__SETROOT(STRING_)
!     ==================================================================
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__UNIT(ID_,UNIT_)
!     ==================================================================
...
!     ******************************************************************
      ...
      IF(IFIL.EQ.0) THEN
        CALL ERROR__MSG('FILE IDENTIFIER NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',trim(ID_))
        CALL ERROR__STOP('FILEHANDLER__UNIT')
      END IF
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__SETFILE(ID_,EXT_,NAME_)
!     ==================================================================
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__SETSPECIFICATION(ID_,SPEC_,VALUE_)
!     ==================================================================
!     ==  SET SPECIFICATIONS                                          ==
...
!     ******************************************************************
      ...
      CALL FILEHANDLER_LOOKUP(ID_,IFIL)
      IF(IFIL.EQ.0) THEN
        CALL ERROR__MSG('IDENTIFIER NOT IN THE LIST')
        CALL ERROR__CHVAL('IDENTIFIER',ID_)
        CALL ERROR__STOP('FILEHANDLER__SETSPECIFICATION')
      END IF
!     ==================================================================
...
!         ==  AND CREATE A NEW FILE =====================================
        ELSE
          CALL ERROR__MSG('VALUE FOR SPECIFICATION STATUS NOT RECOGNIZED')
          CALL ERROR__CHVAL('IDENTIFIER',VALUE_)
          CALL ERROR__CHVAL('FILE%ID',FILE(IFIL)%ID)
          CALL ERROR__CHVAL('FILE%NAME',FILE(IFIL)%NAME)
          CALL ERROR__STOP('FILEHANDLER__SETSPECIFICATION')
        END IF
!     ==================================================================
...
!     ==================================================================
      ELSE IF(SPEC_.EQ.'POSITION') THEN
        ...
        ELSE
          CALL ERROR__MSG('VALUE FOR SPECIFICATION POSITION NOT RECOGNIZED')
          CALL ERROR__CHVAL('IDENTIFIER',VALUE_)
          CALL ERROR__CHVAL('FILE%ID',FILE(IFIL)%ID)
          CALL ERROR__CHVAL('FILE%NAME',FILE(IFIL)%NAME)
          CALL ERROR__STOP('FILEHANDLER__SETSPECIFICATION')
...
!     ==================================================================
      ELSE IF(SPEC_.EQ.'ACTION') THEN
        ...
        ELSE
          CALL ERROR__MSG('VALUE FOR SPECIFICATION ACTION NOT RECOGNIZED')
          CALL ERROR__CHVAL('IDENTIFIER',VALUE_)
          CALL ERROR__CHVAL('FILE%ID',FILE(IFIL)%ID)
          CALL ERROR__CHVAL('FILE%NAME',FILE(IFIL)%NAME)
          CALL ERROR__STOP('FILEHANDLER__SETSPECIFICATION')
...
!     ==================================================================
      ELSE IF(SPEC_.EQ.'FORM') THEN
        ...
        ELSE
          CALL ERROR__MSG('VALUE FOR SPECIFICATION FORM NOT RECOGNIZED')
          CALL ERROR__CHVAL('IDENTIFIER',VALUE_)
          CALL ERROR__CHVAL('FILE%ID',FILE(IFIL)%ID)
          CALL ERROR__CHVAL('FILE%NAME',FILE(IFIL)%NAME)
          CALL ERROR__STOP('FILEHANDLER__SETSPECIFICATION')
...
!     ==================================================================
      ELSE
        CALL ERROR__MSG('SPECIFICATION NOT RECOGNIZED')
        CALL ERROR__CHVAL('SPEC',SPEC_)
        CALL ERROR__CHVAL('FILE%ID',FILE(IFIL)%ID)
        CALL ERROR__CHVAL('FILE%NAME',FILE(IFIL)%NAME)
        CALL ERROR__STOP('FILEHANDLER__SETSPECIFICATION')
      END IF
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__CLOSE(ID_)
!     ==================================================================
!     ==  CLOSE                                                       ==
!     ==================================================================
      USE FILEHANDLER_MODULE
...
!     ******************************************************************
      ...
      IF(IFIL.EQ.0) THEN
        CALL ERROR__MSG('FILE IDENTIFIER NOT RECOGNIZED')
        CALL ERROR__CHVAL('STRING=',ID_)
        CALL ERROR__STOP('FILEHANDLER__CLOSE')
      END IF
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__CLOSEALL
!     ==================================================================
!     ==  CLOSEALL                                                    ==
!     ==================================================================
      USE FILEHANDLER_MODULE
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__PRINTFILEOFUNIT(UNIT_)
!     ==================================================================
!     ==  SAVE                                                        ==
!     ==================================================================
      IMPLICIT NONE
...
!
!     .................................................................
      SUBROUTINE FILEHANDLER__REPORT(NFIL_,STRING_)
!     ==================================================================
...
!     ==================================================================
!     .................................................................
      SUBROUTINE FILEHANDLER__FILENAME(ID_,NAME_)
!     ******************************************************************
!     ******************************************************************
      USE FILEHANDLER_MODULE
      IMPLICIT NONE
...
!     ==================================================================
      ...
      ELSE
        ...
        DO I=1000,10000
          ...
          END IF
        ENDDO
        CALL ERROR__MSG('NO FORTRAN FILE UNIT NUMBER AVAILABLE')
        CALL ERROR__STOP('FILEHANDLER_OPEN')
 200    CONTINUE
      END IF
!
!     ==================================================================
...
!     ==================================================================
      ...
      END IF
      IF(OD) THEN
        CALL ERROR__MSG('ANOTHER FILE WITH THE SAME NAME')
        CALL ERROR__MSG('IS ALREADY OPENED')
        CALL ERROR__CHVAL('FILE ID',FILE_%ID)
        CALL ERROR__CHVAL('FILENAME',FILE_%NAME)
        CALL ERROR__STOP('FILEHANDLER_OPEN')
      END IF
!
!     ==================================================================
...
!     FILE_%POSITION='ASIS'
      ...
      RETURN
 9999 CONTINUE
      CALL FILEHANDLER__IOSTATMESSAGE(IOS,IOSTATMESSAGE)
      CALL ERROR__MSG('IO ERROR')
      CALL ERROR__I4VAL('IOS ',IOS)
      CALL ERROR__I4VAL('IERR ',IERR)
      CALL ERROR__CHVAL('FILE ID',FILE_%ID)
      CALL ERROR__CHVAL('FILENAME ',FILE_%NAME)
      CALL ERROR__CHVAL('POSITION ',FILE_%POSITION)
      CALL ERROR__CHVAL('IOS ',IOSTATMESSAGE)
      CALL ERROR__STOP('FILEHANDLER_OPEN')
      END
!
!     ..................................................................
...
!
!     ..................................................................
      SUBROUTINE FILEHANDLER__IOSTATMESSAGE(IOS,X)
!     ******************************************************************
!     **                                                              **
...
!     **    FILEHANDLER__IOSTATMESSAGE(IOS,X) AND AN EXPLICIT          **
...
!     ******************************************************************
      ...
      CHARACTER(128)           :: X
!     ******************************************************************
      CALL FILEHANDLER__IOSTATMESSAGE(IOS,X)
