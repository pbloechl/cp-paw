...
!
!     ..................................................................
      SUBROUTINE GRAPHICS__GETLIST(LL_GRPHCS_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE GRAPHICS_MODULE
...
!     ******************************************************************
      IF(.NOT.TINI) THEN
        TINI=.TRUE.
        CALL LINKEDLIST__NEW(LL_GRPHCS)
        CALL LINKEDLIST__SET(LL_GRPHCS,'LINKEDLISTNAME',0,'GRAPHICS')
      END IF
...
!
!     ..................................................................
      SUBROUTINE GRAPHICS__SETL4(ID_,VAL_)
      USE GRAPHICS_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('GRAPHICS__SETL4')
      END IF
      RETURN
      END
!
!     ....................................................................
      SUBROUTINE GRAPHICS__SETPWPOT(ID,NGL_,VHARTREE)
!     ********************************************************************
...
!set potential
      NGL=NGL_
print*,'marke 1'
      CALL LINKEDLIST__SELECT(LL_GRPHCS,'~')
print*,'marke 2'
      CALL LINKEDLIST__NLISTS(LL_GRPHCS,'IMAGE',NPICS)
print*,'marke 3'
      tpot=.false.
      DO IPIC=1,NPICS
        CALL LINKEDLIST__SELECT(LL_GRPHCS,'~')
        CALL LINKEDLIST__SELECT(LL_GRPHCS,'IMAGE',IPIC)
        CALL LINKEDLIST__GET(LL_GRPHCS,'TYPE',1,TYPE)
        if(type.eq.'POT') then
...
!
!     ....................................................................
      SUBROUTINE GRAPHICS__SET1CPOT(IDENT_,IAT_,R1,DEX,NR,NRX_,LMRX,POT)
!     ********************************************************************
...
!set potential
      ...
      DO IPIC=1,NPICS
        ...
        IF(TYPE.EQ.'POT') THEN
          ...
          ELSE
            CALL ERROR__MSG('ID MUST BE WITHER "AE" OR "PS"')
            CALL ERROR__STOP('GRAPHICS__SET1CPOT')
          END IF
        end if
      enddo
      RETURN
...
!     ******************************************************************
      IF(.NOT.TINI) RETURN
                              CALL TRACE__PUSH('GRAPHICS__PLOT')
!
!     =================================================================
...
!     =================================================================
      ...
      ALLOCATE(Q(NAT))
      DO IAT=1,NAT
        CALL ATOMLIST__GETCH('NAME',IAT,ATOMNAME(IAT))
        CALL ATOMLIST__GETR8A('R(0)',IAT,3,POS(:,IAT))
        CALL ATOMLIST__GETR8('Z',IAT,Z(IAT))
        CALL ATOMLIST__GETR8('Q',IAT,Q(IAT))
      ENDDO
!
!     =================================================================
...
!     =================================================================
      CALL WAVES__GETI4('NR1',NR1)
      CALL WAVES__GETI4('NR1L',NR1L)
      CALL WAVES__GETI4('NR2',NR2)
      CALL WAVES__GETI4('NR3',NR3)
      NNR=NR1*NR2*NR3
      NNRL=NR1L*NR2*NR3
      CALL WAVES__GETI4('NR1START',NR1START)
      CALL WAVES__GETI4('NB',NB)
      CALL WAVES__GETI4('NKPT',NKPT)
      CALL WAVES__GETI4('NSPIN',NSPIN)
      CALL WAVES__GETL4('RAWSTATES',SAVETRAWSTATES)
!
!     =================================================================
!     ==  GET GENERIC FROM SETUPS OBJECT                             ==
!     =================================================================
      CALL SETUP__LMNXX(LMNXX)
...
!     =================================================================
                              CALL TRACE__PASS('MAKE PICTURES')
      CALL LINKEDLIST__SELECT(LL_GRPHCS,'~')
      CALL LINKEDLIST__NLISTS(LL_GRPHCS,'IMAGE',NPICS)
      DO IPIC=1,NPICS
                              CALL TRACE__PASS('NEW PICTURE')
        CALL LINKEDLIST__SELECT(LL_GRPHCS,'~')
        CALL LINKEDLIST__SELECT(LL_GRPHCS,'IMAGE',IPIC)
        CALL LINKEDLIST__GET(LL_GRPHCS,'TITLE',1,TITLE)
        CALL LINKEDLIST__GET(LL_GRPHCS,'FILE',1,FILE)
        CALL LINKEDLIST__GET(LL_GRPHCS,'TYPE',1,TYPE)
        CALL LINKEDLIST__GET(LL_GRPHCS,'FORMAT',1,FORMAT)
        CALL LINKEDLIST__GET(LL_GRPHCS,'WEIGHTING',1,WEIGHTING)
!
!       =================================================================
...
!       =================================================================
        IF(TRIM(WEIGHTING).EQ.'TOTAL'.OR.TRIM(WEIGHTING).EQ.'SPIN') THEN
          CALL WAVES__SETL4('RAWSTATES',.TRUE.)
PRINT*,'TITLE ',TRIM(TITLE),.TRUE.
        ELSE
          CALL WAVES__SETL4('RAWSTATES',.FALSE.)
...
!       ================================================================
                              CALL TRACE__PASS('PSEUDO WAVE FUNCTIONS')
        IF(TRIM(TYPE).EQ.'WAVE') THEN
          CALL LINKEDLIST__SELECT(LL_GRPHCS,'STATE')
          CALL LINKEDLIST__GET(LL_GRPHCS,'IB',1,IB)
          CALL LINKEDLIST__GET(LL_GRPHCS,'IKPT',1,IKPT)
          CALL LINKEDLIST__GET(LL_GRPHCS,'ISPIN',1,ISPIN)
          CALL LINKEDLIST__EXISTD(LL_GRPHCS,'TIM',1,TCHK)
          IF(TCHK) THEN
            CALL LINKEDLIST__GET(LL_GRPHCS,'TIM',1,TIM)
          ELSE
            TIM=.FALSE.
          END IF
          CALL LINKEDLIST__SELECT(LL_GRPHCS,'..')
          CALL WAVES__SETI4('IB',IB)
          CALL WAVES__SETI4('IKPT',IKPT)
          CALL WAVES__SETI4('ISPIN',ISPIN)
          CALL WAVES__SETL4('TIM',TIM)
          CALL WAVES__GETR8A('PSPSI',NNRL,WAVE)
        ELSE IF(TYPE(1:7).EQ.'DENSITY') THEN
          ...
          ELSE IF(TRIM(WEIGHTING).EQ.'NONE') THEN
            ...
            TSPIN=.FALSE.
          ELSE
            CALL ERROR__MSG('INVALID OPTION OF WEIGHTING')
            CALL ERROR__CHVAL('WEIGHTING',WEIGHTING)
            CALL ERROR__MSG('ALLOWED VALUES: "TOTAL", "SPIN", "NONE".')
            CALL ERROR__STOP('GRAPHICS__PLOT')
          END IF
          IF(TRIM(FORMAT).EQ.'ALL'.OR.TRIM(FORMAT).EQ.'RANGE') THEN
            IF(TRIM(FORMAT).EQ.'RANGE') THEN
              CALL LINKEDLIST__NLISTS(LL_GRPHCS,'STATE',NSTATE)
              IF(NSTATE.NE.2) THEN
                CALL ERROR__MSG('RANGE MUST BE DEFINED BY AT LEAST TWO STATES')
                CALL ERROR__STOP('GRAPHICS__PLOT')
              END IF
              CALL LINKEDLIST__SELECT(LL_GRPHCS,'STATE',1)
              CALL LINKEDLIST__GET(LL_GRPHCS,'IB',1,IB1)
              CALL LINKEDLIST__GET(LL_GRPHCS,'IKPT',1,IKPT1)
              CALL LINKEDLIST__GET(LL_GRPHCS,'ISPIN',1,ISPIN1)
              CALL LINKEDLIST__SELECT(LL_GRPHCS,'..')
              CALL LINKEDLIST__SELECT(LL_GRPHCS,'STATE',2)
              CALL LINKEDLIST__GET(LL_GRPHCS,'IB',1,IB2)
              CALL LINKEDLIST__GET(LL_GRPHCS,'IKPT',1,IKPT2)
              CALL LINKEDLIST__GET(LL_GRPHCS,'ISPIN',1,ISPIN2)
              CALL LINKEDLIST__SELECT(LL_GRPHCS,'..')
            ELSE
              IB1=1
              IB2=NB
          ...
          ELSE  IF(FORMAT(1:4).EQ.'LIST') THEN
            CALL LINKEDLIST__NLISTS(LL_GRPHCS,'STATE',NSTATE)
            DO ISTATE=1,NSTATE
              CALL LINKEDLIST__SELECT(LL_GRPHCS,'STATE',ISTATE)
              CALL LINKEDLIST__GET(LL_GRPHCS,'IB',1,IB)
              CALL LINKEDLIST__GET(LL_GRPHCS,'IKPT',1,IKPT)
              CALL LINKEDLIST__GET(LL_GRPHCS,'ISPIN',1,ISPIN)
              CALL LINKEDLIST__SELECT(LL_GRPHCS,'..')
            ...
            ENDDO
          ELSE
            CALL ERROR__MSG('INVALID OPTION OF FORMAT')
            CALL ERROR__CHVAL('FORMAT',FORMAT)
            CALL ERROR__MSG('ALLOWED VALUES: "ALL", "RANGE", "LIST".')
            CALL ERROR__STOP('GRAPHICS__PLOT')
          END IF
        ELSE
          CALL ERROR__MSG('INVALID OPTION OF TYPE')
          CALL ERROR__CHVAL('TYPE',TYPE)
          CALL ERROR__MSG('ALLOWED VALUES: "DENSITY", "WAVE".')
          CALL ERROR__STOP('GRAPHICS__PLOT')
        END IF
!
!       ================================================================
...
!       ================================================================
        ...
        CALL PLANEWAVE__RSPACECOLLECTR8(NR1L*NR2*NR3,WAVE,NR1*NR2*NR3,WORK1)
...
!       ================================================================
                              CALL TRACE__PASS('ONE-CENTER EXPANSIONS')
!         DO I=1,NR1L
!           DO J=1,NR2
!             DO K=1,NR3
!               WAVE(I,J,K)=0.D0
...
!         __ GET POSITIONS______________________________________________
          CALL ATOMLIST__GETR8A('R(0)',IAT,3,BAS)
...
!         __ GET PROJECTIONS____________________________________________
          IF(TRIM(TYPE).EQ.'WAVE') THEN
            ...
            CALL WAVES__SETL4('TIM',TIM)
            ALLOCATE(PROJ(LMNXX))
            CALL WAVES__GETR8A('<PSPSI|PRO>',LMNXX,PROJ)
            LX=0
            DO LN=1,LNX
              LX=MAX(LX,LOX(LN))
            ENDDO
          ...
          ELSE IF(TRIM(TYPE).EQ.'DENSITY') THEN
            ...
            IF(TRIM(FORMAT).EQ.'ALL'.OR.TRIM(FORMAT).EQ.'RANGE') THEN
              IF(TRIM(FORMAT).EQ.'RANGE') THEN
                CALL LINKEDLIST__NLISTS(LL_GRPHCS,'STATE',NSTATE)
                IF(NSTATE.NE.2) THEN
                  CALL ERROR__STOP('RANGE MUST BE DEFINED &
...
!         ALLOCATE(WORK(1))
!       END IF
!       CALL PLANEWAVE__RSPACECOLLECTR8(NR1L*NR2*NR3,WAVE,NR1*NR2*NR3,WORK)
        IF(NTASKID.EQ.1) THEN
          STRING=' '
          WRITE(STRING,FMT=*)IPIC
          STRING='WAVEPLOT'//ADJUSTL(STRING)
          PRINT*,'FILE ',STRING,'::',TRIM(FILE)
          CALL FILEHANDLER__SETFILE(STRING,.FALSE.,TRIM(FILE))
          CALL FILEHANDLER__SETSPECIFICATION(STRING,'FORM','UNFORMATTED')
          CALL FILEHANDLER__UNIT(STRING,NFIL)
!         CALL WRITEWAVEPLOT(NFIL,TITLE,RBAS,NAT,POS,Z,Q,ATOMNAME &
!   &                       ,NR1,NR2,NR3,WORK)
          CALL WRITEWAVEPLOT(NFIL,TITLE,RBAS,NAT,POS,Z,Q,ATOMNAME &
    &                       ,NR1*fact,NR2*fact,NR3*fact,wavebig)
          CALL FILEHANDLER__CLOSE(STRING)
        END IF
 9000   CONTINUE
      ENDDO
      CALL WAVES__SETL4('RAWSTATES',SAVETRAWSTATES)
      DEALLOCATE(WAVE)
      DEALLOCATE(WAVEBIG)
      DEALLOCATE(OCC)
      DEALLOCATE(ATOMNAME)
...
!     ******************************************************************
      ...
      REAL(8)   ,ALLOCATABLE  :: PROJ(:) ! (LMNXX)  POINTER (__PROJ,PROJ)
      INTEGER(4)              :: LMN1,LMN2
!     ******************************************************************
      CALL WAVES__SETI4('IB',IB)
      CALL WAVES__SETI4('IKPT',IKPT)
...
!     ******************************************************************
      ...
      CALL WAVES__SETI4('ISPIN',ISPIN)
      ALLOCATE(PSI(NNR))
      CALL WAVES__GETR8A('PSPSI',NNR,PSI)
      DO IR=1,NNR
...
!     ******************************************************************
      XEXP=DEXP(DEX)
      IF(LMXX.LT.LMX) THEN
        CALL ERROR__MSG('INCREASE DIMENSION LMXX')
        CALL ERROR__STOP('GRAPHICS_RHOLTOR')
      END IF
!
!     ==================================================================
!     ==  DETERMINE RMAX                                              ==
          ...
          DO I2=MIN2,MAX2
            ...
            DO I3=MIN3,MAX3
              ...
              IF(DIS2.LE.RMAX2) THEN
                ...
                SVAR=0.D0
                DO LM=1,LMX
                  CALL RADIAL__VALUE(R1,DEX,NRX,DRHOL(1,LM),DIS,SVAR1)
...
!     ******************************************************************
      ...
      CALL LIB__3DFFTC8('RTOG',NR1,NR2,NR3,WORKC1,WORKC2)
      DEALLOCATE(WORKC1)
      ...
      CALL LIB__3DFFTC8('GTOR',FACT*NR1,FACT*NR2,FACT*NR3,WORKC1,WORKC2)
...
!
      ...
      CALL PLANEWAVE__GETI4('NR1L',NR1L)
      CALL PLANEWAVE__GETI4('NR1',NR1)
      CALL PLANEWAVE__GETI4('NR2',NR2)
      CALL PLANEWAVE__GETI4('NR3',NR3)
!
!     ==================================================================
...
!     ==================================================================
      ...
      ALLOCATE(WORK(NR1*NR2*NR3))
      CALL PLANEWAVE__SUPFFT('GTOR',1,NGL,PWPOT,NRL,VHARTREE)
      CALL PLANEWAVE__RSPACECOLLECTR8(NR1L*NR2*NR3,VHARTREE,NR1*NR2*NR3,WORK)
      DEALLOCATE(VHARTREE)
...
!     ==================================================================
      ...
      CALL MPE__COMBINE('+',onecPOT)
      DO IAT=1,NAT
        CALL SETUP__RADGRID(IAT,R1,DEX,NR)
