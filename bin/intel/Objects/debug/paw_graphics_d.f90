...
!
!     ..................................................................
      SUBROUTINE GRAPHICS__SETI4(ID,VAL)
!     ******************************************************************
!     **  PLOT                                                        **
!     ******************************************************************
      USE GRAPHICS_MODULE
...
!     ******************************************************************
      IF(ID.EQ.'NWAVE') THEN
        IF(ALLOCATED(WAVEPLOT)) THEN
          CALL ERROR__MSG('WAVEPLOT ALREADY ALLOCATED')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('GRAPHICS__SETI4')
        END IF
        NWAVE=VAL
...
!
      ELSE IF(ID.EQ.'NDENSITY') THEN
        IF(ALLOCATED(DENSITYPLOT)) THEN
          CALL ERROR__MSG('DENSITYPLOT ALREADY ALLOCATED')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('GRAPHICS__SETI4')
        END IF
        NDENSITY=VAL
...
!
      ELSE IF(ID.EQ.'IB') THEN
        IF(IWAVEPTR.EQ.0) THEN
          CALL ERROR__MSG('WAVE PLOT NOT SELECTED')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('GRAPHICS__SETI4')
        END IF
...
!
      ELSE IF(ID.EQ.'IBMIN') THEN
        IF(IDENSITYPTR.EQ.0) THEN
          CALL ERROR__MSG('DENSITY PLOT NOT SELECTED')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('GRAPHICS__SETI4')
        END IF
...
!
!     ..................................................................
      SUBROUTINE GRAPHICS__SETL4(ID,VAL)
!     ******************************************************************
!     **  PLOT                                                        **
!     ******************************************************************
      USE GRAPHICS_MODULE
...
!
!     ..................................................................
      SUBROUTINE GRAPHICS__SETCH(ID,VAL)
!     ******************************************************************
!     **  PLOT                                                        **
!     ******************************************************************
      USE GRAPHICS_MODULE
...
!     ******************************************************************
      IF(ID.EQ.'FILE') THEN
        ...
        ELSE IF(IPOTPTR.NE.0) THEN
          POTPLOT%FILE=VAL
        ELSE
          CALL ERROR__MSG('NEITHER WAVE OR DENSITY OR POT PLOT SELECTED')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('GRAPHICS__SETCH')
        END IF
!
...
!
!     ..................................................................
      SUBROUTINE GRAPHICS__SETR8(ID,VAL)
!     ******************************************************************
!     **  PLOT                                                        **
!     ******************************************************************
      USE GRAPHICS_MODULE
...
!     ******************************************************************
      IF(ID.EQ.'DR') THEN
        ...
        ELSE IF (IPOTPTR.NE.0) THEN
          POTPLOT%DR=VAL
        ELSE
          CALL ERROR__MSG('NEITHER WAVE OR DENSITY PLOT SELECTED')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('GRAPHICS__SETR8')
        END IF
!
...
!     ==================================================================
      ...
      DO ISPIN=1,NSPIN
        DO IKPT=1,NKPT
          CALL WAVES__GETR8A('<PSI|H|PSI>',NB,EB(:,IKPT,ISPIN))
          CALL WAVES__GETR8A('EIGVAL',NB,EIGVAL(:,IKPT,ISPIN))
        ENDDO
      ENDDO
!
      DO I=1,NDENSITY
          ...
          DO ISPIN=1,NSPIN
            DO IKPT=1,NKPT
              ...
              DO IB=1,NB
                ...
                END IF
                IF(E.LT.ELAST) THEN
                  CALL ERROR__MSG('BAND ENERGIES NOT IN INCREASING ORDER ')
...
!
        ELSE IF(SELECTOR.EQ.'NONE') THEN
          ...
          IBMAX(:,:)=NB
        ELSE
          CALL ERROR__MSG('SELECTOR NOT RECOGNIZED')
          CALL ERROR__CHVAL('SELECTOR',SELECTOR)
          CALL ERROR__STOP('GRAPHICS__PLOT')
        END IF
...
!     ******************************************************************
      ...
      INTEGER(4)                :: NFIL
!     ******************************************************************
                              CALL TRACE__PUSH('GRAPHICS_WAVEPLOT')
...
!     =================================================================
      ...
      CALL WAVES__GETI4('NKPT',NKPT)
      CALL WAVES__GETI4('NSPIN',NSPIN)
      CALL WAVES__GETL4('RAWSTATES',SAVETRAWSTATES)  ! USED TO RESET WAVE OBJECT INTO ORIGINAL STATE
!
      CALL WAVES__SETL4('RAWSTATES',.FALSE.)
      CALL WAVES__SETI4('IB',IB)
      CALL WAVES__SETI4('IKPT',IKPT)
      CALL WAVES__SETI4('ISPIN',ISPIN)
      CALL WAVES__SETL4('TIM',TIMAG)
!
!     =================================================================
...
!     ================================================================
      ...
      IF(FACT.EQ.1) THEN
        CALL PLANEWAVE__RSPACECOLLECTR8(NR1L*NR2*NR3,WAVE,NR1*NR2*NR3,WAVEBIG)
      ELSE
...
!     ================================================================
      ...
      IF(NTASKID.EQ.1) THEN
        CALL FILEHANDLER__SETFILE('WAVEPLOT',.FALSE.,TRIM(FILE))
        CALL FILEHANDLER__SETSPECIFICATION('WAVEPLOT','FORM','UNFORMATTED')
        CALL FILEHANDLER__UNIT('WAVEPLOT',NFIL)
        CALL WRITEWAVEPLOT(NFIL,TITLE,RBAS,NAT,POS,Z,Q,ATOMNAME &
    &                     ,NR1*FACT,NR2*FACT,NR3*FACT,WAVEBIG)
        CALL FILEHANDLER__CLOSE('WAVEPLOT')
      END IF
!
!     ==================================================================
!     == CLOSE DOWN                                                   ==
...
!     ******************************************************************
      ...
      INTEGER(4)                :: I
!     ******************************************************************
                              CALL TRACE__PUSH('GRAPHICS_DENSITYPLOT')
      CALL MPE__QUERY(NTASKNUM,NTASKID)
!
!     =================================================================
!     ==  GET GENERIC INFORMATION ABOUT NUMBER AND SIZE OF THE       ==
...
!     =================================================================
      ...
      CALL WAVES__GETI4('NR1START',NR1START)
      CALL WAVES__GETI4('NB',NB)
      CALL WAVES__GETI4('NKPT',NKPT_)
      CALL WAVES__GETI4('NSPIN',NSPIN_)
      IF(MAXVAL(IBMAX).GT.NB.OR.MINVAL(IBMIN).LT.1) THEN
        CALL ERROR__MSG('BANDS OUT OF RANGE')
        CALL ERROR__STOP('GRAPHICS_DENSITYPLOT')
      END IF
      IF(NKPT_.NE.NKPT.OR.NSPIN_.NE.NSPIN) THEN
        CALL ERROR__MSG('NKPT OR NSPIN INCONSISTENT')
        CALL ERROR__STOP('GRAPHICS_DENSITYPLOT')
      END IF
...
!     =================================================================
      CALL WAVES__SETL4('RAWSTATES',.NOT.TDIAG)
!
!     =================================================================
!     ==  FACTOR FOR GRID REFINEMENT                                 ==
!     =================================================================
...
!     ********************************************************************
      ...
      IF(NGL.NE.0.AND.NGL.NE.NGL_) THEN
        ...
        CALL ERROR__I4VAL('NGL',NGL)
        CALL ERROR__I4VAL('NGL_',NGL_)
        CALL ERROR__STOP('GRAPHICS__SETPWPOT')
      END IF
      NGL=NGL_
...
!     ==================================================================
      ...
      IF(FACT.EQ.1) THEN
        CALL PLANEWAVE__RSPACECOLLECTR8(NR1L*NR2*NR3,VHARTREE,NR1*NR2*NR3,POTENTIAL)
      ELSE
