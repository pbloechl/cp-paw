...
!**                                                                   **
!**  FUNCTIONS:                                                       **
!**    TRAJECTORYIO__INITIALIZE(NSTEP)                                 **
!**    TRAJECTORYIO__ON(ID,TON)                                        **
!**    TRAJECTORYIO__ADD(ID,NFI,TIME,NSIZE,ARRAY)                      **
!**    TRAJECTORYIO__FLUSH                                             **
!**                                                                   **
!**  TYPICAL USE:                                                     **
!**    1) ADD INFORMATION FOR EACH TIME STEP AND EACH TRAJECTORY      **
!**       USING TRAJECTORY__ADD (NO INITIALIZATION REQUIRED)           **
...
!**                                                                   **
!**  REMARKS:                                                         **
!**    1) TRAJECTORYIO__INITIALIZE IS OPTIONAL AND CAN BE CALLED       **
!**       ANY TIME. (IT ONLY RESETS THE SIZE OF WORK ARRAYS)          **
!**                                                                   **
!**    2) TRAJECTORYIO__ON ALLOWS TO SWITCH OF SOME TRAJECTORIES       **
!**                                                                   **
!**  DEPENDENCIES:                                                    **
!**    ERROR                                                          **
!**    FILEHANDLER                                                    **
...
!     == WRITE DATA TO FILE                                           ==
!     ==================================================================
      CALL FILEHANDLER__UNIT(TRIM(XXX%ID),NFIL)
      DO I=1,XXX%I-1
        WRITE(NFIL)XXX%ISTEP(I),XXX%TIME(I),XXX%NSIZE,XXX%ARRAY(:,I)
      ENDDO
      CALL FILEHANDLER__CLOSE(TRIM(XXX%ID))
!
!     ==================================================================
!     == RESET ARRAYS                                                 ==
!     ==================================================================
...
!
!     ..................................................................
      SUBROUTINE TRAJECTORYIO__INITIALIZE(NSTEP_)
!     ******************************************************************
!     **  INITIALIZE                                                  **
...
!
!     ..................................................................
      SUBROUTINE TRAJECTORYIO__ON(ID_,TON_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE TRAJECTORYIO__ADD(ID_,NFI_,TIME_,NSIZE_,ARRAY_)
!     ******************************************************************
!     **  ADD                                                         **
!     ******************************************************************
      USE TRAJECTORYIO_MODULE
...
!     ==================================================================
      IF(NSIZE_.NE.XXX%NSIZE)THEN
        CALL ERROR__MSG('SIZE MISMATCH ERROR')
        CALL ERROR__CHVAL('ID',TRIM(XXX%ID))
        CALL ERROR__I4VAL('NSIZE_',NSIZE_)
        CALL ERROR__I4VAL('XXX%NSIZE',XXX%NSIZE)
        CALL ERROR__STOP('TRAJECTORYIO__ADD')
      ENDIF
      IF(XXX%I.GT.NSTEP) THEN
        CALL ERROR__MSG('STEP NUMBER OUT OF BOUNDS')
        CALL ERROR__CHVAL('ID',TRIM(XXX%ID))
        CALL ERROR__I4VAL('XXX%I',XXX%I)
        CALL ERROR__I4VAL('NSTEP',NSTEP)
        CALL ERROR__STOP('TRAJECTORYIO__ADD')
      END IF
!
!     ==================================================================
...
!
!     ..................................................................
      SUBROUTINE TRAJECTORYIO__FLUSH
!     ******************************************************************
!     ** FLUSH                                                        **
!     **                                                              **
