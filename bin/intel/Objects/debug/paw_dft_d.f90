...
!**                                                                   **
!**  USAGE:                                                           **
!**    1) SELECT A FUNCTIONAL: DFT__SETI4('TYPE',ITYPE)                **
!**    2) CHOSE SPIN (UN) RESTRICTED: DFT__SETL4('SPIN',TSPIN)         **
...
!***********************************************************************
!     ==  SWITCHES =====================================================
      INTEGER(4)           :: IT=0   ! FUNCTIONAL SELECTOR SEE DFT__REPORT
...
!     ==================================================================
      ...
      ELSE IF(IT.EQ.1) THEN
        TX=.TRUE.
        CALL EXCHANGE__SETI4('TYPE',1)  ! SELECT LOCAL EXCHANGE
        TPZ=.TRUE.
        TGRATARGET=.FALSE.
...
!     ==================================================================
      ELSE IF(IT.EQ.4) THEN
        TX=.TRUE.
        CALL EXCHANGE__SETI4('TYPE',12)  ! SELECT LOCAL EXCHANGE TIMES 1.5*0.7
        TGRATARGET=.FALSE.
...
!     ==================================================================
      ELSE
        CALL ERROR__MSG('CHOICE OF FUNCTIONAL NOT DEFINED')
        CALL ERROR__I4VAL('IT',IT)
        CALL ERROR__STOP('DFT_INITIALIZE')
      END IF
!
!     ================================================================
...
!     ================================================================
      IF(TGRA.NEQV.TGRATARGET) THEN
        CALL ERROR__MSG('FLAG FOR GRADIENTS IS NOT SET PROPERLY')
        CALL ERROR__I4VAL('IT',IT)
        CALL ERROR__L4VAL('TGRA',TGRA)
        CALL ERROR__L4VAL('TGRATARGET',TGRATARGET)
        CALL ERROR__STOP('DFT_INITIALIZE')
      END IF
!
!     ================================================================
...
!     ******************************************************************
      ...
      CALL REPORT__TITLE(NFIL,'DENSITY FUNCTIONAL')
      IF(TSPIN) THEN
        CALL REPORT__STRING(NFIL,'SPIN POLARIZED (LSD) FUNCTIONAL')
      ELSE
        CALL REPORT__STRING(NFIL,'NON-SPIN POLARIZED (LDA) FUNCTIONAL')
      END IF
      DO I=1,NDESCRIPTION
        IF(LEN(TRIM(DESCRIPTION(I))).EQ.0) CYCLE
        CALL REPORT__STRING(NFIL,TRIM(DESCRIPTION(I)))
      ENDDO
      RETURN
      END
!
!     .....................................................DFT..........
      SUBROUTINE DFT__SETI4(ID,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE DFT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      IF(ID.EQ.'TYPE') THEN
        ...
        IF(.NOT.TCHK) THEN
          CALL ERROR__MSG('DFT FUNCTIONAL SELECTION INVALID')
          CALL ERROR__I4VAL('ID',VALUE)
          CALL ERROR__STOP('DFT__SETI4')
        END IF
        TINI=.FALSE.
      ELSE
...
!
!     .....................................................DFT..........
      SUBROUTINE DFT__SETCH(ID,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE DFT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      IF(ID.EQ.'TYPE') THEN
        ...
        IF(.NOT.TCHK) THEN
          CALL ERROR__MSG('DFT FUNCTIONAL SELECTION INVALID')
          CALL ERROR__CHVAL('ID',VALUE)
          CALL ERROR__STOP('DFT__SETCH')
        END IF
        TINI=.FALSE.
      ELSE
        CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED')
...
!
!     .....................................................DFT..........
      SUBROUTINE DFT__SETL4(IDENT_,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE DFT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('DFT__SETL4')
      END IF
      RETURN
      END
!
!     .....................................................DFT..........
      SUBROUTINE DFT__GETL4(IDENT_,VALUE)
!     ******************************************************************
!     ******************************************************************
      USE DFT_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED')
        CALL ERROR__CHVAL('IDENT_',IDENT_)
        CALL ERROR__STOP('DFT__GETL4')
      END IF
      RETURN
      END
!
!     .....................................................DFT..........
      SUBROUTINE DFT__GRADIENTSWITCH(TGRA_)
!     ==================================================================
...
!     ==================================================================
      IF(TX) THEN
        CALL EXCHANGE__EVAL1(VAL,EXC1,DEXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
      END IF
!
...
!     ==================================================================
      IF(TPW91L) THEN
        CALL PERDEWWANG91L__EVAL1(VAL,EXC1,DEXC1)
        EXC=EXC+EXC1
        DEXC(:)=DEXC(:)+DEXC1(:)
      END IF
!
...
!     ==================================================================
      IF(TPZ) THEN
        CALL PERDEWZUNGER__EVAL1(VAL,EXC1,DEXC1)
        EXC=EXC+EXC1
        DEXC(:)=DEXC(:)+DEXC1(:)
      END IF
!
...
!     ==================================================================
      IF(TPERDEW) THEN
        CALL PERDEW__EVAL1(VAL,EXC1,DEXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
      END IF
!
...
!     ==================================================================
      IF(TPW91G) THEN
        DEXC1=0.D0
        CALL PERDEWWANG91G__EVAL(VAL(1),VAL(2),VAL(3),EXC1 &
...
!     ==================================================================
      IF(TPBE96) THEN
        CALL PBE__EVAL1(VAL,EXC1,DEXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
      END IF
!
...
!     ==================================================================
      IF(TLYP88) THEN
        CALL LYP88__EVAL1(VAL,EXC1,DEXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
      END IF
!
...
!
     ...
     &             .OR.GVXCT2.NE.GVXCT2.OR.GVXCS2.NE.GVXCS2) THEN
        CALL ERROR__MSG('ERROR AFTER DFT')
        CALL ERROR__L4VAL('TSPIN ',TSPIN)
        CALL ERROR__R8VAL('EXC ',EXC)
        CALL ERROR__R8VAL('RHOT ',RHOT)
        CALL ERROR__R8VAL('RHOS ',RHOS)
        CALL ERROR__R8VAL('GRHOT2 ',GRHOT2)
        CALL ERROR__R8VAL('GRHOS2 ',GRHOS2)
        CALL ERROR__R8VAL('GRHOST ',GRHOST)
        CALL ERROR__R8VAL('VXCT ',VXCT)
        CALL ERROR__R8VAL('VXCS ',VXCS)
        CALL ERROR__R8VAL('GVXCT2 ',GVXCT2)
        CALL ERROR__R8VAL('GVXCS2 ',GVXCS2)
        CALL ERROR__R8VAL('GVXCST ',GVXCST)
        CALL ERROR__STOP('DFT')
      END IF
      RETURN
      END
!
...
!     ==================================================================
      IF(TX) THEN
        CALL EXCHANGE__EVAL2(VAL,EXC1,DEXC1,D2EXC1)
        EXC=EXC+EXC1
        DEXC(:)=DEXC(:)+DEXC1(:)
        D2EXC(:,:)=D2EXC(:,:)+D2EXC1(:,:)
      END IF
...
!     ==================================================================
      IF(TPW91L) THEN
        CALL PERDEWWANG91L__EVAL2(VAL,EXC1,DEXC1,D2EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
      END IF
...
!     ==================================================================
      IF(TPZ) THEN
        CALL PERDEWZUNGER__EVAL2(VAL,EXC1,DEXC1,D2EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
      END IF
...
!     ==================================================================
      IF(TPERDEW) THEN
        CALL PERDEW__EVAL2(VAL,EXC1,DEXC1,D2EXC1)
        EXC=EXC+EXC1
        DEXC(:)=DEXC(:)+DEXC1(:)
        D2EXC(:,:)=D2EXC(:,:)+D2EXC1(:,:)
      END IF
...
!     ==================================================================
      IF(TPW91G) THEN
        CALL PERDEWWANG91G__EVAL2(VAL(1),VAL(2),VAL(3),EXC1 &
     &       ,DEXC1(1),DEXC1(2),DEXC1(3) &
...
!     ==================================================================
      IF(TPBE96) THEN
        CALL PBE__EVAL2(VAL,EXC1,DEXC1,D2EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
      END IF
...
!     ==================================================================
      IF(TLYP88) THEN
        CALL LYP88__EVAL2(VAL,EXC1,DEXC1,D2EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
      END IF
...
!     ==================================================================
      ...
      IF(EXC.NE.EXC.OR.DEXC(1).NE.DEXC(1).OR.DEXC(2).NE.DEXC(2) &
        ...
        CALL ERROR__R8VAL('VAL(1)',VAL(1))
        CALL ERROR__R8VAL('VAL(2)',VAL(2))
        CALL ERROR__R8VAL('VAL(3)',VAL(3))
        CALL ERROR__R8VAL('EXC ',EXC)
        CALL ERROR__R8VAL('DEXC(1)',DEXC(1))
        CALL ERROR__R8VAL('DEXC(2)',DEXC(2))
        CALL ERROR__R8VAL('DEXC(3)',DEXC(3))
        CALL ERROR__R8VAL('D2EXC(1,1)',D2EXC(1,1))
        CALL ERROR__R8VAL('D2EXC(1,2)',D2EXC(1,2))
        CALL ERROR__R8VAL('D2EXC(1,3)',D2EXC(1,3))
        CALL ERROR__R8VAL('D2EXC(2,2)',D2EXC(2,2))
        CALL ERROR__R8VAL('D2EXC(2,3)',D2EXC(2,3))
        CALL ERROR__R8VAL('D2EXC(3,3)',D2EXC(3,3))
        CALL ERROR__STOP('DFT2')
      END IF
      RETURN
      END
!
...
!     ==================================================================
      IF(TX) THEN
        CALL EXCHANGE__EVAL3(VAL,EXC1,DEXC1,D2EXC1,D3EXC1)
        EXC=EXC+EXC1
        DEXC(:)=DEXC(:)+DEXC1(:)
        D2EXC(:,:)=D2EXC(:,:)+D2EXC1(:,:)
        D3EXC(:,:,:)=D3EXC(:,:,:)+D3EXC1(:,:,:)
...
!     ==================================================================
      IF(TPW91L) THEN
        CALL PERDEWWANG91L__EVAL3(VAL,EXC1,DEXC1,D2EXC1,D3EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
        D3EXC=D3EXC+D3EXC1
...
!     ==================================================================
      IF(TPZ) THEN
        CALL PERDEWZUNGER__EVAL3(VAL,EXC1,DEXC1,D2EXC1,D3EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
        D3EXC=D3EXC+D3EXC1
...
!     ==================================================================
      IF(TPERDEW) THEN
        CALL PERDEW__EVAL3(VAL,EXC1,DEXC1,D2EXC1,D3EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
        D3EXC=D3EXC+D3EXC1
...
!     ==================================================================
      IF(TPBE96) THEN
        CALL PBE__EVAL3(VAL,EXC1,DEXC1,D2EXC1,D3EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
        D3EXC=D3EXC+D3EXC1
...
!     ==================================================================
      IF(TLYP88) THEN
        CALL LYP88__EVAL3(VAL,EXC1,DEXC1,D2EXC1,D3EXC1)
        EXC=EXC+EXC1
        DEXC=DEXC+DEXC1
        D2EXC=D2EXC+D2EXC1
        D3EXC=D3EXC+D3EXC1
...
!**                                                                   **
!**  FUNCTIONS:                                                       **
!**    TABLE1D__MAKE(XTABLE,XMIN,XMAX,NX                               **
!**    TABLE1D__XOFI(XTABLE,I,X)                                       **
!**    TABLE1D__SETX(XTABLE,X,TCHK)                                    **
!**    TABLE1D__GETF(XTABLE,F,FOFX)                                    **
!**                                                                   **
!**  REMARKS:                                                         **
...
CONTAINS
!     ..................................................................
      SUBROUTINE TABLE1D__MAKE(XTABLE,XMIN,XMAX,NX)
!     ******************************************************************
...
!
      ...
      XTABLE%NNONHITS=0
      RETURN
      END SUBROUTINE TABLE1D__MAKE
!     ..................................................................
      SUBROUTINE TABLE1D__XOFI(XTABLE,I,X)
!     ******************************************************************
!     **  F(X) = W1*F(I1)+W2*F(I1+1)+W3*F(I1+2)+W4*F(I1+3)            **
!     ******************************************************************
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      RETURN
      END SUBROUTINE TABLE1D__XOFI
!     ..................................................................
      SUBROUTINE TABLE1D__SETX(XTABLE,X,TCHK)
!     ******************************************************************
!     **  F(X) = W1*F(I1)+W2*F(I1+1)+W3*F(I1+2)+W4*F(I1+3)            **
!     ******************************************************************
      IMPLICIT NONE
...
!     ==================================================================
      ...
      RETURN
      END SUBROUTINE TABLE1D__SETX
!     ..................................................................
      SUBROUTINE TABLE1D__GETF(XTABLE,NX,F,FOFX)
!     ******************************************************************
!     **  F(X) = W1*F(I1)+W2*F(I1+1)+W3*F(I1+2)+W4*F(I1+3)            **
!     ******************************************************************
      IMPLICIT NONE
...
!     ******************************************************************
      IX=XTABLE%IX
      IF(IX.EQ.0) THEN
        CALL ERROR__MSG('XTABLE IS NOT POSITIONED')
        CALL ERROR__STOP('TABLE1D__GETF')
      END IF
      IF(NX.NE.XTABLE%NX) THEN
        CALL ERROR__MSG('LENGTH OF ARRAY NOT CONSISTENT WITH XTABLE')
        CALL ERROR__STOP('TABLE1D__GETF')
      END IF
!
      FOFX=DOT_PRODUCT(XTABLE%W(:),F(IX:IX+3))
      RETURN
      END SUBROUTINE TABLE1D__GETF
...
!**                                                                   **
!**  FUNCTIONS:                                                       **
!**    EXCHANGE__SETI4(ID,VAL)                                         **
!**    EXCHANGE__EVAL1(VAL,EXC,DEXC)                                   **
!**    EXCHANGE__EVAL2(VAL,EXC,DEXC,D2EXC)                             **
!**    EXCHANGE__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)                       **
!**                                                                   **
!**  METHODS:                                                         **
!**                                                                   **
!***********************************************************************
...
!       == + PBE96 GRADIENT CORRECTION CORRECT LONG-RANGE SCALING ======
      ...
      ELSE
        CALL ERROR__MSG('FUNCTIONAL SELECTION FOR EXCHANGE NOT RECOGNIZED')
        CALL ERROR__I4VAL('ITYPE',ITYPE)
        CALL ERROR__STOP('EXCHANGE_INITIALIZE (SEE DFT OBJECT)')
      END IF
!
!     =================================================================
...
!       == EX(GRHO->INFTY)=EX(GRHO=0)*KAPPA =============================
        KAPPA=0.804D0
      ELSE
        CALL ERROR__MSG('AT LEAST ONE PARAMETER SET MUST BE SELECTED')
        CALL ERROR__STOP('EXCHANGE_INITIALIZE')
      END IF
      MUBYKAPPA=MU/KAPPA
...
!     =================================================================
      IF(TGRA) THEN
        CALL TABLE1D__MAKE(S2TABLE,1.D-3,10.D0,NS2)
        DO I=1,NS2
          CALL TABLE1D__XOFI(S2TABLE,I,S2)
...
!
!     ..................................................................
      SUBROUTINE EXCHANGE__SETI4(ID,VAL_)
!     ******************************************************************
!     ******************************************************************
      USE EXCHANGE_MODULE
      IMPLICIT NONE
...
!
!     ..................................................................
      SUBROUTINE EXCHANGE__EVAL1(VAL,EXC,DEXC)
      USE EXCHANGE_MODULE
      IMPLICIT NONE
...
!
!     ..................................................................
      SUBROUTINE EXCHANGE__EVAL2(VAL,EXC,DEXC,D2EXC)
      USE EXCHANGE_MODULE
      IMPLICIT NONE
      REAL(8),INTENT(IN) :: VAL(5)     !RHOT,RHOS,GRHOT2,GRHOS2,GRHOTS
      REAL(8),INTENT(OUT):: EXC        ! EXCHANGE ENERGY (RHOT*EPS_X)
...
!
!     ..................................................................
      SUBROUTINE EXCHANGE__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)
      USE EXCHANGE_MODULE
      IMPLICIT NONE
      REAL(8),INTENT(IN) :: VAL(5)     !RHOT,RHOS,GRHOT2,GRHOS2,GRHOTS
      REAL(8),INTENT(OUT):: EXC        ! EXCHANGE ENERGY (RHOT*EPS_X)
...
!     =================================================================
      TTABLE=.NOT.TSAFE
      IF(TTABLE) CALL TABLE1D__SETX(S2TABLE,S2,TTABLE)
      IF(TTABLE) THEN
        CALL TABLE1D__GETF(S2TABLE,NS2,FXARRAY,FX)
        CALL TABLE1D__GETF(S2TABLE,NS2,DFXARRAY,FX_S)
      ELSE
...
!     =================================================================
      ...
      IF(TTABLE) THEN
        CALL TABLE1D__GETF(S2TABLE,NS2,FXARRAY,FX)
        CALL TABLE1D__GETF(S2TABLE,NS2,DFXARRAY,FX_S)
        CALL TABLE1D__GETF(S2TABLE,NS2,D2FXARRAY,FX_SS)
      ELSE
        CALL EXCHANGE_FX2(S2,FX,FX_S,FX_SS)
      END IF
      FX_R=FX_S*S2_R
...
!     =================================================================
      ...
      IF(TTABLE) THEN
        ...
        CALL TABLE1D__GETF(S2TABLE,NS2,DFXARRAY,FX_S)
        CALL TABLE1D__GETF(S2TABLE,NS2,D2FXARRAY,FX_SS)
        CALL TABLE1D__GETF(S2TABLE,NS2,D3FXARRAY,FX_SSS)
      ELSE
...
!       ===============================================================
      ...
      ELSE
        CALL ERROR__MSG('FXTYPE NOT RECOGNIZED (ALLOWED VALUES ARE "PBE", "RPBE", "BECKE")')
        CALL ERROR__CHVAL('FXTYPE',FXTYPE)
        CALL ERROR__STOP('EXCHANGE_FX1')
      END IF
      RETURN
      END
!
...
!**                                                                   **
!**  FUNCTIONS:                                                       **
!**    PERDEWZUNGER__EVAL1(VAL,EXC,DEXC)                               **
!**    PERDEWZUNGER__EVAL2(VAL,EXC,DEXC,D2EXC)                         **
!**    PERDEWZUNGER__EVAL2(VAL,EXC,DEXC,D2EXC,D3EXC)                   **
!**                                                                   **
!**  DEPENDENCIES:                                                    **
...
!
!     ..................................................................
      SUBROUTINE PERDEWZUNGER__EVAL1(VAL,EXC,DEXC)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE PERDEWZUNGER_MODULE
...
!
!     ..................................................................
      SUBROUTINE PERDEWZUNGER__EVAL2(VAL,EXC,DEXC,D2EXC)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE PERDEWZUNGER_MODULE
...
!
!     ..................................................................
      SUBROUTINE PERDEWZUNGER__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE PERDEWZUNGER_MODULE
...
!
!     ...................................................................
      SUBROUTINE BARTHHEDIN__INITIALIZE(ID_)
!     ********************************************************************
!     ********************************************************************
      USE DFT_MODULE
...
!       == PARAMETERS FOR RAJAGOPAL, SINGHAL,KIMBALL
        ...
        RFBH=70.6D0
      ELSE
        CALL ERROR__MSG('ID_ NOT RECOGNIZED')
        CALL ERROR__STOP('BARTHHEDIN_INITIALIZE')
      END IF
!     ********************************************************************
      RETURN
      END
!
!     ...................................................................
      SUBROUTINE BARTHHEDIN__EVAL(RHOT,ARRU,DARRU,ARRP,DARRP)
!     ********************************************************************
!     ********************************************************************
      USE BARTHHEDIN_MODULE
      IMPLICIT NONE
...
!**  FUNCTIONS::                                                      **
!**    PERDEW__EVAL                                                    **
!**    PERDEW__EVAL2(VAL,EXC,DEXC,D2EXC)                               **
!**    PERDEW__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)                         **
!**                                                                   **
!***********************************************************************
USE DFT_MODULE ,ONLY : TSPIN,TSAFE
USE TABLE1D_MODULE
...
!     ================================================================
      ...
      CALL TABLE1D__MAKE(RHOTABLE,1.D-3,1.D0,NRHO)
      DO I=1,NRHO
        CALL TABLE1D__XOFI(RHOTABLE,I,RHOT)
...
!     ================================================================
      ...
      CALL TABLE1D__MAKE(SIGTABLE,0.D0,1.D0-1.D-12,NSIG)
      DO I=1,NRHO
        CALL TABLE1D__XOFI(SIGTABLE,I,SIGMA)
...
!
!     ..................................................................
      SUBROUTINE PERDEW__EVAL1(VAL,EXC,VXC)
!     ******************************************************************
!     **                                                              **
...
!
!     ..................................................................
      SUBROUTINE PERDEW__EVAL2(VAL,EXC,VXC,V2XC)
!     ******************************************************************
!     **                                                              **
!     **  CALCULATES PERDEW'S 86 GRADIENT CORRECTION FOR CORRELATION  **
!     **    J.P. PERDEW PHYS. REV. B, 33, 8822 (1986)                 **
...
!
!     ..................................................................
      SUBROUTINE PERDEW__EVAL3(VAL,EXC,VXC,V2XC,V3XC)
!     ******************************************************************
!     **                                                              **
!     **  CALCULATES PERDEW'S 86 GRADIENT CORRECTION FOR CORRELATION  **
!     **    J.P. PERDEW PHYS. REV. B, 33, 8822 (1986)                 **
...
!**                                                                   **
!** FUNCTIONS:                                                        **
!**   PERDEWWANG91L__EPSVAL(RHOT,RHOS,EPS,EPS_T,EPS_S)                 **
!**   PERDEWWANG91L__EPSVAL2(RHOT,RHOS,EPS,EPS_T,EPS_S &               **
!**                        EPS_TT,EPS_TS,EPS_SS)                      **
!**   PERDEWWANG91L__EPSVAL3(RHOT,RHOS,EPS,EPS_T,EPS_S &               **
!**                        ,EPS_TT,EPS_TS,EPS_SS                      **
!**                        ,EC_TTT,EC_TTS,EC_TSS,EC_SSS)              **
!**   PERDEWWANG91L__EVAL(VAL,EXC,DEXC,D2EXC)                          **
!**   PERDEWWANG91L__EVAL2(VAL,EXC,DEXC,D2EXC)                         **
!**   PERDEWWANG91L__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)                   **
!**                                                                   **
!** DEPENDENCIES:                                                     **
...
!      ==  INITIALIZE TABLES                                         ==
!      ================================================================
       CALL TABLE1D__MAKE(RHOTABLE,1.D-3,1.D-1,NRHO)
       DO I=1,NRHO
         CALL TABLE1D__XOFI(RHOTABLE,I,RHOT)
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91L__EPSVAL(RHOT,RHOS,EC,EC_T,EC_S)
!      *****************************************************************
!      ** J.P.PERDEW AND Y. WANG, PHYS. REV. B45, 13244 (1992-I)      **
!      *****************************************************************
...
!      =================================================================
       TTABLE=.NOT.TSAFE
       IF(TTABLE) THEN
         CALL TABLE1D__SETX(RHOTABLE,RHOT,TTABLE)
       END IF
!
!      =================================================================
...
!      =================================================================
       IF(TTABLE) THEN
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECUARRAY,ECU)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECUARRAY,DECU)
       ELSE
...
!      =================================================================
       IF(TTABLE) THEN
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECPARRAY,ECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECPARRAY,DECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECXARRAY,ECX)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECXARRAY,DECX)
       ELSE
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91L__EPSVAL2(RHOT,RHOS,EC,EC_T,EC_S &
      &                        ,EC_TT,EC_TS,EC_SS)
!      *****************************************************************
!      ** J.P.PERDEW AND Y. WANG, PHYS. REV. B45, 13244 (1992-I)      **
!      *****************************************************************
...
!      =================================================================
       IF(TTABLE) THEN
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECUARRAY,ECU)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECUARRAY,DECU)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D2ECUARRAY,D2ECU)
       ELSE
...
!      =================================================================
       IF(TTABLE) THEN
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECPARRAY,ECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECPARRAY,DECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D2ECPARRAY,D2ECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECXARRAY,ECX)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECXARRAY,DECX)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D2ECXARRAY,D2ECX)
       ELSE
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91L__EPSVAL3(RHOT,RHOS,EC,EC_T,EC_S &
...
!      =================================================================
       IF(TTABLE) THEN
         ...
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECUARRAY,DECU)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D2ECUARRAY,D2ECU)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D3ECUARRAY,D3ECU)
       ELSE
         CALL PERDEWWANG91L_RHODEP3(RHOT,ECU,DECU,D2ECU,D3ECU &
      &                         ,AU,ALPHA1U,BETA1U,BETA2U,BETA3U,BETA4U)
       END IF
...
!      =================================================================
       IF(TTABLE) THEN
         ...
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECPARRAY,DECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D2ECPARRAY,D2ECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D3ECPARRAY,D3ECP)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,ECXARRAY,ECX)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,DECXARRAY,DECX)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D2ECXARRAY,D2ECX)
         CALL TABLE1D__GETF(RHOTABLE,NRHO,D3ECXARRAY,D3ECX)
       ELSE
         CALL PERDEWWANG91L_RHODEP3(RHOT,ECP,DECP,D2ECP,D3ECP &
      &                         ,AP,ALPHA1P,BETA1P,BETA2P,BETA3P,BETA4P)
!      == NOTE THAT ECX IS -ALPHA_C FOMR THE PAPER !!
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91L__EVAL1(VAL,EXC,DEXC)
       IMPLICIT NONE
       REAL(8),INTENT(IN) :: VAL(5)
       REAL(8),INTENT(OUT):: EXC
       REAL(8),INTENT(OUT):: DEXC(5)
...
!      *****************************************************************
       ...
       RHOT=VAL(1)
       RHOS=VAL(2)
       CALL PERDEWWANG91L__EPSVAL(RHOT,RHOS,EC,EC_T,EC_S)
       EXC=RHOT*EC
       DEXC(1)=RHOT*EC_T+EC
       DEXC(2)=RHOT*EC_S
       RETURN
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91L__EVAL2(VAL,EXC,DEXC,D2EXC)
       IMPLICIT NONE
       REAL(8),INTENT(IN) :: VAL(5)
       REAL(8),INTENT(OUT):: EXC
       REAL(8),INTENT(OUT):: DEXC(5)
...
!      *****************************************************************
       ...
       RHOT=VAL(1)
       RHOS=VAL(2)
       CALL PERDEWWANG91L__EPSVAL2(RHOT,RHOS,EC,EC_T,EC_S,EC_TT,EC_TS,EC_SS)
       EXC=RHOT*EC
       DEXC(1)=RHOT*EC_T+EC
       DEXC(2)=RHOT*EC_S
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91L__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)
       IMPLICIT NONE
       REAL(8),INTENT(IN) :: VAL(5)
       REAL(8),INTENT(OUT):: EXC
       REAL(8),INTENT(OUT):: DEXC(5)
...
!      *****************************************************************
       ...
       RHOT=VAL(1)
       RHOS=VAL(2)
       CALL PERDEWWANG91L__EPSVAL3(RHOT,RHOS,EC,EC_T,EC_S &
      &                  ,EC_TT,EC_TS,EC_SS &
      &                  ,EC_TTT,EC_TTS,EC_TSS,EC_SSS)
       EXC=RHOT*EC
       DEXC(1)=RHOT*EC_T+EC
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91G__EVAL(RHOT,RHOS,GRHO2,EPSC,EPSC_D,EPSC_S,EPSC_N)
...
!      ================================================================
       CALL PERDEWWANG91L__EPSVAL2(RHOT,RHOS,E,E_D,E_S,E_DD,E_DS,E_SS)
!
!      ================================================================
!      == T IS T**2 OF EQ. 10                                       ==
!      ================================================================
...
!
!      .................................................................
       SUBROUTINE PERDEWWANG91G__EVAL2(RHOT,RHOS,GRHO2,EPSC &
...
!**                                                                   **
!**  DEPENDENCIES:                                                    **
!**    PERDEWWANG91L(PERDEWWANG91L__EPSVAL1,PERDEWWANG91L__EPSVAL2      **
!**                 ,PERDEWWANG91L__EPSVAL3                            **
!**                                                                   **
!***********************************************************************
LOGICAL(4)         :: TINI=.FALSE.
REAL(8)            :: PI
...
!
!      .................................................................
       SUBROUTINE PBE__EVAL1(VAL,EXC,DEXC)
       USE PBE_MODULE
       IMPLICIT NONE
       REAL(8),INTENT(IN) :: VAL(5)
       REAL(8),INTENT(OUT):: EXC
...
!      == CALCULATE LOCAL CORRELATION                                ==
!      ================================================================
       CALL PERDEWWANG91L__EPSVAL(RHOT,RHOS,E,E_D,E_S)
!
!      ================================================================
!      == EQ.11: SPIN DEPENDENCE G                                   ==
!      ================================================================
...
!
!      .................................................................
       SUBROUTINE PBE__EVAL2(VAL,EXC,DEXC,D2EXC)
       USE PBE_MODULE
       IMPLICIT NONE
       REAL(8),INTENT(IN) :: VAL(5)
       REAL(8),INTENT(OUT):: EXC
...
!
!      .................................................................
       SUBROUTINE PBE__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)
       USE PBE_MODULE
       IMPLICIT NONE
       REAL(8),INTENT(IN) :: VAL(5)
       REAL(8),INTENT(OUT):: EXC
...
!      ================================================================
       IF(T.LT.-1.D-8) THEN
         CALL PERDEWWANG91L__EVAL3(VAL,EXC,DEXC,D2EXC,D3EXC)
         RETURN
       ELSE
         CALL PERDEWWANG91L__EPSVAL3(RHOT,RHOS,E,E_D,E_S,E_DD,E_DS,E_SS &
