...
!
!     ..................................................................
      SUBROUTINE ATOMS__SETR8(ID_,VAL_)
!     ******************************************************************
!     ******************************************************************
      USE ATOMS_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__STOP('ATOMS__SETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMS__GETR8(ID_,VAL_)
!     ******************************************************************
!     ******************************************************************
      USE ATOMS_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID_ NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID_',ID_)
        CALL ERROR__STOP('ATOMS__GETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMS__SETL4(ID_,VAL_)
!     ******************************************************************
!     ******************************************************************
      USE ATOMS_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID_ NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID_',ID_)
        CALL ERROR__STOP('ATOMS__SETL4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMS__GETL4(ID_,VAL_)
!     ******************************************************************
!     ******************************************************************
      USE ATOMS_MODULE
      IMPLICIT NONE
...
!     ******************************************************************
      ...
      CALL REPORT__TITLE(NFIL,'ATOMS')
      CALL REPORT__I4VAL(NFIL,'NUMBER OF ATOMS',NAT,' ')
      IF(.NOT.TDYN) THEN
        CALL REPORT__STRING(NFIL,'ATOMS ARE NOT PROPAGATED')
      ELSE
        CALL REPORT__STRING(NFIL,'ATOMS ARE PROPAGATED')
        IF(TSTOP) THEN
          CALL REPORT__CHVAL(NFIL,'INITIAL VELOCITIESARE SET TO ','ZERO')
        END IF
        IF(TRANDOMIZE) THEN
          CALL REPORT__R8VAL(NFIL,'INITIAL VELOCITIES RANDOMIZED WITH' &
     &                           ,AMPRE/KELVIN,'KELVIN')
        END IF
        IF(ANNER.NE.0.D0) THEN
          CALL REPORT__R8VAL(NFIL,'FRICTION',ANNER,' ')
        END IF
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMLIST__REPORT(NFIL)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      ...
      IF(THISTASK.NE.1) RETURN
      CALL CONSTANTS('U',U)
      CALL WAVES__GETR8('EMASS',EMASS)
      CALL WAVES__GETR8('EMASSCG2',EMASSCG2)
      CALL ATOMS_EFFEMASS(NAT,EMASS,EMASSCG2,PSG2,PSG4,EFFEMASS)
      CALL CELL__GETR8A('T(0)',9,RBAS)
!
      CALL REPORT__TITLE(NFIL,'ATOMLIST REPORT')
...
!     ******************************************************************
      ...
      INTEGER(4)           :: NFREE
!     ******************************************************************
                              CALL TRACE__PUSH('ATOMS__INITIALIZE')
!     CALL SHADOW__INITIALIZE
!     CALL SHADOW__OPTIMIZE(100,TOL,TCHK)
      DO IAT=1,NAT
        DO I=1,3
          FORCE(I,IAT)=0.D0
        ENDDO
...
!     ==================================================================
      ...
      CALL CONSTRAINTS__NFREE(NAT,NFREE)
      CALL THERMOSTAT__SELECT('ATOMS')
      CALL THERMOSTAT__GETL4('ON',TCHK)
      IF(TCHK)CALL THERMOSTAT__SCALEGFREE(REAL(NFREE,KIND=8))
!
!     ==================================================================
...
!     ==================================================================
      ...
      DO IAT=1,NAT
        IF(EFFEMASS(IAT).GT.RMASS(IAT)) THEN
          IF(.NOT.TERR) THEN
            CALL ERROR__MSG('REDUCED MASS MUST NOT BE NEGATIVE')
            TERR=.TRUE.
          END IF
          CALL ERROR__CHVAL('ATOM ',NAME(IAT))
          CALL ERROR__R8VAL('MASS ',RMASS(IAT))
          CALL ERROR__R8VAL('REDUCED MASS ',RMASS(IAT)-EFFEMASS(IAT))
        END IF
      ENDDO
      IF(TERR) THEN
        CALL ERROR__MSG('SOLUTION: REDUCE WAVE FUNCTION MASS')
        CALL ERROR__MSG('OR SET PS<G2> AND PS<G4> TO ZERO')
        CALL ERROR__STOP('ATOMS__INITIALIZE')
      END IF
!
!     ==================================================================
...
!     ==================================================================
      ...
      IF(TCHK) THEN
        ...
        CALL THERMOSTAT__GETL4('ON',TCHK)
        IF(TCHK) THEN
          CALL THERMOSTAT__GETR8('TARGET',EKIN)
          SVAR=0.D0
          DO IAT=1,NAT
          ...
          IF(SVAR.EQ.0.D0) THEN
            CALL ERROR__MSG('TEMPERATURE FOR WAVE FUNCTION THERMOSTAT IS ZERO')
            CALL ERROR__MSG('PROBABLY !STRUCTURE!SPECIES:PS<G2> IS NOT SPECIFIED')
            CALL ERROR__STOP('ATOMS__INITIALIZE')
          END IF
          CALL THERMOSTAT__SELECT('WAVES')
          CALL THERMOSTAT__GETR8('TARGET',EKIN)
          CALL THERMOSTAT__SCALEGFREE(SVAR/EKIN)
          CALL THERMOSTAT__SETR8('GFREE',0.D0)
        ELSE
          CALL ERROR__MSG('WAVE FUNCTION THERMOSTAT CANNOT BE USED')
          CALL ERROR__MSG('WITHOUT ATOM THERMOSTAT')
          CALL ERROR__STOP('ATOMS__INITIALIZE')
        END IF
      END IF
                               CALL TRACE__POP
...
!     ******************************************************************
      ...
      REAL(8)        :: RBAS(3,3)
!     ******************************************************************
                              CALL TRACE__PUSH('ATOMS__PROPAGATE')
DO IAT=1,NAT
...
!     ==================================================================
      IF(.NOT.TCONSTRAINTREFERENCE) THEN
        CALL CELL__GETR8A('T(0)',9,RBAS)
        CALL CONSTRAINTS__SETREFERENCE(RBAS,NAT,R0,RM,REDRMASS,DELT)
...
!     ==================================================================
      CALL QMMM__SCALEFORCE(NAT,FORCE)
!
!     == PRECONDITIONING =============================================
!     CALL SHADOW__PRECONDITION(NAT,RMASS,R0,FORCE)
!
!     == SYMMETRIZE FORCE ============================================
      CALL SYMMETRIZE__FORCE(NAT,FORCE)
!
!     ================================================================
!     ==  PROPAGATE ATOMS                                           ==
!     ================================================================
...
!     IF(TSTRESS) CALL CELL__GETR8A('FRICMAT',9,CELLFRIC)
...
!     ******************************************************************
      ...
      REAL(8)        :: RBAS(3,3)
!     ******************************************************************
                              CALL TRACE__PUSH('ATOMS__CONSTRAINTS')
      IF(.NOT.TDYN) THEN
        CALL TRACE__POP ;RETURN
      END IF
!
...
!     ==================================================================
PRINT*,'WARNING FROM ATOMS__CONSTRAINTS: TSTRESS SET TO FALSE'
TSTRESS=.FALSE.
      IF(TSTRESS) THEN
...
!     ==================================================================
      CALL CELL__GETR8A('T(0)',9,RBAS)
      CALL CONSTRAINTS__APPLY(RBAS,NAT,R0,RP1,REDRMASS,FORCE,DELT)
      CALL SYMMETRIZE__POSITION(NAT,RP1)
!
!     ==================================================================
!     == MAP NEW COORDINATES BACK INTO THE CURRENT UNIT CELL          ==
!     ==================================================================
      IF(TSTRESS) THEN
        CALL lib__invertr8(3,MAPTOCELL,MAPTOCELL)
...
!     ==================================================================
      ...
      DO IAT=1,NAT
        ...
        ENDDO
      ENDDO
      CALL CELL__SETR8A('KINSTRESS',9,STRESS1)
...
!     ******************************************************************
      ...
      CALL QMMM__DEKIN(DELT,DEKIN)   !CORRECTION FOR LINK ATOMS
      EKIN_=EKIN_+DEKIN
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMS__EFFEKIN(EKIN_)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE ATOMS__DEPOTDT(DEPOTDT)
!     ******************************************************************
!     **  CALCULATE KINETIC ENERGY                                    **
!     ******************************************************************!
      USE ATOMS_MODULE
...
!
!     ..................................................................
      SUBROUTINE ATOMS__TEMPERATURE(TEMPERATURE_)
!     ******************************************************************
!     **  CALCULATE KINETIC ENERGY                                    **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      FORCE(:,:)=0.D0
      CALL QMMM__SWITCH
      CALL CONTINUUM__SWITCH(NAT,RP,R0)
!
      CALL CELL__GETL4('MOVE',TSTRESS)
      IF(TSTRESS) THEN
        CALL CELL__GETR8A('MAPTOCELL',9,MAPTOCELL)
...
!         ==  PROPAGATE ==================================================
          ...
          ENDDO
          CALL lib__INVERTr8(3,MATP,MATPINV)
...
!
!     ..................................................................
      SUBROUTINE ATOMS__WRITE(NFIL,NFILO,TCHK)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE ATOMS__READ(NFIL,NFILO,TCHK)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE RESTART_INTERFACE
...
!         == read r(0)
          DO IAT=1,NAT
            ...
            ENDDO
            IF(.NOT.TCHK1) THEN
              CALL ERROR__MSG('ATOM NAMES ON RESTART FILE AND STRUCTURE FILE INCONSISTENT')
              CALL ERROR__CHVAL('NO PARTNER FOUND FOR ATOM ',NAME(IAT))
              CALL ERROR__STOP('ATOMS__READ')
            END IF
          ENDDO
!         == read r(-)
          READ(NFIL)TMP(:,:)
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__INITIALIZE(NAT_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__NATOM(NAT_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__INDEX(IDENT_,IAT_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      DO IAT=1,NAT
        ...
        END IF
      ENDDO
      CALL ERROR__MSG('ATOM IDENTIFIER NOT RECOGNIZED')
      CALL ERROR__CHVAL('IDENT_',IDENT_)
      CALL ERROR__STOP('ATOMLIST__INDEX')
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETR8A(ID_,IAT_,LENG_,VAL_)
!     ******************************************************************
!     **                                                              **
...
!     ******************************************************************
      IF(IAT_.GT.NAT.OR.IAT_.LT.0) THEN
        CALL ERROR__MSG('ATOM INDEX OUT OF RANGE')
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__GET')
      END IF
!     ==================================================================
!     == R(0)                                                         ==
!     ==================================================================
...
!     ==================================================================
      ELSE IF(ID_.EQ.'MASS') THEN
        IF(IAT_.EQ.0) THEN
          ...
          VAL_=RMASS
        ELSE
           CALL ERROR__MSG('GETR8A DOES NOT HANDLE SCALARS')
           CALL ERROR__STOP('ATOMLIST__GETR8A')
        END IF
!     ==================================================================
!     ==  PS<G2> PSG2                                                 ==
!     ==================================================================
...
!     ==================================================================
      ...
      RETURN
 9000 CONTINUE
      CALL ERROR__MSG('INCONSISTENT ARRAY SIZE')
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETR8(ID_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.LE.0.OR.IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__GETR8')
      END IF
      IF(ID_.EQ.'MASS') THEN
        VAL_=RMASS(IAT_)
      ELSE IF(ID_.EQ.'PS<G2>') THEN
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETCHA(ID_,IAT_,LENG_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.LT.0.OR.IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__GETCHA')
      END IF
      IF(ID_.EQ.'NAME') THEN
        IF(IAT_.EQ.0) THEN
          IF(LENG_.NE.NAT) GOTO 9000
          VAL_(:)=NAME(:)
        ELSE
           CALL ERROR__MSG('GETI4A DOES NOT HANDLE SCALARS')
           CALL ERROR__STOP('ATOMLIST__GETCHA')
        END IF
      ELSE
        CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED')
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETCH(ID_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID_',ID_)
        CALL ERROR__STOP('ATOMLIST__GETCH')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETI4A(ID_,IAT_,LENG_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID_',ID_)
        CALL ERROR__STOP('ATOMLIST__GETI4A')
      END IF
      RETURN
 9000 CONTINUE
      CALL ERROR__MSG('INCONSISTENT ARRAY SIZE')
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETI4(ID_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.LE.0.OR.IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__GETI4')
      END IF
      IF(ID_.EQ.'ISPECIES') THEN
        VAL_=ISPECIES(IAT_)
      ELSE
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETR8A(ID_,IAT_,LENG_,VAL_)
!     ******************************************************************
!     **                                                              **
!     **  RETURNS REAL(8) ARRAYS                                      **
!     **  IAT=0 RETURNS THE DATA FOR ALL ATOMS                        **
...
!     ******************************************************************
      IF(IAT_.GT.NAT.OR.IAT_.LT.0) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETR8A')
      END IF
!     ==================================================================
!     == R(0)                                                         ==
!     ==================================================================
...
!     ==================================================================
      ELSE IF(ID_.EQ.'MASS') THEN
        IF(IAT_.EQ.0) THEN
          ...
          RMASS=VAL_(:)
        ELSE
          CALL ERROR__MSG('SETR8A DOES NOT HANDLE SCALARS')
          CALL ERROR__STOP('ATOMLIST__SETR8A')
        END IF
!     ==================================================================
!     ==  PSG2 'PS<G2>'                                               ==
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETR8(ID_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.LE.0.OR.IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETR8')
      END IF
      IF(ID_.EQ.'MASS') THEN
        RMASS(IAT_)=VAL_
      ELSE IF(ID_.EQ.'PS<G2>') THEN
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETCHA(ID_,IAT_,LENG_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.LT.0.OR.IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETCHA')
      END IF
      IF(ID_.EQ.'NAME') THEN
        IF(IAT_.EQ.0) THEN
          IF(LENG_.NE.NAT) GOTO 9000
          NAME(:)=VAL_(:)
        ELSE
           CALL ERROR__MSG('SETI4A DOES NOT HANDLE SCALARS')
           CALL ERROR__STOP('ATOMLIST__SETCHA')
        END IF
      ELSE
        CALL ERROR__MSG('IDENTIFIER NOT RECOGNIZED')
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETCH(ID_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETCH')
      END IF
      IF(ID_.EQ.'NAME') THEN
        NAME(IAT_)=VAL_
      ELSE
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETI4A(ID_,IAT_,LENG_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETI4A')
      END IF
      IF(ID_.EQ.'ISPECIES') THEN
        IF(IAT_.EQ.0) THEN
          IF(LENG_.NE.NAT) GOTO 9000
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETI4(ID_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.LE.0.OR.IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETI4')
      END IF
      IF(ID_.EQ.'ISPECIES') THEN
        ISPECIES(IAT_)=VAL_
      ELSE
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GET(ID_,NBYTE_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__GETALL(ID_,NBYTE_,NAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(NAT_.NE.NAT) THEN
        ...
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__GETALL')
      END IF
      IF(ID_.EQ.'NAME') THEN
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SET(ID_,NBYTE_,IAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(IAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('IAT_',IAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SET')
      END IF
      IF(ID_.EQ.'NAME') THEN
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETALL(ID_,NBYTE_,NAT_,VAL_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      IF(NAT_.GT.NAT) THEN
        ...
        CALL ERROR__I4VAL('NAT_',NAT_)
        CALL ERROR__I4VAL('NAT',NAT)
        CALL ERROR__STOP('ATOMLIST__SETALL')
      END IF
      IF(ID_.EQ.'NAME') THEN
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__SETLATTICE(RBAS_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      ...
      CALL ERROR__STOP('ATOMLIST__SETLATTICE')
      RBAS(:,:)=RBAS_(:,:)
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__LATTICE(RBAS_)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      CALL ERROR__MSG('ROUTINE MARKED FOR DELETION')
      CALL ERROR__STOP('ATOMLIST__LATTICE')
      RBAS_(:,:)=RBAS(:,:)
      CALL CELL__GETR8A('T0',9,RBAS_)
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMLIST__ORDER
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
...
!     ******************************************************************
      ...
      DO WHILE(IAT.LT.NAT)
        DO IAT1=IAT,NAT
          ISP1=ISPECIES(IAT1)
          IF(ISP1.LT.1) THEN
            CALL ERROR__MSG('ISPECIES MUST BE SET')
            CALL ERROR__STOP('ATOMLIST__ORDER')
          END IF
          IF(ISP1.EQ.ISP) THEN
            IF(IAT1.NE.IAT) THEN
              XNAME        =NAME(IAT)
...
!
!     ..................................................................
      SUBROUTINE ATOMLIST__INDEXX(NAME,IAT,IT)
!     ******************************************************************
!     **  ATOMLIST__INDEXX                                             **
...
!       ==  IF
        ...
        END IF
      END IF
      CALL ATOMLIST__INDEX(NAME1,IAT)
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE ATOMS__DUMMYCHECK(STRING)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE ATOMS_MODULE
