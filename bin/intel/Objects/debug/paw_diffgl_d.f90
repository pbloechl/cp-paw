...
!     ==================================================================
      ...
      IF(R1*DEXP(DEX*DBLE(MCH-1)).GT.BOXRADIUS) THEN
         call error__STOP('PAWBOUNDSTATE')
      END IF
!
!     ==================================================================
...
!     ==================================================================
      ...
      DO I1=1,NPRO
        CALL RADIAL__INTEGRAL(R1,DEX,NR,R(:)**2*PRO(:,I1)*PHI(:,1),VEC(I1))
      ENDDO
      GINH(:)=PHI(:,1)
      DO I1=1,NPRO
        SVAR=0.D0
...
!     ==================================================================
      DO I1=1,NPRO
        CALL RADIAL__INTEGRAL(R1,DEX,NR,r(:)**2*pro(:,i1)*phi(:,1),VEC(I1))
      ENDDO
      CALL RADIAL__INTEGRAL(R1,DEX,NR,(phi(:,1)*r(:))**2,SVAR)
      DO I1=1,NPRO
        DO I2=1,NPRO
...
exit
          IF(DABS(PRO(IR,I1)).GT.1.D-6) THEN
            CALL ERROR__MSG('WARNING FROM PAWDER PRO DOES NOT VANISH OUTSIDE')
            CALL ERROR__I4VAL('L',L)
            CALL ERROR__I4VAL('IPRO',I1)
            CALL ERROR__I4VAL('IR',IR)
            CALL ERROR__I4VAL('MCH',MCH)
            CALL ERROR__r8VAL('r(mcH)',r1*exp(dex*real(mch-1)))
            CALL ERROR__STOP('PAWDER')
          END IF
        ENDDO
      ENDDO
!
...
!     ==================================================================
      DO I1=1,NPRO
        DO I2=1,NPRO
          ...
          ENDDO
          CALL RADIAL__INTEGRAL(R1,DEX,NR,AUX,AMAT(I1,I2))
        ENDDO
      ENDDO
      DO I1=1,NPRO
        RI=R1/XEXP
        ...
        ENDDO
        CALL RADIAL__INTEGRAL(R1,DEX,NR,AUX,CVEC(I1))
      ENDDO
!
!     ==================================================================
...
!        CALL DGEICD(BMAT,NPRO,NPRO,0,RCOND,DET,AUX1,NAUX,*9999)
        call lib__invertr8(npro,bmat,bmatinv)
        bmat=bmatinv
      END IF
!
!     ==================================================================
...
!     ==================================================================
      ...
      RETURN
 9999 CONTINUE
      CALL ERROR__MSG('ERROR IN ESSL MATRIX INVERSION')
      CALL ERROR__STOP('PAWDER')
      STOP
      END
!
...
!       == ESTIMATE CLASSICAL TURNING POINT FOR LOW LYING SHELLS
     ...
     &                     ,POT,PHI,GINH)
        ...
        IF(ITER.GT.ITERX) THEN
          CALL ERROR__MSG('SHOOTING LOOP NOT CONVERGED')
          CALL ERROR__I4VAL('L',L)
          CALL ERROR__R8VAL('E',E)
          CALL ERROR__I4VAL('TARGET #(NODES)',NN)
          CALL ERROR__I4VAL('ACTUAL #(NODES)',NNODE)
          CALL ERROR__STOP('FIXNODE')
        END IF
      ENDDO
...
!     ==========================================================
      IF(NNODE.NE.NN)  THEN
        IF(E.LT.-0.1D0) THEN
          CALL ERROR__MSG('#NODES INCORRECT')
          CALL ERROR__I4VAL('L',L)
          CALL ERROR__R8VAL('E',E)
          CALL ERROR__I4VAL('TARGET #(NODES)',NN)
          CALL ERROR__I4VAL('ACTUAL #(NODES)',NNODE)
...
!     ==================================================================
      IF(PRESENT(DLG)) THEN
        CALL RADIAL__VALUE(R1,DEX,NR,PHI1(:,1),boxradius,PHI1x(1))
        CALL RADIAL__VALUE(R1,DEX,NR,PHI1(:,2),boxradius,PHI1x(2))
        CALL RADIAL__VALUE(R1,DEX,NR,PHI1dot(:,1),boxradius,PHI1dotx(1))
        CALL RADIAL__VALUE(R1,DEX,NR,PHI1dot(:,2),boxradius,PHI1dotx(2))
...
!     == NORMALIZE                                                    ==
!     ==================================================================
      CALL RADIAL__INTEGRAL(R1,DEX,NR,(R(:)*PHI(:,1))**2,SVAR)
      SVAR=1.D0/DSQRT(SVAR)
      PHI(:,:)=PHI(:,:)*SVAR
      STEP(:)=STEP(:)*SVAR
...
!     ==================================================================
      ...
      CALL RADIAL__INTEGRAL(R1,DEX,NR,R(:)**2*GINH(:),SVAR)
      E=SVAR
      RETURN
      END
!
...
!       print*,up(ir),-Aez*dex*r(ir) ! to jeszcze sprawdzic
    ...
    &         +CG(IR)/r(ir)**gamma
        ...
        if(it.gt.500) then
          call error__msg('iteration not converged')
          call error__stop('inboundary')
        end if
      end do
...
!!__     print *,(DEX+FRP(IR))*UP(IR),(CF(IR)+FR(IR))*U(IR)
      end do
      RETURN
...
!     ******************************************************************
      ...
      IF(AEZ.GT.0) THEN
         IF(L .EQ. 0) THEN
            GAMMA=DSQRT(1.0D0-FSS*AEZ**2)
         ELSE
!______            GAMMA=(    L*DSQRT(   L**2 -FSS*AEZ**2)+ &
!______     &           (L+1)*DSQRT((L+1)**2-FSS*AEZ**2)   )/(2*L+1)
            gamma = sqrt(1.0+l*(l+1)-FSS*AEZ**2)
...
!!__      DO IR=1,4
!!__         RIGAMMA=R(ir)**GAMMA
!!__         U(IR)=RIGAMMA
!!__         UP(IR)=DEX*GAMMA*RIGAMMA
!!__         UPP(IR)=(DEX+FRP(IR))*UP(IR)+(CF(IR)+FR(IR))*U(IR)+CG(IR)
!!__      ENDDO
!cwk my new  Attention!! U() means now u/r**gamma
      DO IR=1,4
        U(IR)=1.0
...
!     ================================================================
      ...
      IF(SVAR.EQ.0.D0) THEN
        ...
        ENDDO
        CALL ERROR__MSG('NORMALIZATION IS ZERO')
        CALL ERROR__I4VAL('IR1',IR1)
        CALL ERROR__I4VAL('IR2',IR2)
        CALL ERROR__I4VAL('NR',NR)
        CALL ERROR__I4VAL('IRBND',IRBND)
        CALL ERROR__I4VAL('IRBOXRADIUS',INT(XBOXRADIUS))
        CALL ERROR__L4VAL('THOM',THOM)
        CALL ERROR__L4VAL('PRESENT(DLG)',PRESENT(DLG))
        CALL ERROR__STOP('OUTBOUNDARY')
      END IF
      SVAR=1.D0/SVAR
      DO IR=1,NR
        U(IR)=SVAR*U(IR)
...
!     ******************************************************************
      ...
      call error__stop('oldoutboundary')
!
!     ==================================================================
!     ==  START WITH U==0 FOR INHOMOGENEOUS SOLUTION                  ==
!     ==================================================================
