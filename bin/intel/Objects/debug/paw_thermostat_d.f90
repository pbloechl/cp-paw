...
!**                                                                   **
!**    THERMOSTAT__NEW(ID)                                             **
!**    THERMOSTAT__SETR8('TIMESTEP',DT)                                **
!**    THERMOSTAT__SETR8('MASS',Q)                                     **
!**    THERMOSTAT__SETR8('FRICTION',ANNE)                              **
!**    THERMOSTAT__SETR8('GFREE',GFREE)    (OPTIONAL,ONLY FOR REPORT)  **
!**                                                                   **
!**    THERMOSTAT__SELECT(ID)                                          **
!**    THERMOSTAT__SETR8('EKIN(SYSTEM)')                               **
!**    THERMOSTAT__PROPAGATE                                           **
!**    THERMOSTAT__SWITCH                                              **
!**                                                                   **
!**  REMARKS:                                                         **
...
!**    THERMOSTAT__PROPAGATE THE VALUE IS OBTAINED FROM A POLYNOMIAL   **
!**    EXTRAPOLATION OF THE THERMOSTAT VARIABLE.                      **
!**    IF THE 'COOLING' IS OBTaIND AFTER CALLING THERMOSTAT__PROPAGATE **
      ...
      ELSE
        ...
        DO WHILE (ASSOCIATED(THIS%NEXT))
          IF(THIS%ID.EQ.ID) THEN
            CALL ERROR__MSG('CANNOT CREATE THERMOSTAT WITH THE SAME NAME')
            CALL ERROR__STOP('THERMOSTAT__NEW')
          END IF
          THIS=>THIS%NEXT
        ENDDO
        ALLOCATE(THIS%NEXT)
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__DElete')
      END IF
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__SELECT(ID)
!     ******************************************************************
!     **                                                              **
!     ******************************************************************
      USE THERMOSTAT_MODULE
...
!     ******************************************************************
      IF(.NOT.TINI) THEN
        CALL ERROR__MSG('CREATE THERMOSTATS BEFORE SELECTING')
        CALL ERROR__STOP('THERMOSTAT__SELECT')
      END IF
!
!     == UNSELECT ======================================================
      IF(ID.EQ.'NONE') THEN
...
!     == FIND THERMOSTAT AND SELECT
      ...
      DO WHILE (THIS%ID.NE.ID)
        IF(.NOT.ASSOCIATED(THIS%NEXT)) THEN
          CALL ERROR__MSG('THERMOSTAT DOES NOT EXIST')
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__REPORT(NFIL)
!     ==================================================================
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__REPORT')
      END IF
      IF(.NOT.THIS%ON) RETURN
!
      CALL CONSTANTS('KB',KELVIN)
      PI=4.D0*DATAN(1.D0)
      CALL REPORT__TITLE(NFIL,'THERMOSTAT: '//TRIM(THIS%ID))
      IF(THIS%STOP) THEN
        CALL REPORT__CHVAL(NFIL,'INITIAL VELOCITIES SET TO','ZERO')
      END IF
      IF(THIS%GFREE.NE.0.D0) THEN
        SVAR=THIS%EKIN_TARGET/(0.5D0*THIS%GFREE*KELVIN)
        CALL REPORT__R8VAL(NFIL,'TEMPERATURE',SVAR,'K')
        CALL REPORT__R8VAL(NFIL,'#(DEGREES OF FREEDOM)',THIS%GFREE,' ')
      ELSE
        SVAR=THIS%EKIN_TARGET
        CALL REPORT__R8VAL(NFIL,'TARGET KINETIC ENERGY',SVAR,'H')
      END IF
      OMEGA0=SQRT(4.D0*THIS%EKIN_TARGET/THIS%Q)
      SVAR=2.D0*PI/OMEGA0
      CALL REPORT__R8VAL(NFIL,'OSCILATION PERIOD',SVAR,'A.U.')
      IF(THIS%FRICTION.NE.0.D0) THEN
        CALL REPORT__R8VAL(NFIL,'FRICTION ON NOSE VARIABLE',THIS%FRICTION,' ')
        SVAR=THIS%FRICTION/(OMEGA0*THIS%DT)
        CALL REPORT__R8VAL(NFIL,'SAME DIVIDED BY CRITICAL FRICTION',SVAR,' ')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__CONVERT(DT,TARGET,PERIOD,Q,FRICTION)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__SCALEGFREE(GFREE)
!     ******************************************************************
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__SCALEGFREE')
      END IF
      IF(THIS%Q.LE.0.D0) THEN
        CALL ERROR__MSG('THERMOSTAT MASS IS ZERO')
        CALL ERROR__CHVAL('ID',THIS%ID)
        CALL ERROR__STOP('THERMOSTAT__SCALEGFREE')
      END IF
      IF(THIS%EKIN_TARGET.LE.0.D0) THEN
        CALL ERROR__MSG('TARGET KINETIC ENERGY IS ZERO')
        CALL ERROR__CHVAL('ID',THIS%ID)
        CALL ERROR__STOP('THERMOSTAT__SCALEGFREE')
      END IF
!     ==================================================================
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__PROPAGATE()
!     ******************************************************************
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__PROPAGATE')
      END IF
!
      IF(.NOT.THIS%ON) RETURN
!
      ...
      IF(THIS%DT.LE.0.D0) THEN
        CALL ERROR__MSG('TIME STEP MUST BE POSITIVE AND FINITE')
        CALL ERROR__CHVAL('ID',THIS%ID)
        CALL ERROR__STOP('THERMOSTAT__PROPAGATE')
      END IF
!
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__SWITCH')
      END IF
      IF(.NOT.THIS%ON) RETURN
!
!     =================================================================
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__WRITE(NFIL,NFILO,TCHK)
!     ******************************************************************
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__WRITE')
      END IF
      IF(.NOT.THIS%ON) RETURN
!
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__READ(NFIL,NFILO,TCHK)
!     ******************************************************************
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__READ')
      END IF
      CALL MPE__QUERY(NTASKS,THISTASK)
!
!     == READ DATA FROM FILE ===========================================
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__SETR8(ID,VAL)
!     ==================================================================
!     ==  SET DATA                                                    ==
!     ==================================================================
      USE THERMOSTAT_MODULE
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__SETR8')
      END IF
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__SETL4(ID,VAL)
!     ==================================================================
!     ==  SET DATA                                                    ==
!     ==================================================================
      USE THERMOSTAT_MODULE
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__SETL4')
      END IF
      IF(ID.EQ.'ON') THEN
         THIS%ON=VAL
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__GETR8(ID,VAL)
!     ==================================================================
!     ==  SET DATA                                                    ==
!     ==================================================================
      USE THERMOSTAT_MODULE
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__GETR8')
      END IF
...
!
!     ..................................................................
      SUBROUTINE THERMOSTAT__GETL4(ID,VAL)
!     ==================================================================
!     ==  SET DATA                                                    ==
!     ==================================================================
      USE THERMOSTAT_MODULE
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS)) THEN
        CALL ERROR__MSG('NO THERMOSTAT SELECTED')
        CALL ERROR__STOP('THERMOSTAT__GETL4')
      END IF
      IF(ID.EQ.'ON') THEN
         VAL=THIS%ON
      ELSE
