...
!**  EVERY DATA OBJECT HAS AN ENTRY IN A LINKEDLIST (__LISTSTART)      **
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__REPORT(NFIL)
!     ******************************************************************
!     **  PLANEWAVE__REPORT DEASCRIBED THE SETTING FOR THE             **
...
!     ******************************************************************
      ...
      CALL REPORT__TITLE(NFIL,'PLANEWAVE')
      CALL REPORT__CHVAL(NFIL,'ID',THIS%ID)
      CALL REPORT__L4VAL(NFIL,'TINV',THIS%TINV)
      CALL REPORT__L4VAL(NFIL,'TSUPER',THIS%TSUPER)
      WRITE(NFIL,FMT='("GBASINV*K=(",3F10.5,")")')THIS%KVEC
      CALL REPORT__I4VAL(NFIL,'NR1',THIS%NR1,' ')
      CALL REPORT__I4VAL(NFIL,'NR2',THIS%NR2,' ')
      CALL REPORT__I4VAL(NFIL,'NR3',THIS%NR3,' ')
      DO ITASK=1,NTASKS
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__SELECT(ID)
!     ******************************************************************
!     **  PLANEWAVE__SELECT                                            **
!     **                                                              **
!     **  SELECTS                                                     **
...
!     ******************************************************************
      IF(.NOT.ASSOCIATED(THIS))THEN
        CALL ERROR__MSG('LIST NOT INITIALIZED')
        CALL ERROR__STOP('PLANEWAVE__SELECT')
      ENDIF
!
!     ==================================================================
...
!     ==================================================================
      ...
      DO WHILE(THIS%ID.NE.ID)
        ...
        IF(ASSOCIATED(THIS,THIS1)) THEN
          CALL ERROR__MSG('PWPARALLELOBJECT NOT FOUND')
...
!     ==================================================================
      ...
      DO WHILE(.NOT.ASSOCIATED(THIS,THIS1))
        IF(THIS%ID.EQ.THIS1%ID) THEN
          CALL ERROR__MSG('ID ALREADY EXISTS')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('PLANEWAVE_NEW')
        END IF
        THIS1=>THIS1%NEXT
      ENDDO
      RETURN
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETL4(ID,VAL)
!     ******************************************************************
...
!     ******************************************************************
      ...
      ELSE
        ...
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('PLANEWAVE__GETL4')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__SETL4(ID,VAL)
!     ******************************************************************
...
!     ******************************************************************
      IF(ID.EQ.'SUPER') THEN ! SELECT SUPER WAVE FUNCTION INTERFACE
        ...
        IF(VAL.AND.DOT_PRODUCT(THIS%KVEC,THIS%KVEC).GT.1.D-6) THEN
          CALL ERROR__MSG('SUPER WAVE FUNCTION INTERFACE CAN BE SELECTED')
          CALL ERROR__MSG('ONLY FOR THE GAMMA POINT')
          CALL ERROR__CHVAL(ID,ID)
          CALL ERROR__STOP('PLANEWAVE__SETL4')
        END IF
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETI4(ID,VAL)
!     ******************************************************************
...
!     ******************************************************************
      ...
      ELSE IF(ID.EQ.'NGAMMA') THEN ! INDEX OF THE GAMMA POINT OR ZERO
        IF(TSUPER) THEN
          VAL=THIS%NGAMMA
        ELSE
          CALL ERROR__MSG('NO SUPER WAVE FUNCTION INTERFACE')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('PLANEWAVE__GETI4')
        END IF
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETI4A(ID,LEN,VAL)
!     ******************************************************************
      ...
      IF(ID.EQ.'IGVEC') THEN   ! G-VECTORS
        ...
        IF(.NOT.SUPER) THEN
          IF(LEN.NE.3*NGL) THEN
            ...
            CALL ERROR__I4VAL('3*NGL',3*NGL)
            CALL ERROR__STOP('PLANEWAVE__GETI4A')
          END IF
        ...
        ELSE
          ...
          DO IG=1,NGL
            ...
            IF(IGH+3.GT.LEN) THEN
              ...
              CALL ERROR__CHVAL('ID',ID)
              CALL ERROR__I4VAL('LEN',LEN)
              CALL ERROR__I4VAL('1+3*(NGL-1)/2',1+3*(NGL-1)/2)
              CALL ERROR__STOP('PLANEWAVE__GETI4A')
            END IF
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETR8(ID,VAL)
!     ******************************************************************
...
!     ******************************************************************
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('PLANEWAVE__GETR8')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETR8A(ID,LEN,VAL)
!     ******************************************************************
!     **  HANDLES REQUESTS FOR REAL ARRAYS                            **
!     ******************************************************************
      USE PLANEWAVE_MODULE
      ...
      IF(ID.EQ.'GVEC') THEN   ! G-VECTORS
        ...
        IF(.NOT.SUPER) THEN
          IF(LEN.NE.3*NGL) THEN
            ...
            CALL ERROR__I4VAL('LEN',LEN)
            CALL ERROR__I4VAL('3*NGL',3*NGL)
            CALL ERROR__STOP('PLANEWAVE__GETR8A')
          END IF
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__SETR8A(ID,LEN,VAL)
!     ******************************************************************
!     **  ACCEPTS REAL ARRAYS                                         **
!     ******************************************************************
      USE PLANEWAVE_MODULE
...
!     ==================================================================
      IF(ID.EQ.'RBAS') THEN   ! LATTICE VECTORS
        IF(LEN.NE.9) THEN
          ...
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__I4VAL('LEN',LEN)
          CALL ERROR__STOP('PLANEWAVE__SETR8A')
        END IF
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETC8A(ID,LEN,VAL)
!     ******************************************************************
...
!     ==================================================================
      IF(ID.EQ.'EIKR') THEN   ! PHASE FACTOR
        IF(LEN.NE.THIS%NR1L*THIS%NR2*THIS%NR3) THEN
          CALL ERROR__MSG('INCONSISTENT SIZE')
          CALL ERROR__CHVAL('ID',ID)
          CALL ERROR__STOP('PLANEWAVE__GETC8A')
        END IF
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__GETCH(ID,VAL)
!     ******************************************************************
!     **  HANDLES REQUESTS FOR COMPLEX ARRAYS                         **
!     ******************************************************************
      USE PLANEWAVE_MODULE
...
!     ==================================================================
      ...
      ELSE
        CALL ERROR__MSG('ID NOT RECOGNIZED')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('PLANEWAVE__GETCH')
      END IF
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__CHECKINVERSION(KVEC,TINV)
!     ******************************************************************
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__DIVIDERGRIDONTASKS(EPW,RBAS,NR1START,NR1L,NR2,NR3)
!     ******************************************************************
...
!     ******************************************************************
     ...
     &           ,MIN1,MAX1,MIN2,MAX2,MIN3,MAX3)
      ...
      NR2=MAX2-MIN2+1
      NR3=MAX3-MIN3+1
      CALL LIB__FFTadjustGRD(NR1G)
      CALL LIB__FFTadjustGRD(NR2)
      CALL LIB__FFTadjustGRD(NR3)
      NR1L=NR1G/NTASKS
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__INITIALIZE(ID,RBAS,KVEC,TINV,EPW &
...
!     **    DONE AHEAD OF TIME                                        **
!     **    USE TOOL PLANEWAVE__DIVIDERGRIDONTASKS                     **
!     **                                                              **
!     ******************************************************************
      USE PLANEWAVE_MODULE
      USE MPE_MODULE
...
!     ******************************************************************
                           CALL TRACE__PUSH('PLANEWAVE__INITIALIZE')
      ...
      CALL MPE__GATHER(1,NR1L,THIS%NR1LARR)
      CALL MPE__BROADCAST(1,THIS%NR1LARR)
...
!     ===================================================================
      ...
      ALLOCATE(MINUSG(NG))
      IF(TINV) THEN
        CALL PLANEWAVE__MINUSG(KVEC,NG,IGVEC,MINUSG)
      ELSE
        MINUSG(:)=0
      END IF
!
...
!     ===================================================================
      ...
      DO IG=1,NG
        ...
        IF(ITASK.EQ.0) THEN
          CALL ERROR__MSG('FAILURE IN TASK ASSIGNMENT')
          CALL ERROR__CHVAL('ID',THIS%ID)
          CALL ERROR__I4VAL('IG',IG)
          CALL ERROR__I4VAL('I2',I2)
          CALL ERROR__I4VAL('I3',I3)
          CALL ERROR__STOP('PLANEWAVE__INITIALIZE')
        END IF
      ...
      IF(THIS%TINV) THEN
        DO IG=1,NGL
          IF(MAP(THIS%MINUSG(IG)).EQ.0) THEN
            CALL ERROR__MSG('MINUSG MUST BE ON THE SAME TASK AS G')
            CALL ERROR__STOP('PLANEWAVE__INITIALIZE')
          END IF
...
!
      DO IG=1,NGL
        ...
        IF(ISTRIPE.EQ.0) THEN
          CALL ERROR__MSG('FAILED CONSISTENCY CHECK "ISTRIPE=0"')
          CALL ERROR__STOP('PLANEWAVE__INITIALIZE')
        END IF
        ...
        DO IG=1,NGL
          ...
          IF(THIS%MINUSG(IG).EQ.IG) THEN
            IF(THIS%NGAMMA.EQ.0) THEN
              THIS%NGAMMA=NGH
            ELSE
              CALL ERROR__MSG(' MORE THAN ONE GAMMA POINT?')
              CALL ERROR__STOP('PLANEWAVE__INITIALIZE')
            END IF
          END IF
        ENDDO
        ALLOCATE(THIS%NGHARR(NTASKS))
        CALL MPE__GATHER(1,NGH,THIS%NGHARR)
        CALL MPE__BROADCAST(1,THIS%NGHARR)
!
...
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__MINUSG(KVEC,NG,IG,MINUSG)
!     ******************************************************************
...
!     ==================================================================
      DO I=1,3
        ...
        IF(.NOT.TCHK) THEN
          CALL ERROR__MSG('SUPER WAVE FUNCTIONS SELECTED')
          CALL ERROR__MSG('FOR A NON-INVERSION SYMMETRIC RECIPROCAL GRID')
          CALL ERROR__STOP('PLANEWAVE__MINUSG')
        END IF
      ENDDO
!
!     ==================================================================
...
!     ==================================================================
      ...
      DO IG1=1,NG
        ...
        DO IG2=IG1,NG
          ...
          GOTO 100
        ENDDO
        CALL ERROR__MSG('INVERSION SYMMETRIC PARTNER NOT FOUND')
        CALL ERROR__STOP('PLANEWAVE__MINUSG')
 100    CONTINUE
      ENDDO
      RETURN
        ...
        DO J=MIN2,MAX2
          ...
          DO K=MIN3,MAX3
            ...
            IF(ICOUNT.GT.NG) THEN
              CALL ERROR__MSG('INCONSISTENT #(GVECTORS)')
              CALL ERROR__STOP('PLANEWAVE__GETIG')
            END IF
            IGVEC(1,ICOUNT)=I
            IGVEC(2,ICOUNT)=J
            IGVEC(3,ICOUNT)=K
...
!     ******************************************************************
      ...
      REAL(8)               :: COSTOFSTRIPE
!     ******************************************************************
             CALL TRACE__PUSH('PLANEWAVE_MAPSTRIPESTOTASKS')
!
!     ==================================================================
      ...
      DO IG=1,NG
        ...
        IF(TINV) THEN
          ...
          IF(IGROUP1.NE.IGROUP2) THEN
            ...
            ELSE
              CALL ERROR__MSG('STRIPES RELATED BY INVERSION ENDED UP')
              CALL ERROR__MSG('IN DIFFERENT GROUPS')
              CALL ERROR__STOP('PLANEWAVE_MAPSTRIPESTOTASKS')
            END IF
          END IF
        END IF
      ENDDO
...
!     ******************************************************************
      IF(NGL.NE.THIS%NGLARR(THISTASK)) THEN
        CALL ERROR__MSG('SIZE INCONSISTENT')
        CALL ERROR__STOP('PLANEWAVE_GVECTORS')
      END IF
      GBAS(:,:)=THIS%GBAS(:,:)
      KVEC(:)=THIS%KVEC(:)
      KARTK=MATMUL(GBAS,KVEC)
...
!
!     .....................................................PHFAC........
      SUBROUTINE PLANEWAVE__STRUCTUREFACTOR(R,NGL,EIGR)
!     ******************************************************************
!     **                                                              **
...
!     ******************************************************************
      IF(.NOT.THIS%TSUPER) THEN
        IF(NGL.NE.THIS%NGLARR(THISTASK)) THEN
          CALL ERROR__MSG('INCONSISTENT SIZE')
          CALL ERROR__STOP('PLANEWAVE__STRUCTUREFACTOR')
        END IF
     ...
     &                         ,THIS%IGVEC,EIGR)
      ELSE
        ...
        IF(NGL.NE.NGH) THEN
          ...
          CALL ERROR__STOP('PLANEWAVE__STRUCTUREFACTOR')
        END IF
!!__        ALLOCATE(IGVEC(3,NGL))
!!__        IGH=0
!!__        DO IG=1,THIS%NGLARR(THISTASK)
!!__          IF(THIS%MINUSG(IG).LT.IG) CYCLE
!!__          IGH=IGH+1
!!__          IGVEC(:,IGH)=THIS%IGVEC(:,IG)
!!__        ENDDO
!!__        CALL PLANEWAVE_STRUCTUREFACTOR_OLD(R,THIS%GBAS,THIS%KVEC &
!!__     &                         ,THIS%NR1,THIS%NR2,THIS%NR3,NGL &
!!__     &                         ,IGVEC,EIGR)
!!__         DEALLOCATE(IGVEC)
!
        CALL PLANEWAVE_STRUCTUREFACTOR(R,THIS%GBAS,THIS%KVEC &
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__UNSUPER(NGL,FOFG,FOFGUNSUPER)
!     ******************************************************************
!     **  PLANEWAVE__UNSUPER                                           **
...
!     ******************************************************************
      IF(.NOT.THIS%TINV) THEN
        CALL ERROR__MSG('NO TIME INVERSION SYMMETRY')
        CALL ERROR__STOP('PLANEWAVE__INVERTG')
      END IF
      IF(THIS%TSUPER) THEN
        CALL ERROR__MSG('INVERSION DOES NOT MAKE SENSE')
        CALL ERROR__MSG('IF SUPER WAVE FUNCTION INTERFACE IS SELECTED')
        CALL ERROR__STOP('PLANEWAVE__INVERTG')
      END IF
      IF(NGL.NE.THIS%NGLARR(THISTASK)) THEN
        CALL ERROR__MSG('INCONSISTENT SIZE')
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__INVERTG(NGL,FOFG,FOFMG)
!     ******************************************************************
!     **  PLANEWAVE__INVERTG                                           **
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__SUPFFT(ID,NFFT,NGLH,FOFG,NRL,FOFR)
!     ******************************************************************
!     **                                                              **
...
!     ******************************************************************
      ...
      COMPLEX(8)                 :: F1,F2
!     ******************************************************************
                                 CALL TRACE__PUSH('PLANEWAVE__SUPFFT')
                                 CALL TIMING__CLOCKON('PLANEWAVE__SUPFFT')
!
!     ==================================================================
!     == SET UP INITIAL DATA                                          ==
!     ==================================================================
...
!     ==================================================================
      IF(.NOT.THIS%TSUPER) THEN
        CALL ERROR__MSG('SUPERWAVE FUNCTIONS ONLY WORK AT THE GAMMA POINT')
        CALL ERROR__STOP('PLANEWAVE__SUPFFT')
      END IF
      ...
      END IF
      IF(.NOT.THIS%TSUPER) THEN
        CALL ERROR__MSG('SUPER WAVE FUNCTION ONLY ACCESSIBLE FOR GAMMA POINT')
        CALL ERROR__STOP('PLANEWAVE__SUPFFT')
      END IF
      IGH=0
      DO IG=1,NGL
...
!         ==============================================================
        ...
        ENDDO
      ELSE
        CALL ERROR__MSG('ID MUST BE WITHER GTOR OR RTOG')
        CALL ERROR__CHVAL('ID',ID)
        CALL ERROR__STOP('PLANEWAVE__SUPFFT')
      END IF
      DEALLOCATE(FOFG1)
      DEALLOCATE(FOFR1)
                                 CALL TIMING__CLOCKOFF('PLANEWAVE__SUPFFT')
                                 CALL TRACE__POP
      RETURN
      END
!
!     .................................................................
      SUBROUTINE PLANEWAVE__FFT(ID,NFFT,NGL,FOFG,NRL,FOFR)
!     ******************************************************************
!     ******************************************************************
      USE PLANEWAVE_MODULE
      IMPLICIT NONE
      ...
      INTEGER(4)                 :: IFFT
!     ******************************************************************
                                 CALL TRACE__PUSH('PLANEWAVE__FFT')
                                 CALL TIMING__CLOCKON('PLANEWAVE__FFT')
      IF(NGL.NE.THIS%NGLARR(THISTASK)) THEN
        CALL ERROR__MSG('SIZE OF F(G) INCONSISTENT')
        CALL ERROR__STOP('PLANEWAVE__FFT')
      END IF
      IF(NRL.NE.THIS%NR1L*THIS%NR2*THIS%NR3) THEN
        CALL ERROR__MSG('SIZE OF F(G) INCONSISTENT')
        CALL ERROR__STOP('PLANEWAVE__FFT')
      END IF
      IF(THIS%TSUPER) THEN
        CALL ERROR__MSG('WRONG INTERFACE FOR SUPER WAVE FUNCTIONS')
        CALL ERROR__STOP('PLANEWAVE__FFT')
      END IF
      NR1=THIS%NR1
      NR2=THIS%NR2
     ...
     &           ,FOFG(1,IFFT),FOFR(1,IFFT))
      ...
      ELSE
        ...
        CALL ERROR__STOP('PLANEWAVE__FFT')
      END IF
                                 CALL TIMING__CLOCKOFF('PLANEWAVE__FFT')
                                 CALL TRACE__POP
      RETURN
      END
!
...
!     ******************************************************************
      IF(MAX(NR1,NR2,NR3).GT.2048) THEN
        CALL ERROR__MSG('INSUFFICIENT SIZE FOR AUXILIARY ARRAYS')
        CALL ERROR__STOP('PLANEWAVE_FFTGTOR')
      END IF
...
!     ==================================================================
      CALL LIB__FFTC8('GTOR',NR1,NSTRIPEL,FOFG1,FOFG1)
!
!     ==================================================================
...
!     ==================================================================
      DO IR1L=1,NR1L
        CALL LIB__FFTC8('GTOR',NR2,NR3END1,FOFR(1+(IR1L-1)*NR2*NR3) &
     &                                   ,FOFR(1+(IR1L-1)*NR2*NR3))
      ENDDO
      DO IR1L=1,NR1L
        CALL LIB__FFTC8('GTOR',NR2,NR3-NR3END2+1 &
...
!     ==================================================================
      caLL LIB__FFTC8('GTOR',NR3,NR2*NR1L,FOFR1,FOFR1)
!
!     ==================================================================
!     == TRANSPOSE TO OPTIMIZE STRIDE                                 ==
...
!     ==================================================================
      CALL LIB__FFTC8('RTOG',NR3,NR2*NR1L,FOFR1,FOFR1)
!
!     ==================================================================
...
!     ==================================================================
      DO IR1L=1,NR1L
        CALL LIB__FFTC8('RTOG',NR2,NR3END1,FOFR2(1+(IR1L-1)*NR2*NR3) &
     &                                   ,FOFR2(1+(IR1L-1)*NR2*NR3))
      ENDDO
      DO IR1L=1,NR1L
        CALL LIB__FFTC8('RTOG',NR2,NR3-NR3END2+1 &
...
!     ==================================================================
      CALL LIB__FFTC8('RTOG',NR1,NSTRIPEL,FOFG1,FOFG1)
!
!     ==================================================================
!     == MAP STRIPES TO IG                                            ==
!     ==================================================================
...
!     ******************************************************************
                          CALL TIMING__CLOCKON('PLANEWAVE__SCALARPRODUCT')
      IF(ID.EQ.''.OR.ID.EQ.'=') THEN
        CALL LIB__SCALARPRODUCTC8((ID.EQ.'='),NGL*NDIM,N1,F1,N2,F2,MAT)
      ELSE IF(ID.EQ.'-') THEN
        IF(.NOT.THIS%TINV.OR.THIS%TSUPER) THEN
          CALL ERROR__MSG('OPTION - CAN ONLY USED INVERSION SYMMETRY')
          CALL ERROR__MSG('AND NOT FOR SUPER WAVE FUNCTIONS')
          CALL ERROR__STOP('PLANEWAVE__SCALARPRODUCT')
        END IF
...
!           __F2M(G)=F2(-G)^*____________________________________________
            CALL PLANEWAVE__INVERTG(NGL,F2(1,IDIM,I2),F2M(1,IDIM))
          ENDDO
          CALL LIB__SCALARPRODUCTC8(.FALSE.,NGL*NDIM,N1,F1,1,F2M,MAT(1,I2))
        ENDDO
        DO I1=1,N1
          DO I2=1,N2
...
!     ELSE IF(ID.EQ.'=') THEN
!       IF(N1.NE.N2) THEN
!         CALL ERROR__MSG('N1 AND N2 MUST BE IDENTICAL FOR OPTION "="')
!         CALL ERROR__STOP('PLANEWAVE__SCALARPRODUCT')
!       END IF
!       IF(TESSL) THEN
!         DO I1=1,N1
...
!       DO I1=1,N1
!         DO IDIM=1,NDIM
!           CALL PLANEWAVE__INVERTG(NGL,F1(1,IDIM,I1),F1M(1,IDIM))
!         ENDDO
!         IF(TESSL) THEN
...
!     ==================================================================
      MAT=MAT*THIS%GWEIGHT
                         CALL TIMING__CLOCKOFF('PLANEWAVE__SCALARPRODUCT')
      RETURN
      END
!
!     ..................................................................
      SUBROUTINE PLANEWAVE__ADDPRODUCT(ID,NGL,NDIM,N1,F1,N2,F2,MAT)
!     ******************************************************************
!     **                                                              **
!     **  f1=f1+f2*mat                                                **
!     **  THREE OPTIONS SPECIFIED BY ID                               **
...
!     ******************************************************************
      ...
      LOGICAL(4)  ,PARAMETER    :: TESSL=.TRUE.
!     ******************************************************************
                             CALL TIMING__CLOCKON('PLANEWAVE__ADDPRODUCT')
      IF(ID.EQ.'') THEN
        CALL LIB__ADDPRODUCTC8(.FALSE.,NGL*NDIM,N2,N1,F2,MAT,F1)
!       IF(TESSL) THEN
...
!     ==================================================================
      ELSE IF(ID.EQ.'=') THEN
        IF(N1.NE.N2) THEN
          CALL ERROR__MSG('N1 MUST BE EQUAL TO N2 IF ID="=" IS SELECTED')
          CALL ERROR__STOP('PLANEWAVE__ADDPRODUCT')
        END IF
        CALL LIB__ADDPRODUCTC8(.TRUE.,NGL*NDIM,N2,N1,F2,MAT,F1)
!       ALLOCATE(F2M(NGL,NDIM,N2))
!       F2M=F2
!       DO I1=1,N1
!         DO I2=1,N2
...
!     ==================================================================
      ELSE IF(ID.EQ.'-') THEN
        ...
        DO I2=1,N2
          DO IDIM=1,NDIM
            CALL PLANEWAVE__INVERTG(NGL,F2(1,IDIM,I2),F2M(1,IDIM,I2))
          ENDDO
        ENDDO
        CALL LIB__ADDPRODUCTC8(.FALSE.,NGL*NDIM,N2,N1,F2M,MAT,F1)
!       DO I1=1,N1
!         DO I2=1,N2
!           CSUM=MAT(I2,I1)
!           DO IDIM=1,NDIM
...
!       ENDDO
        DEALLOCATE(F2M)
      END IF
                            CALL TIMING__CLOCKOFF('PLANEWAVE__ADDPRODUCT')
      RETURN
      END
!
!.......................................................................
!#TEMPLATE PLANEWAVE__DISTRIBUTE
...
!!
!!     .................................................................
!      SUBROUTINE PLANEWAVE__DISTRIBUTE<TYPEID>(NDIM,NGGLOB,XGLOB,NGLOC,XLOC)
!!     ******************************************************************
!!     **  PLANEWAVE__DISTRIBUTE                                        **
!!     ******************************************************************
!      USE MPE_MODULE
!      USE PLANEWAVE_MODULE
!      IMPLICIT NONE
...
!      LOGICAL(4)              :: TSUPER
!!     ******************************************************************
!                              CALL TRACE__PUSH('PLANEWAVE__DISTRIBUTE')
!      IF (.NOT.TINI) THEN
!        CALL ERROR__MSG('LIST NOT INITIALIZED')
!        CALL ERROR__STOP('PLANEWAVE__DISTRIBUTE<TYPEID>')
!      ENDIF
!      TSUPER=THIS%TSUPER
!!     IF (THIS%TSUPER) THEN
!!       CALL ERROR__MSG('NOT PERMITTED FOR SUPERWAVEFUNCTION INTERFACE')
!!       CALL ERROR__STOP('PLANEWAVE%DISTRIBUTE<TYPEID>')
!!     ENDIF
!      IF(TSUPER) THEN
!        NG=SUM(THIS%NGHARR)
...
!        CALL ERROR__MSG('GLOBAL %(G-VECTORS) INCONSISTENT WITH OBJECT')
!        CALL ERROR__I4VAL('NG (ON INPUT)',NGGLOB)
!        CALL ERROR__I4VAL('NG (IN OBJECT)',NG)
!        CALL ERROR__STOP('PLANEWAVE%DISTRIBUTE<TYPEID>')
!      END IF
!      IF(NGLOC.NE.NGL) THEN
!        CALL ERROR__MSG('LOCAL %(G-VECTORS) INCONSISTENT WITH OBJECT')
!        CALL ERROR__STOP('PLANEWAVE%DISTRIBUTE<TYPEID>')
!      END IF
!!     ******************************************************************
...
!                CALL ERROR__MSG('PACKAGE LARGER THAN ALLOCATED ARRAY')
!                CALL ERROR__STOP('PLANEWAVE_DISTRIBUTE')
!              END IF
!              XTMP(:,NGSEND)=XGLOB(:,I)
!            ENDIF
!          ENDDO
!          IF(TOTASK.NE.1) THEN
!            CALL MPE__SEND(TOTASK,10,XTMP(:,1:NGSEND))
!          ELSE
!            XLOC(:,:)=XTMP(:,1:NGSEND)
!          END IF
!        ENDDO
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__DISTRIBUTER8(NDIM,NGGLOB,XGLOB,NGLOC,XLOC)
!     ******************************************************************
!     **  PLANEWAVE__DISTRIBUTE                                        **
!     ******************************************************************
      USE MPE_MODULE
      ...
      INTEGER(4)  ,INTENT(IN) :: NGGLOB
      INTEGER(4)  ,INTENT(IN) :: NGLOC
      REAL(8)      ,INTENT(IN) :: XGLOB(NDIM,NGGLOB)
      REAL(8)      ,INTENT(OUT):: XLOC(NDIM,NGLOC)
      INTEGER(4)              :: MSGLEN
      INTEGER(4)              :: NGSEND
      INTEGER(4)              :: TOTASK
...
!     ******************************************************************
      ...
      IF (.NOT.TINI) THEN
        CALL ERROR__MSG('LIST NOT INITIALIZED')
        CALL ERROR__STOP('PLANEWAVE__DISTRIBUTER8')
      ENDIF
      TSUPER=THIS%TSUPER
!     IF (THIS%TSUPER) THEN
!       CALL ERROR__MSG('NOT PERMITTED FOR SUPERWAVEFUNCTION INTERFACE')
!       CALL ERROR__STOP('PLANEWAVE%DISTRIBUTER8')
!     ENDIF
      IF(TSUPER) THEN
        NG=SUM(THIS%NGHARR)
        NGL=THIS%NGHARR(THISTASK)
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__DISTRIBUTEI4(NDIM,NGGLOB,XGLOB,NGLOC,XLOC)
!     ******************************************************************
!     **  PLANEWAVE__DISTRIBUTE                                        **
!     ******************************************************************
      USE MPE_MODULE
      ...
      INTEGER(4)  ,INTENT(IN) :: NGGLOB
      INTEGER(4)  ,INTENT(IN) :: NGLOC
      INTEGER(4)      ,INTENT(IN) :: XGLOB(NDIM,NGGLOB)
      INTEGER(4)      ,INTENT(OUT):: XLOC(NDIM,NGLOC)
      INTEGER(4)              :: MSGLEN
      INTEGER(4)              :: NGSEND
      INTEGER(4)              :: TOTASK
      INTEGER(4)      ,ALLOCATABLE:: XTMP(:,:)
      INTEGER(4)              :: NBYTES6
      INTEGER(4)              :: I
      INTEGER(4)              :: NGLX
      INTEGER(4)              :: NG,NGL
...
!     ******************************************************************
      ...
      IF (.NOT.TINI) THEN
        CALL ERROR__MSG('LIST NOT INITIALIZED')
        CALL ERROR__STOP('PLANEWAVE__DISTRIBUTEI4')
      ENDIF
      TSUPER=THIS%TSUPER
!     IF (THIS%TSUPER) THEN
!       CALL ERROR__MSG('NOT PERMITTED FOR SUPERWAVEFUNCTION INTERFACE')
!       CALL ERROR__STOP('PLANEWAVE%DISTRIBUTEI4')
!     ENDIF
      IF(TSUPER) THEN
        NG=SUM(THIS%NGHARR)
        NGL=THIS%NGHARR(THISTASK)
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__DISTRIBUTEC8(NDIM,NGGLOB,XGLOB,NGLOC,XLOC)
!     ******************************************************************
!     **  PLANEWAVE__DISTRIBUTE                                        **
!     ******************************************************************
      USE MPE_MODULE
      ...
      INTEGER(4)  ,INTENT(IN) :: NGGLOB
      INTEGER(4)  ,INTENT(IN) :: NGLOC
      COMPLEX(8)      ,INTENT(IN) :: XGLOB(NDIM,NGGLOB)
      COMPLEX(8)      ,INTENT(OUT):: XLOC(NDIM,NGLOC)
      INTEGER(4)              :: MSGLEN
      INTEGER(4)              :: NGSEND
      INTEGER(4)              :: TOTASK
...
!     ******************************************************************
      ...
      IF (.NOT.TINI) THEN
        CALL ERROR__MSG('LIST NOT INITIALIZED')
        CALL ERROR__STOP('PLANEWAVE__DISTRIBUTEC8')
      ENDIF
      TSUPER=THIS%TSUPER
!     IF (THIS%TSUPER) THEN
!       CALL ERROR__MSG('NOT PERMITTED FOR SUPERWAVEFUNCTION INTERFACE')
!       CALL ERROR__STOP('PLANEWAVE%DISTRIBUTEC8')
!     ENDIF
      IF(TSUPER) THEN
        NG=SUM(THIS%NGHARR)
        NGL=THIS%NGHARR(THISTASK)
...
!     ******************************************************************
      ...
      RETURN
      END
!#END TEMPLATE PLANEWAVE__DISTRIBUTE
!
!.......................................................................
!#TEMPLATE PLANEWAVE__COLLECT
!(<TYPEID><TYPE>)=([R8][REAL(8)])
!                 ([I4][INTEGER(4)])
!                 ([C8][COMPLEX(8)])
!#BODY
!!
!!     .................................................................
!      SUBROUTINE PLANEWAVE__COLLECT<TYPEID>(NDIM,NGLOC,XLOC,NGGLOB,XGLOB)
!!     ******************************************************************
!!     **  PLANEWAVE__COLLECT                                           **
!!     ******************************************************************
!      USE PLANEWAVE_MODULE
...
!      logical(4)               :: TSUPER
!!     ******************************************************************
!                     CALL TRACE__PUSH('PLANEWAVE__COLLECT')
!      IF (.NOT.TINI) THEN
!        CALL ERROR__MSG('LIST NOT INITIALIZED')
!        CALL ERROR__STOP('PLANEWAVE__COLLECT')
!      ENDIF
!      TSUPER=THIS%TSUPER
!!     IF (THIS%TSUPER) THEN
!!       CALL ERROR__MSG('NOT PERMITTED FOR SUPERWAVEFUNCTION INTERFACE')
...
!      CALL MPE__SYNC !THIS IS ONLY USED TO AVOID FILLING UP THE BUFFER
!      IF(THISTASK.EQ.1)THEN
!!
!!       ================================================================
...
!            CALL MPE__RECEIVE(FROMTASK,FROMTASK*2,NGFROM)
!            IF(NGFROM.GT.NGLX) THEN
!              CALL ERROR__MSG('PACKAGE TOO LARGE')
!              CALL ERROR__I4VAL('FROMTASK',FROMTASK)
!              CALL ERROR__I4VAL('NGFROM',NGFROM)
!              CALL ERROR__I4VAL('NGLX',NGLX)
!              CALL ERROR__STOP('PLANEWAVE_COLLECT')
!            END IF
!            CALL MPE__RECEIVE(FROMTASK,FROMTASK*2+1,X_LOCTMP)
!            IGL=0
!            DO IG=1,NG
...
!!       ================================================================
!        CALL MPE__SEND(1,THISTASK*2,NGL)
!        X_LOCTMP(:,1:NGL)=XLOC(:,:)
!        CALL MPE__SEND(1,THISTASK*2+1,X_LOCTMP)
!      ENDIF
!      DEALLOCATE(X_LOCTMP)
!                     CALL TRACE__POP
!      RETURN
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__COLLECTR8(NDIM,NGLOC,XLOC,NGGLOB,XGLOB)
!     ******************************************************************
!     **  PLANEWAVE__COLLECT                                           **
!     ******************************************************************
      USE PLANEWAVE_MODULE
      ...
      INTEGER(4)  ,INTENT(IN) :: NDIM
      INTEGER(4)  ,INTENT(IN) :: NGLOC
      REAL(8)      ,INTENT(IN) :: XLOC(NDIM,NGLOC)
      INTEGER(4)  ,INTENT(IN) :: NGGLOB
      REAL(8)      ,INTENT(OUT):: XGLOB(NDIM,NGGLOB)
      REAL(8)      ,ALLOCATABLE :: X_LOCTMP(:,:)   !(NGLOCX)
      INTEGER(4)               :: NGFROM
      INTEGER(4)               :: FROMTASK
      INTEGER(4)               :: IG,IGL
      INTEGER(4)               :: NGL,NGLX,NG
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__COLLECTI4(NDIM,NGLOC,XLOC,NGGLOB,XGLOB)
!     ******************************************************************
!     **  PLANEWAVE__COLLECT                                           **
!     ******************************************************************
      USE PLANEWAVE_MODULE
      ...
      INTEGER(4)  ,INTENT(IN) :: NDIM
      INTEGER(4)  ,INTENT(IN) :: NGLOC
      INTEGER(4)      ,INTENT(IN) :: XLOC(NDIM,NGLOC)
      INTEGER(4)  ,INTENT(IN) :: NGGLOB
      INTEGER(4)      ,INTENT(OUT):: XGLOB(NDIM,NGGLOB)
      INTEGER(4)      ,ALLOCATABLE :: X_LOCTMP(:,:)   !(NGLOCX)
      INTEGER(4)               :: NGFROM
      INTEGER(4)               :: FROMTASK
      INTEGER(4)               :: IG,IGL
      INTEGER(4)               :: NGL,NGLX,NG
...
!
!     .................................................................
      SUBROUTINE PLANEWAVE__COLLECTC8(NDIM,NGLOC,XLOC,NGGLOB,XGLOB)
!     ******************************************************************
!     **  PLANEWAVE__COLLECT                                           **
!     ******************************************************************
      USE PLANEWAVE_MODULE
      ...
      INTEGER(4)  ,INTENT(IN) :: NDIM
      INTEGER(4)  ,INTENT(IN) :: NGLOC
      COMPLEX(8)      ,INTENT(IN) :: XLOC(NDIM,NGLOC)
      INTEGER(4)  ,INTENT(IN) :: NGGLOB
      COMPLEX(8)      ,INTENT(OUT):: XGLOB(NDIM,NGGLOB)
      COMPLEX(8)      ,ALLOCATABLE :: X_LOCTMP(:,:)   !(NGLOCX)
      INTEGER(4)               :: NGFROM
      INTEGER(4)               :: FROMTASK
      INTEGER(4)               :: IG,IGL
      INTEGER(4)               :: NGL,NGLX,NG
...
!       ================================================================
      ...
      RETURN
      END
!#END TEMPLATE PLANEWAVE__COLLECT
!
!     .................................................................
      SUBROUTINE PLANEWAVE__RSPACECOLLECTR8(NRL,XL,NRG,XG)
!     ******************************************************************
!     **  PLANEWAVE__COLLECT                                           **
!     ******************************************************************
      USE PLANEWAVE_MODULE
      ...
      INTEGER(4)              :: FROMTASK
!     ******************************************************************
                     CALL TRACE__PUSH('PLANEWAVE__RSPACECOLLECTR8')
      IF(.NOT.TINI) THEN
        CALL ERROR__MSG('LIST NOT INITIALIZED')
        CALL ERROR__STOP('PLANEWAVE__RSPACECOLLECTR8')
      ENDIF
      NR1L=THIS%NR1L
      ...
      IF(NRG.NE.NR1G*NR2*NR3) THEN
        CALL ERROR__MSG('GLOBAL %(R-POINTS) INCONSISTENT WITH OBJECT')
        CALL ERROR__STOP('PLANEWAVE__RSPACECOLLECTR8')
      END IF
      IF(NRL.NE.NR1L*NR2*NR3) THEN
        CALL ERROR__MSG('LOCAL %(R-POINTS) INCONSISTENT WITH OBJECT')
        CALL ERROR__STOP('PLANEWAVE__RSPACECOLLECTR8')
      END IF
      XG(:)=0.D0
...
!       ================================================================
        ...
        DO FROMTASK=1,NTASKS
          ...
          IF(FROMTASK.NE.1) THEN
            CALL MPE__RECEIVE(FROMTASK,FROMTASK*2,NRLFROM)
            IF(NRLFROM.GT.NRLX) THEN
              CALL ERROR__MSG('PACKAGE TO LARGE')
              CALL ERROR__STOP('PLANEWAVE__RSPACECOLLECTR8')
            END IF
            CALL MPE__RECEIVE(FROMTASK,FROMTASK*2+1,X_LOCTMP)
            I23=-1
...
!       == OTHER TASKS SEND TO TASK ONE                               ==
!       ================================================================
        CALL MPE__SEND(1,THISTASK*2,NRL)
        X_LOCTMP(1:NRL)=XL(:)
        CALL MPE__SEND(1,THISTASK*2+1,X_LOCTMP)
      ENDIF
      DEALLOCATE(X_LOCTMP)
